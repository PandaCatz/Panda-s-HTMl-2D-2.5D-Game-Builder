...
- Reuse recurring deterministic mechanics through the native interactable registry instead of rebuilding them ad hoc. Search with `list_interactable_templates`, inspect with `get_interactable_template` / `get_interactable_report`, preview one stable-ID bottom-center placement with `preview_interactable_template`, and apply only its unchanged source/template/preview digests. Springs, ladders, conveyors, crumble platforms, key/door pairs, pressure-plate/gate pairs, and one-way drop-through platforms keep art, sensors, solids, supports, logic, save state, and evidence independent. Remove a complete bundle with `remove_interactable_instance`; never delete one role or let generated art own collision. Production instances require calibrated acceptance/replay evidence, and replay v13 preserves exact support/interactable state while v1-v12 remain frozen.
...
- A `runVariationProgram` remains authored tick-zero simulation input. Preview it across explicit seeds or real UTC `YYYY-MM-DD` values; never read wall-clock/local time or use `Math.random()` inside simulation. Keep strict full-program/source compatibility separate from the gameplay-only selection digest so labels, evidence IDs, and presentation ghosts cannot reshuffle variants or alter replay v14. `LR1` codes restart a compatible run at tick zero and are not authenticated score evidence; `LL1` remains the full mid-run save. Ghosts are presentation-only replay observations and never become objects, collision, navigation, support, completion, acceptance, save overrides, or replay hashes.
...
- When bounded orientation is needed, search the native registry with `list_capability_packs` or `query_capability_knowledge`, then inspect an exact revision with `get_capability_pack`. The same registry is available at `looplab://capability-packs` and `/capability-packs.json`; it is deterministic, provider-free, non-executable, and excluded from projects and game exports. Never treat a pack as implementation evidence, collision authority, or a creative winner. Inspect proposed updates only through `inspect_capability_pack_refresh`: reject rollback and same-revision equivocation, require exact digest/source/license/provenance/policy/calibration validity for a higher revision, and remember that inspection never installs it.
...
- Canvas plus the pinned Phaser 3.90.0, PixiJS 8.19.0, and melonJS 17.4.0 adapters are release-ready. Use `set_runtime_profile` rather than manually editing `runtimeProfile` and `release`; it atomically binds one selected primary adapter to truthful one-file delivery metadata. Every adapter still faces the same static, browser, replay, and exact-artifact gates.
...
- Canvas 2D is the compact default. Phaser ships through LoopLab's pinned, SHA-256-checked inline browser script bundle; PixiJS ships through the pinned inline UMD/IIFE plus official CSP polyfill; melonJS ships as the pinned tree-shaken inline IIFE. Each game still has one primary frame owner. Module-only, CDN, network-asset, or multi-file output violates the upload contract.
...
Use `npm run agent -- guide [query]`, `looplab://agent-guide-index`, or browser command `get_agent_guide_index` to find the exact owning section without loading the full guide. The generated digest-bound index is orientation only and cannot execute, mutate, verify, or override policy. See `docs/AI_AGENT_GUIDE.md` for the full protocol and examples.
