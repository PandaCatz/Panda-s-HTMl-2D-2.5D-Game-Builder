...
type ExportReceipt = {
  ...
  privacy: { schemaVersion: string; status: "clear" | "review-required" | "blocked"; sourceDigest: string | null; digest: string; sourceReportDigest: string | null; artifactReportDigest: string | null; findingCount: number; errorCount: number; warningCount: number; matchedValuesReturned: false; proofBoundary: string };
    ...
    const bridge: AgentBridge = {
      ...
      run: async (command) => {
        try {
          ...
          if (command.op === "get_privacy_report") return { ok: true, report: analyzeProject(projectRef.current, { profile: command.profile }).privacyReport };
          ...
          {experienceMode === "director" ? (
            <div className="doctor-sidebar" data-report-fresh={doctorReportFresh ? "true" : "false"} data-analysis-pending={doctorAnalysisPending ? "true" : "false"}>
              ...
              <div className="doctor-scroll">
                ...
                <section id="looplab-project-privacy" className={`visual-readiness-card privacy-preflight-card status-${privacyInspection.status}`} aria-label="Project privacy preflight" data-source-digest={privacyInspection.sourceDigest ?? ""} data-report-digest={privacyInspection.digest} data-status={privacyInspection.status} data-finding-count={privacyInspection.findingCount}>
                  <header><div><span className="eyebrow">Privacy preflight</span><strong>{privacyInspection.status.replaceAll("-", " ")}</strong></div><b>{privacyInspection.findingCount === 0 ? "✓" : privacyInspection.findingCount}</b></header>
                  <p>{privacyInspection.findingCount === 0 ? "No declared private-data pattern was found in the editable project." : `${privacyInspection.errorCount} blocker(s) · ${privacyInspection.warningCount} review finding(s). Matched values are never displayed.`}</p>
                  {privacyInspection.issues.slice(0, 3).map((issue) => <span key={`${issue.code}-${issue.path}`} className={issue.severity === "error" ? "failed" : "warning"} title={issue.action}>{issue.severity === "error" ? "!" : "•"} {issue.code.replaceAll("privacy-", "").replaceAll("-", " ")} · {issue.path}</span>)}
                  <small>{privacyInspection.metrics.scannedStrings.toLocaleString()} text values checked · {privacyInspection.metrics.skippedOpaquePayloads.toLocaleString()} embedded payloads treated as opaque. A clear heuristic scan is not proof of absence.</small>
                </section>
              ...
              {lastExportReceipt ? (
                <div className={`export-receipt ${exportReceiptFresh ? "is-current" : "is-stale"} ${exportReleaseReady ? "is-release-ready" : "is-draft"}`} role="status" aria-live="polite">
                  ...
                  <dl>
                    ...
                    <div><dt>Privacy</dt><dd>{lastExportReceipt.privacy.status === "clear" ? "Clear" : "Blocked"}</dd></div>
