...
9. When implementation or routing knowledge is uncertain, read `looplab://capability-packs` or use the live `list_capability_packs`, `query_capability_knowledge`, and `get_capability_pack` tools. These are deterministic orientation only and require no provider. Inspect proposed updates with `inspect_capability_pack_refresh`; it never installs them.
10. Read Project Doctor and retain `sourceDigest`.
11. Create a variation before risky experimentation.
12. For a proven repeated sequence, call `list_command_macros`, then `preview_command_macro`. Apply only with `apply_command_macro` plus the returned `sourceDigest` and `expansionDigest` when `applicable` is true.
13. For an arbitrary coherent change, call `preview_batch` with the exact current `expectedSourceDigest`, stable-ID commands, and a non-empty summary. Review `applicable`, validation, per-command outcomes, both current/release Doctor deltas, and the canonical `previewDigest`.
14. Apply only by sending the unchanged commands, summary, source digest, and returned preview digest to `apply_previewed_batch`. Any source or plan drift rejects the whole write. Keep `apply_batch` only for legacy callers that deliberately do not use the review receipt.
15. Renew long-running claims. Finish shared work with `update_work_item` and evidence, or use `release_work_item` for a handoff; each uses the latest exact `expectedLedgerDigest`.
16. Re-read the brief/Doctor, run acceptance, completion, replay, runtime-join, and browser evidence gates. Store the latest `currentCursor` for the next session.
17. Export one self-contained offline HTML file only after the exact candidate passes its required gates.
18. Persist accepted shared work with `save_shared_project` and the exact latest full-document revision digest. On 412, preserve the draft and use the explicit preview/apply rebase workflow; do not force an overwrite or auto-save a rebase.
...
- Capability packs are versioned read-only decision context at `looplab://capability-packs`. They cannot execute, download, mutate, own collision, become evidence, or choose a creative winner. License metadata is not proof of rights. Refresh inspection rejects rollback and equivocation but never installs a candidate.
