...
| **Community formats enter through review, not trust** | Tiled JSON/TMX and Aseprite JSON are retained with exact bytes and hashes, projected only through a non-mutating preview, and applied only with matching source/preview digests. External TSX is supplied explicitly, existing collision remains authoritative, and later edits make the original clearly stale rather than falsely “round-tripped.” |
...
Tiled and Aseprite use the same guarded exchange model through the Agent API, CLI/MCP, Codex, Claude, and a mouse-accessible **Tiled + Aseprite exchange** panel. Tiled JSON/TMJ and uncompressed TMX may bring in visual tile layers after every tileset is bound to an existing embedded atlas; referenced TSX or tileset JSON must be supplied explicitly. GID transforms, infinite-map origin shifts, stable layer IDs, exact 128×64 dimetric projection, and unknown source fields are retained or reported. Object layers and tileset collision objects remain advisory—LoopLab preserves its separately authored collision layers and profiles byte-for-byte. Aseprite JSON array/hash metadata may update an existing embedded atlas and renderer-neutral animation machine only when frames form an unrotated, untrimmed uniform grid. Unequal frame timing requires explicit reviewed approximation, while native `.ase` binaries and ambiguous layouts fail closed.

Every imported source lives under authoring-only metadata and is omitted from gameplay digests and the shipped one-file runtime. An unchanged source can be viewed or downloaded byte-identically. Once canonical LoopLab targets change, the retained original is labelled stale and can be exported only by explicit opt-in; exchange v1 never claims to regenerate a current Tiled/Aseprite document after later edits.
...
### Deterministic continuous-world chunks

Large 2D and 2.5D routes can be composed from authored source maps without turning the renderer into a second level editor. A strict `looplab-world-stream/v1` program references stable chunk templates, exact entry/exit sockets, a finite sequence or seeded route, and explicit residency, tile-cell, collision-cell, decoded-RGBA, and culling budgets. Socket compatibility includes edge, tag, cross-axis coordinate, support z, and span. Compatible candidates are stable-ID sorted and chosen from canonical SHA-256 identity, so array order and `Math.random()` never change the route.

Version 1 is deliberately bounded to horizontal or vertical embedded map chunks. It does not claim arbitrary branching, rotation, scaling, network streaming, or 3D. Templates with portals, motion bodies, traversal paths, elevation transitions, navigation, actors, or combat are rejected until those systems have an explicit remapping contract; art and tile pixels never create adjacency or collision. The runtime rotates a bounded resident window around the same live player, activates already embedded data with zero fetches, counts decoded assets once per unique asset, and keeps camera culling separate from residency. Save-state v6 and replay v12 preserve deterministic route identity and current chunk position while excluding camera, asset-readiness, first-draw timing, and other visual state from simulation hashes.

Map Studio's **World** composer and the headless `get_world_stream`, `get_world_stream_report`, `get_world_stream_plan`, `suggest_world_stream`, `set_world_stream`, and `remove_world_stream` commands operate on the same source. Runtime API 2.35 exposes `getWorldStreamState()` / `get_world_stream_state`; `markWorldStreamDraw()` / `mark_world_stream_draw` records renderer evidence without changing gameplay truth. Project Doctor can prove schema, exact socket alignment, deterministic planning, bounded residency, and supported-system scope. A valid plan is not permission to call a join visually seamless: production still requires captured first-draw, asset-ready, unique-pixel seam evidence and a real one-file browser crossing.

### Seeded runs, daily challenges, and replay ghosts

LoopLab can author controlled replayable variety without letting randomness leak into rendering or wall-clock state. A strict `looplab-run-variation-program/v1` assigns typed gameplay variables through stable-ID variation pools. Selection happens before simulation tick zero from canonical SHA-256 identity; array order and `Math.random()` cannot reshape a run. Standard runs require an explicit seed. Daily challenges require an explicit real UTC `YYYY-MM-DD` value, so the host may choose “today” while the simulation never reads the clock. Gameplay variables declare whether a new run resets them or preserves them for the current session.

Fine Tune's **Runs · seeds, dailies & ghosts** panel and the canonical `get_run_variation_program`, `get_run_variation_report`, `suggest_run_variation_program`, `preview_run_variation`, `set_run_variation_program`, and `remove_run_variation_program` commands edit the same source. A strict full-program digest protects save/run-code compatibility, while a separate gameplay-only selection digest excludes labels, evidence IDs, and presentation ghosts. Changing a ghost color or improving a label therefore cannot reshuffle variants or invalidate replay v14. Production Doctor requires valid typed assignments, non-conflicting variable ownership, explicit reset policies, and replay evidence from at least two authored run seeds or UTC days.

Runtime API 2.35 exposes preview/start methods for standard and daily runs plus presentation-only ghost observations. Compact `LR1` codes identify a source-bound tick-zero run and detect accidental corruption; they do not authenticate scores or replace `LL1` mid-run saves. A ghost can be previewed and applied only from an exact source-bound replay trajectory. Ghost frames never enter runtime objects, collision, navigation, save overrides, completion, or replay hashes. Canvas, Phaser, Pixi, and melon render the same canonical run state and optional ghost observations inside one offline HTML file.
...
Its **Event-driven presentation** workbench now authors four connected renderer-neutral layers: procedural or selected embedded OGG/WAV sound, event feedback, map-bound camera zones, and explicit sprite animation state machines. Camera overlaps resolve deterministically by priority, area, and stable ID. Animation transitions bind to runtime events, semantic actions, movement, grounded state, authored runtime state, or completion, with explicit immediate/frame/cycle/locked interruption behavior. Reusable effect plugins contain only checked particles, shake, flash, and squash primitives; each plugin must replace or omit motion under reduced-motion preference and may declare exact embedded asset/frame requirements. Fine Tune exposes structured tabs plus advanced JSON, while Codex, Claude, CLI, MCP, and the browser bridge use the same whole-program commands without opening the UI. Project Doctor blocks missing maps, targets, sprites, frames, plugins, assets, reduced-motion variants, and arbitrary fields. The identical controller drives editor preview and the offline one-file export across Canvas, Phaser, Pixi, and melon; presentation never enters collision, completion, simulation, acceptance, or replay hashes. Technical validity still does not prove artistic quality or game feel.

### Deterministic moving objects

Moving platforms, hazards, doors, carriers, and environmental machinery can now follow authored paths under fixed-tick simulation. They may respond to a held semantic action or run automatically, use stop or ping-pong endpoints, and resolve substepped X/Y collision against authored solids and the player. Canvas, Phaser, PixiJS, and melonJS render the same canonical pose; none owns a parallel physics object.
...
### Reusable authored interactables

LoopLab now ships a versioned native vocabulary for seven recurring mechanics: springs or impulse pads, ladders, conveyors, crumble platforms, logical keys with doors, pressure plates with exact bound gates, and target-specific drop-through one-way platforms. These are complete authored behavior bundles, not sprite presets. Each template separates visible art, ground-contact anchors, support footprints, sensors, solid collision, parameters, emitted events, feature-contract starters, acceptance requirements, and replay-fixture starters. Generated art remains presentation-only and `collisionOwner: "authored-map"` stays explicit.

Codex, Claude, CLI, MCP, the browser bridge, and the mouse workbench all consume the same strict registry and six-command workflow: `list_interactable_templates`, `get_interactable_template`, `get_interactable_report`, `preview_interactable_template`, `apply_interactable_template`, and `remove_interactable_instance`. Preview is bound to the exact project source, template revision, and canonical preview digest. Apply refuses stale source or altered bytes, while removal deletes the complete stable-ID instance instead of leaving detached sensors or blockers.

The fixed-tick runtime uses swept sensors, exact support ownership, explicit ladder controller state, bounded cooldown/countdown ticks, logical inventory IDs, and safe collider enable/disable transitions. Portable saves retain logical keys and exact object overrides. Replay v13 adds support/interactable state that can change later ticks while preserving every v1-v12 projection. Project Doctor rejects changed template bytes, missing roles, sensor/solid drift, broken targets, and production instances without calibrated acceptance/replay evidence. Canvas, Phaser, PixiJS, and melonJS all render the same canonical objects and execute the same one-file runtime; none owns a parallel mechanic implementation.
...
LoopLab is not organized around platformers. Genre-neutral choice/dialogue pages, typed variables, bounded integer formulas, turn/day/round clocks, state rules, events, quests, objectives, and accessible HUD bindings can drive narrative, trading, management, tactics, RPG, puzzle, top-down, platforming, or dimetric games. The built-in **Lantern Market Ledger** starter deliberately has no player object or movement loop; it proves the same systems through semantic input, Project Doctor acceptance, replay v4, headless control, visual preview, and one-file export.

Dialogue pages can bind stable line and speaker IDs to the Narrative Contract and an optional portrait to an embedded asset. Quests are not a second story VM or mutable store: their journal derives status and objective progress from the same deterministic variables and rules used by preview, acceptance, replay, save/restore, headless control, and the offline artifact. Typewriter reveal is presentation-only, reduced-motion users receive instant text, the full readable line is exposed atomically, and dialogue never auto-advances in the current contract.
...
LoopLab currently exposes **217 core project commands** and **306 browser-session commands** through one versioned manifest. Every command carries a JSON Schema 2020-12 input contract, surface ownership, mutation metadata, and MCP safety annotations instead of relying on an agent to infer arguments from prose.
