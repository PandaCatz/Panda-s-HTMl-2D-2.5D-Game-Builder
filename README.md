...
| **The runtime is chosen for the game** | LoopLab owns Canvas, Phaser, PixiJS, and melonJS decision knowledge, then chooses only a release-ready adapter. Canvas is lean; optional Phaser 3.90.0 is embedded and SHA-256 checked when its scenes, cameras, tilemaps, animation, or physics tooling adds value. PixiJS/melonJS knowledge can improve native design decisions without pretending their pending adapters already ship. Existing projects are never silently migrated. |
...
flowchart LR
    ...
    D --> E["Doctor, replay, joins, and browser QA"]
    E -->|"Needs work"| C
...
6. **Prove the result.** Run Project Doctor, deterministic replay and acceptance fixtures, runtime transition checks, responsive visual capture, and Playwright-based browser QA.
...
Candidate selection is shared too. `compare_iterations` returns a source-bound `looplab-candidate-decision/v1` packet to the UI, CLI, MCP, Codex, and Claude. Hard gates constrain feasibility and matching frozen-profile dimensions expose dominance or tradeoffs, but `automaticWinner` remains `null`: both candidates must be previewed and played, and a human-directed choice determines the next editable parent.
...
Its **Event-driven presentation** workbench maps canonical and authored gameplay events to bounded procedural Web Audio cues plus renderer-neutral particles, shake, flash, and squash. The same strict `presentationProgram` drives editor preview, Canvas exports, and the pinned Phaser draw hook; input is recorded before audio unlock, audio failure is isolated, reduced-motion disables spatial motion effects, and presentation never enters collision, completion, simulation, acceptance, or replay hashes. Agents can suggest, inspect, set, remove, preview, mute, and diagnose it through the canonical headless contract. Project Doctor can require it and reports event coverage without pretending that schema validity proves artistic quality or game feel.
...
LoopLab currently exposes **162 core project commands** and **243 browser-session commands** through one versioned manifest. Every command carries a JSON Schema 2020-12 input contract, surface ownership, mutation metadata, and MCP safety annotations instead of relying on an agent to infer arguments from prose.
...
Canvas and Phaser are both verified against this contract. Canvas uses LoopLab's built-in inline runtime. `set_runtime_profile` can select Phaser, which embeds the pinned Phaser 3.90.0 browser build directly in the document, authenticates its exact SHA-256 during the static audit, and exposes the loaded adapter/version through the offline runtime bridge. The hostile Playwright harness still blocks every actual external request. An ES-module import, CDN script, or runtime asset request is a release blocker. PixiJS and melonJS decision knowledge is native, but their engine adapters are not described as release-ready until they pass these same gates. No engine adapter owns deterministic simulation, authored collision, or saveable project truth.
...
npm run public:audit      # scan the publish set without printing suspected secret values
...
The repository excludes `.env*`, `.looplab/`, private Claude handoffs, local MCP/agent settings, deployment bindings, DPAPI blobs, and common key/certificate files. `npm run public:audit` scans every tracked and publish-candidate text file for credential formats, email addresses, private Windows paths, and forbidden local state while returning only category, path, and line—never a suspected value. Key-shaped strings in security tests are synthetic and carry explicit test markers.
