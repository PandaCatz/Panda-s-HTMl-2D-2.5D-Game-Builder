import { analyzeColorAccessibility } from "./looplab-color-accessibility.mjs";
...
/**
 * @param {{ captureId?: unknown, sourceDigest?: unknown, frame?: VisualPixelFrame, baselineFrame?: VisualPixelFrame | null, baselineSha256?: unknown, semanticTargets?: any[], colorTargets?: any[], options?: Record<string, unknown> }} [input]
 */
export function analyzeVisualPerception({ captureId, sourceDigest, frame, baselineFrame = null, baselineSha256 = null, semanticTargets = [], colorTargets = [], options = {} } = {}) {
  ...
  const colorAccessibility = analyzeColorAccessibility({ captureId, sourceDigest, frame: current, targets: colorTargets, options: options.colorAccessibility && typeof options.colorAccessibility === "object" ? options.colorAccessibility : {} });
  const colorAnnotations = colorAccessibility.issues.map((entry) => {
    const bounds = normalizeVisualBounds(entry.bounds, current.width, current.height);
    if (!bounds) return null;
    return {
      kind: entry.kind,
      ...
      label: entry.label,
      detail: entry.detail,
      source: "color-accessibility",
      sourceEvidenceIds: [entry.id],
      affectedIds: [entry.targetId],
      bounds,
      metrics: entry.metrics ?? {},
    };
  }).filter(Boolean);
  const annotations = [...semantics, ...colorAnnotations, ...changes].map((annotation, index) => ({
    ...annotation,
