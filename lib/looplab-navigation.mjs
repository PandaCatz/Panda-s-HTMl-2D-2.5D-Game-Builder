...
export function importPathEditorNavigation(data, map) {
  ...
  const nodeWorldPositions = extension.nodeWorldPositions && typeof extension.nodeWorldPositions === "object" ? extension.nodeWorldPositions : {};
  const areaHeights = extension.areaHeights && typeof extension.areaHeights === "object" ? extension.areaHeights : {};
  const areaWorldPoints = extension.areaWorldPoints && typeof extension.areaWorldPoints === "object" ? extension.areaWorldPoints : {};
  ...
  const nodes = (data.nodes ?? []).map((node, index) => {
    ...
    const exact = nodeWorldPositions[id];
    const position = exact && typeof exact === "object" && !Array.isArray(exact)
      ? { x: finite(exact.x), y: finite(exact.y), z: finite(exact.z, finite(nodeHeights[id])) }
      : fromPercent(node, finite(nodeHeights[id]));
    return {
      id,
      ...position,
  ...
  const areas = (data.areas ?? []).map((area, index) => {
    ...
    return {
      ...
      name: String(area.name || `${area.kind === "blocked" ? "Blocked" : "Walkable"} area ${index + 1}`),
      kind: area.kind === "blocked" ? "blocked" : "walkable",
      points: (Array.isArray(areaWorldPoints[id]) ? areaWorldPoints[id] : area.points ?? []).map((point) => Array.isArray(areaWorldPoints[id])
        ? { x: finite(point?.x), y: finite(point?.y), z: finite(point?.z, zMin) }
        : fromPercent(point, zMin)),
...
export function exportPathEditorNavigation(input, map = {}) {
  ...
  const nodeWorldPositions = Object.fromEntries(navigation.nodes.map((node) => [node.id, { x: node.x, y: node.y, z: node.z }]));
  const areaHeights = Object.fromEntries(navigation.areas.map((area) => [area.id, { zMin: area.zMin, zMax: area.zMax }]));
  const areaWorldPoints = Object.fromEntries(navigation.areas.map((area) => [area.id, cloneJson(area.points)]));
  ...
  return {
    ...
    looplab: {
      ...
      areaHeights,
      areaWorldPoints,
      ...(map.elevationTransitions ? {
        elevationTransitions: cloneJson(map.elevationTransitions),
        elevationTransitionPolicy: "lossless authored world x/y/z support corridors; navigation links and collision chains retain independent authority",
      } : {}),
      ...(authoredRoute ? {
        authoredRoute,
