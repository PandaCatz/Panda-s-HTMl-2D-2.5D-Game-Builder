...
import { inspectRunVariationProgram } from "./looplab-run-variation.mjs";
...
function analyzeRunVariationProgram(project, issues, strict, sourceDigest) {
  const report = inspectRunVariationProgram(project, project.runVariationProgram, { strict, sourceDigest });
  for (const finding of report.issues) addIssue(issues, finding.severity, finding.code, finding.message, finding.code.includes("ghost") ? "visuals" : "gameplay", {
    path: finding.path,
    poolId: finding.poolId,
    variantId: finding.variantId,
    variableId: finding.variableId,
    ghostId: finding.ghostId,
    replayCaseId: finding.replayCaseId,
    testId: finding.testId,
    action: finding.code.includes("ghost") ? "Rebuild the ghost from its exact passing replay preview; never repair trajectory pixels or collision by hand." : "Repair the authored pool, typed assignment, explicit UTC seed input, or two-seed evidence, then rerun replay and Project Doctor.",
    invalidates: ["runVariationProgram", "replay", "save-state", "one-file-export", "candidate-verification"],
    evidenceRequired: finding.code.includes("ghost") ? ["deterministic-replay", "browser-harness", "visual-review"] : ["project-doctor", "two-seed-replay", "one-file-export"],
  });
  return report;
}
...
export function analyzeProject(project, options = {}) {
  ...
  const interactableReport = inspectInteractables(project, { requireEvidence: strict });
  for (const message of interactableReport.errors) addIssue(issues, "error", "interactable-template-invalid", message, "gameplay", {
    action: "Repair or rematerialize the complete native interactable instance, then rerun its deterministic replay and browser playtest.",
    invalidates: ["interactable-template", "collision", "replay", "save-state", "candidate-verification"],
    evidenceRequired: ["project-doctor", "deterministic-replay", "browser-harness"],
  });
  for (const message of interactableReport.warnings) addIssue(issues, strict ? "error" : "warning", "interactable-evidence-missing", message, "tests", {
    action: "Calibrate and install the instance's supplied acceptance and replay fixture templates, then record current-source evidence.",
    invalidates: ["interactable-template", "replay", "candidate-verification"],
    evidenceRequired: ["acceptance-suite", "deterministic-replay", "browser-playtest"],
  });
  ...
  const runVariationReport = analyzeRunVariationProgram(project, issues, strict, sourceDigest);
