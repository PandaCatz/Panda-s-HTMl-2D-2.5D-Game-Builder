...
function analyzeBrowser2DContracts(project, issues, strict) {
  ...
  const expectedDelivery = framework === "phaser" ? "inline-script-tag" : framework === "pixi" ? "inline-umd" : framework === "melon" ? "tree-shaken-inline-iife" : "built-in-inline";
  if (framework === "pixi" || framework === "melon") addIssue(issues, "error", "runtime-adapter-unavailable", `${framework} capability and decision knowledge is installed, but its exact one-file runtime adapter has not passed LoopLab's release gates yet.`, "release");
...
export function analyzeProject(project, options = {}) {
  ...
  const nextActions = actionableIssues
    ...
    .slice(0, 8)
    .map((issue) => ({ code: issue.code, owner: issue.owner, action: issue.action, context: { mapId: issue.mapId, objectId: issue.objectId, assetId: issue.assetId, actionId: issue.actionId, featureId: issue.featureId, beatId: issue.beatId, endingId: issue.endingId, pageId: issue.pageId, choiceId: issue.choiceId, lineId: issue.lineId, characterId: issue.characterId, testId: issue.testId, parameterId: issue.parameterId, objectiveId: issue.objectiveId, constraintId: issue.constraintId, cueId: issue.cueId, event: issue.event, path: issue.path, target: issue.target, campaignScope: issue.campaignScope, expectedMinMaps: issue.expectedMinMaps, expectedMaxMaps: issue.expectedMaxMaps, actualMapCount: issue.actualMapCount, maxJumpRise: issue.maxJumpRise, maximumHorizontalTravel: issue.maximumHorizontalTravel, requiredRise: issue.requiredRise, requiredHorizontalGap: issue.requiredHorizontalGap, predictionMethod: issue.predictionMethod, completionStatus: issue.completionStatus, completionReason: issue.completionReason, completionConfigDigest: issue.completionConfigDigest, exploredStates: issue.exploredStates, expandedTransitions: issue.expandedTransitions }, evidenceRequired: issue.evidenceRequired }));
  return {
    profile,
    digest,
    sourceDigest,
