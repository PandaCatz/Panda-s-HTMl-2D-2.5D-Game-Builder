...
export function buildClaudeOperabilityArgs({
  projectPath,
  ...
  const budget = Math.max(0.01, Math.min(5, Number(maxBudgetUsd) || 0.25));
  const args = [
    "-p",
  ...
  if (typeof effort === "string" && ["low", "medium", "high", "xhigh", "max"].includes(effort.trim())) args.push("--effort", effort.trim());
