...
const AUDIO_KINDS = new Set(["tone", "noise"]);
...
const ALLOWED_KEYS = Object.freeze({
  ...
  audioCue: new Set(["id", "event", "enabled", "kind", "waveform", "frequency", "endFrequency", "filterFrequency", "durationMs", "attackMs", "releaseMs", "volume", "pitchVariationCents"]),
...
export function inspectPresentationProgram(project, input = project?.presentationProgram, options = {}) {
  ...
  if (!input || typeof input !== "object" || Array.isArray(input)) return {
    ...
    sourceDigest,
    program: null,
    metrics: { audioCueCount: 0, motionCueCount: 0, effectCount: 0, mappedEventCount: 0, feedbackCoverage: 0 },
    mappedEvents: [],
...
export function createPresentationRuntime(inputProgram, options = {}) {
  ...
  function triggerAudioCue(cue, event, fromQueue) {
    ...
    safeCall(function () {
      ...
      const duration = Math.max(0.02, Math.min(2, Number(cue.durationMs || 140) / 1000));
  ...
  function getStatus() {
    return {
      enabled: enabled,
      audio: { enabled: audioConfig.enabled !== false && enabled, state: audioState, muted: muted, masterVolume: masterVolume, contextState: context?.state || null, activeVoices: voices.length, pendingEvents: pendingAudio.length, triggeredCueCount: triggeredAudioCueCount, error: audioError },
  ...
  function destroy() { disposed = true; stopVoices(); pendingAudio = []; clearMotion(); media?.removeEventListener?.("change", onMediaChange); try { context?.close?.(); } catch { /* Context teardown is best effort and never affects game state. */ } }
