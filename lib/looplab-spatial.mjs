...
export function analyzeSpatialProject(project) {
  ...
  if (maps.length) {
    ...
    for (let index = 0; index < maps.length - 1; index += 1) {
      ...
      if (!forwardPortal) add("error", "map-route-gap", `${source.name} does not have an authored exit to the next map, ${target.name}.`, { mapId: source.id, targetMapId: target.id });
    }
    ...
    for (const map of maps) if (!reachable.has(map.id)) add("error", "map-unreachable", `${map.name} cannot be reached from the first map through authored portal connections.`, { mapId: map.id, startMapId });
  }
