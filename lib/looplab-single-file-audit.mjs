...
import { LOOPLAB_PIXI_BROWSER_SHA256, LOOPLAB_PIXI_BROWSER_VERSION } from "./generated/looplab-pixi-browser-bundle.mjs";
import { LOOPLAB_MELON_BROWSER_SHA256, LOOPLAB_MELON_BROWSER_VERSION } from "./generated/looplab-melon-browser-bundle.mjs";
import {
  exportProfileId,
  inspectSaveProgram,
...
import { inspectStandaloneArtifactPrivacy } from "./looplab-project-privacy.mjs";
...
const TRUSTED_RUNTIME_VENDORS = Object.freeze({
  phaser: Object.freeze({ label: "Phaser", version: LOOPLAB_PHASER_BROWSER_VERSION, sha256: LOOPLAB_PHASER_BROWSER_SHA256, delivery: "inline-script-tag" }),
  pixi: Object.freeze({ label: "PixiJS", version: LOOPLAB_PIXI_BROWSER_VERSION, sha256: LOOPLAB_PIXI_BROWSER_SHA256, delivery: "inline-umd-with-official-csp-polyfill" }),
  melon: Object.freeze({ label: "melonJS", version: LOOPLAB_MELON_BROWSER_VERSION, sha256: LOOPLAB_MELON_BROWSER_SHA256, delivery: "tree-shaken-inline-iife" }),
});
...
export function auditStandaloneHtml(html, options = {}) {
  ...
  const trustedRuntimeScriptTags = [];
  for (const script of scripts) {
    const attributes = script[1] ?? "";
    const source = script[2] ?? "";
    const closingAttributes = script[3] ?? "";
    if (closingAttributes.trim()) add("error", "malformed-script-close-tag", "Closing script tags cannot contain attributes or trailing content.");
    ...
    if (vendor) {
      ...
      const trusted = TRUSTED_RUNTIME_VENDORS[vendor];
      if (!trusted) add("error", "unknown-runtime-vendor", `Embedded runtime vendor ${vendor} is not allowlisted.`);
      else if (declaredVersion !== trusted.version) add("error", "runtime-vendor-version", `Embedded ${trusted.label} version ${declaredVersion ?? "(missing)"} does not match pinned version ${trusted.version}.`);
      else if (declaredSha256 !== trusted.sha256 || actualSha256 !== trusted.sha256) add("error", "runtime-vendor-integrity", `The embedded ${trusted.label} browser bundle does not match LoopLab's pinned SHA-256.`);
      ...
      if (trustedVendor) trustedRuntimeScriptTags.push({ vendor, tag: script[0] });
    }
  ...
  if (project) {
    ...
    const selectedVendor = TRUSTED_RUNTIME_VENDORS[framework];
    if (selectedVendor) {
      const trustedSelectedVendors = embeddedRuntimeVendors.filter((entry) => entry.vendor === framework && entry.trusted);
      if (trustedSelectedVendors.length !== 1) add("error", "runtime-vendor-missing", `A ${selectedVendor.label} project must contain exactly one byte-verified pinned ${selectedVendor.label} browser bundle.`);
      if (embeddedRuntimeVendors.length !== 1) add("error", "runtime-vendor-count", `A ${selectedVendor.label} project must embed exactly one selected runtime vendor and no competing engine bundle.`);
      if (project.release?.engineDelivery !== selectedVendor.delivery) add("error", "runtime-delivery-metadata", `A ${selectedVendor.label} project must record ${selectedVendor.delivery} engine delivery.`);
  ...
  let privacySubject = html;
  for (const trustedScript of trustedRuntimeScriptTags) privacySubject = privacySubject.replace(trustedScript.tag, `<script data-looplab-vendor="${trustedScript.vendor}">/* exact SHA-256-authenticated vendor bytes omitted from private user-data scan */</script>`);
  const privacy = inspectStandaloneArtifactPrivacy(privacySubject, project, { sourceDigest: options.sourceDigest });
  for (const finding of privacy.issues) {
    const code = finding.kind === "credential" || finding.kind === "private-key" || finding.kind === "credential-field"
      ? "embedded-credential"
      : "embedded-private-data";
    add("error", code, `${finding.message} Structural location: ${finding.path}. The matched value is intentionally omitted.`);
  }
  ...
  check("no-private-data", !uniqueErrors.some((issue) => issue.code === "embedded-private-data"), "Personal email addresses and absolute local paths are rejected without disclosing matched values");
