...
import { validUtcRunDay } from "./looplab-run-variation-runtime.mjs";
...
export const LOOPLAB_REPLAY_RUN_VARIATION_HASH_VERSION = 14;
export const LOOPLAB_REPLAY_HASH_VERSION = LOOPLAB_REPLAY_RUN_VARIATION_HASH_VERSION;
...
function runVariationReplaySnapshot(state) {
  ...
  return {
    ...
    mode: state.mode,
    seed: state.seed,
    utcDay: state.utcDay ?? null,
    selectionProgramDigest: state.selectionProgramDigest,
    selectionAlgorithm: state.selectionAlgorithm,
    selectionDigest: state.selectionDigest,
    selections: clone(state.selections ?? []),
...
export function captureReplayState(runtime, options = {}) {
  ...
  return {
    ...
    ...(hashVersion >= LOOPLAB_REPLAY_RUN_VARIATION_HASH_VERSION ? { runVariation: runVariationReplaySnapshot(runtime.getRunVariationState?.() ?? state.runVariation ?? null) } : {}),
  };
}
...
export function validateReplayCase(replayCase, options = {}) {
  ...
  if (replayCase.runSeed !== undefined && (typeof replayCase.runSeed !== "string" || !replayCase.runSeed.trim() || replayCase.runSeed.length > 128)) errors.push(`${prefix}.runSeed must contain 1 through 128 characters when provided.`);
  if (replayCase.utcDay !== undefined && !validUtcRunDay(replayCase.utcDay)) errors.push(`${prefix}.utcDay must be a real UTC date in YYYY-MM-DD format when provided.`);
  if (replayCase.runSeed !== undefined && replayCase.utcDay !== undefined) errors.push(`${prefix} cannot provide both runSeed and utcDay.`);
...
export function runReplayCase(project, replayCase, options = {}) {
  ...
  if (replayCase.utcDay !== undefined) runtime.startDailyChallenge({ utcDay: replayCase.utcDay });
  else if (replayCase.runSeed !== undefined) runtime.startRun({ seed: replayCase.runSeed });
  ...
  const capturePlayerEvery = Number.isSafeInteger(Number(options.capturePlayerEveryTicks)) && Number(options.capturePlayerEveryTicks) > 0 ? Math.min(60, Number(options.capturePlayerEveryTicks)) : null;
  const playerTrajectory = [];
  let ghostFacingX = 1;
  const capturePlayer = (tick) => {
    const state = runtime.getState();
    if (!state.player) return;
    if (Number(state.player.vx || 0) < 0) ghostFacingX = -1;
    else if (Number(state.player.vx || 0) > 0) ghostFacingX = 1;
    playerTrajectory.push({ tick, mapId: state.activeMapId, x: Number(state.player.x), y: Number(state.player.y), z: Number(state.player.z || 0), facingX: ghostFacingX });
  };
  if (capturePlayerEvery) capturePlayer(0);
  let finalHash = "";
  ...
  for (let tickIndex = 0; tickIndex < replayCase.tickCount; tickIndex += 1) {
    ...
    if (capturePlayerEvery && (tick % capturePlayerEvery === 0 || tick === replayCase.tickCount)) capturePlayer(tick);
  ...
  return {
    ...
    runSeed: replayCase.runSeed ?? null,
    utcDay: replayCase.utcDay ?? null,
    ...
    emittedEventCounts,
    ...(capturePlayerEvery ? { playerTrajectory } : {}),
  };
}
...
export function recordReplayCase(project, specification) {
  ...
  const draft = {
    ...
    ...(specification.runSeed !== undefined ? { runSeed: String(specification.runSeed) } : {}),
    ...(specification.utcDay !== undefined ? { utcDay: String(specification.utcDay) } : {}),
