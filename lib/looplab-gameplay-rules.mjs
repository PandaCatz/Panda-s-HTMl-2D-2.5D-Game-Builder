...
const DIALOGUE_REVEAL_MODES = new Set(["instant", "typewriter"]);
const DIALOGUE_PORTRAIT_SIDES = new Set(["left", "right"]);
const QUEST_STATUS = Object.freeze({ inactive: 0, active: 1, completed: 2, failed: 3 });
const QUEST_STATUS_VALUES = new Set(Object.values(QUEST_STATUS));
const QUEST_MAXIMUMS = Object.freeze({ quests: 64, objectivesPerQuest: 32 });
...
function normalizeChoicePage(page = {}) {
  return {
    ...
    lineId: typeof page.lineId === "string" ? page.lineId.trim() : undefined,
    speakerId: typeof page.speakerId === "string" ? page.speakerId.trim() : undefined,
    portraitAssetId: typeof page.portraitAssetId === "string" ? page.portraitAssetId.trim() : undefined,
    portraitSide: page.portraitSide === undefined ? undefined : String(page.portraitSide).trim(),
    revealMode: page.revealMode === undefined ? "instant" : String(page.revealMode).trim(),
    charactersPerSecond: page.charactersPerSecond === undefined ? undefined : Number(page.charactersPerSecond),
    ariaLabel: typeof page.ariaLabel === "string" ? page.ariaLabel.trim() : undefined,
...
function normalizeQuestObjective(objective = {}) {
  return {
    ...
    label: String(objective.label ?? objective.id ?? "").trim(),
    visibleWhen: Array.isArray(objective.visibleWhen) ? objective.visibleWhen.map(normalizeCondition) : [],
    completeWhen: Array.isArray(objective.completeWhen) ? objective.completeWhen.map(normalizeCondition) : [],
    progressVariableId: typeof objective.progressVariableId === "string" ? objective.progressVariableId.trim() : undefined,
    target: objective.target === undefined ? undefined : Number(objective.target),
    acceptanceTestIds: Array.isArray(objective.acceptanceTestIds) ? objective.acceptanceTestIds.map((id) => String(id).trim()).filter(Boolean) : [],
  };
}
...
function normalizeQuest(quest = {}) {
  return {
    ...
    title: String(quest.title ?? quest.id ?? "").trim(),
    description: String(quest.description ?? "").trim(),
    giverId: typeof quest.giverId === "string" ? quest.giverId.trim() : undefined,
    statusVariableId: String(quest.statusVariableId ?? "").trim(),
    visibleWhen: Array.isArray(quest.visibleWhen) ? quest.visibleWhen.map(normalizeCondition) : [],
    activationRuleId: typeof quest.activationRuleId === "string" ? quest.activationRuleId.trim() : undefined,
    completionRuleId: typeof quest.completionRuleId === "string" ? quest.completionRuleId.trim() : undefined,
    failureRuleId: typeof quest.failureRuleId === "string" ? quest.failureRuleId.trim() : undefined,
    rewardRuleId: typeof quest.rewardRuleId === "string" ? quest.rewardRuleId.trim() : undefined,
    objectives: Array.isArray(quest.objectives) ? quest.objectives.map(normalizeQuestObjective) : [],
    acceptanceTestIds: Array.isArray(quest.acceptanceTestIds) ? quest.acceptanceTestIds.map((id) => String(id).trim()).filter(Boolean) : [],
  };
}
...
export function normalizeGameplayProgram(input = {}) {
  return {
    ...
    quests: Array.isArray(input.quests) ? input.quests.map(normalizeQuest) : [],
...
export function inspectGameplayProgram(project, input = project?.gameplayProgram) {
  if (!input || typeof input !== "object" || Array.isArray(input)) {
    return { present: false, program: null, errors: [], warnings: [], metrics: { variableCount: 0, ruleCount: 0, executableRuleCount: 0, choicePageCount: 0, choiceCount: 0, dialoguePageCount: 0, questCount: 0, questObjectiveCount: 0, clockCount: 0, hudBindingCount: 0, triggerTypes: [] } };
  }
  ...
  const assetIds = new Set((project?.assets ?? []).map((asset) => asset?.id).filter(stableId));
  const narrativeCharacterIds = new Set((project?.narrativeContract?.characters ?? []).map((character) => character?.id).filter(stableId));
  const narrativeLineIds = new Set((project?.narrativeContract?.lines ?? []).map((line) => line?.id).filter(stableId));
  ...
  for (const [pageIndex, page] of program.choicePages.entries()) {
    ...
    if (page.lineId && !narrativeLineIds.has(page.lineId)) errors.push(`${prefix}.lineId references missing Narrative Contract line ${page.lineId}.`);
    if (page.speakerId && !narrativeCharacterIds.has(page.speakerId)) errors.push(`${prefix}.speakerId references missing Narrative Contract character ${page.speakerId}.`);
    if (page.portraitAssetId && !assetIds.has(page.portraitAssetId)) errors.push(`${prefix}.portraitAssetId references missing project asset ${page.portraitAssetId}.`);
    if (page.portraitSide !== undefined && !DIALOGUE_PORTRAIT_SIDES.has(page.portraitSide)) errors.push(`${prefix}.portraitSide must be left or right.`);
    if (!DIALOGUE_REVEAL_MODES.has(page.revealMode)) errors.push(`${prefix}.revealMode must be instant or typewriter.`);
    if (page.revealMode === "typewriter" && (!Number.isFinite(page.charactersPerSecond) || page.charactersPerSecond < 5 || page.charactersPerSecond > 120)) errors.push(`${prefix}.charactersPerSecond must be a finite number from 5 through 120 for typewriter reveal.`);
    if (page.revealMode !== "typewriter" && page.charactersPerSecond !== undefined) warnings.push(`${prefix}.charactersPerSecond is ignored unless revealMode is typewriter.`);
  ...
  if (program.quests.length > QUEST_MAXIMUMS.quests) errors.push(`gameplayProgram.quests must contain at most ${QUEST_MAXIMUMS.quests} quests.`);
  const ruleById = new Map(program.rules.map((rule) => [rule.id, rule]));
  const validateQuestRule = (quest, field, expectedStatus, prefix) => {
    const ruleId = quest[field];
    if (!ruleId) return;
    const rule = ruleById.get(ruleId);
    if (!rule) {
      errors.push(`${prefix}.${field} references missing gameplay rule ${ruleId}.`);
      return;
    }
    if (expectedStatus !== undefined && !rule.effects.some((effect) => effect.type === "set-variable" && effect.variableId === quest.statusVariableId && effect.value === expectedStatus)) {
      errors.push(`${prefix}.${field} must reference a rule that sets ${quest.statusVariableId} to ${expectedStatus}.`);
    }
  };
  for (const [questIndex, quest] of program.quests.entries()) {
    const prefix = `gameplayProgram.quests[${questIndex}]`;
    if (!stableId(quest.id)) errors.push(`${prefix}.id must be a stable non-empty id.`);
    else if (questIds.has(quest.id)) errors.push(`${prefix}.id duplicates ${quest.id}.`);
    questIds.add(quest.id);
    if (!quest.title) errors.push(`${prefix}.title must be player-facing text.`);
    if (!quest.description) warnings.push(`${prefix} has no player-facing description.`);
    validateTextTokens(quest.title, `${prefix}.title`);
    validateTextTokens(quest.description, `${prefix}.description`);
    if (quest.giverId && !narrativeCharacterIds.has(quest.giverId)) errors.push(`${prefix}.giverId references missing Narrative Contract character ${quest.giverId}.`);
    const statusVariable = variableDefinitions.get(quest.statusVariableId);
    if (!statusVariable) errors.push(`${prefix}.statusVariableId references missing variable ${quest.statusVariableId || "(empty)"}.`);
    else if (statusVariable.type !== "number" || !Number.isSafeInteger(statusVariable.initial) || !QUEST_STATUS_VALUES.has(statusVariable.initial)) errors.push(`${prefix}.statusVariableId must reference a number variable with a safe-integer initial value from 0 through 3.`);
    if (statusVariable && (statusVariable.min === undefined || statusVariable.min > QUEST_STATUS.inactive || statusVariable.max === undefined || statusVariable.max < QUEST_STATUS.failed)) warnings.push(`${prefix}.statusVariableId should use min 0 and max 3 so every canonical quest state remains representable.`);
    validateConditions(quest.visibleWhen, `${prefix}.visibleWhen`);
    validateQuestRule(quest, "activationRuleId", QUEST_STATUS.active, prefix);
    validateQuestRule(quest, "completionRuleId", QUEST_STATUS.completed, prefix);
    validateQuestRule(quest, "failureRuleId", QUEST_STATUS.failed, prefix);
    validateQuestRule(quest, "rewardRuleId", undefined, prefix);
    if (statusVariable?.initial === QUEST_STATUS.inactive && !quest.activationRuleId) warnings.push(`${prefix} starts inactive but has no activationRuleId.`);
    if (!quest.completionRuleId) warnings.push(`${prefix} has no completionRuleId.`);
    for (const testId of quest.acceptanceTestIds) if (!acceptanceIds.has(testId)) errors.push(`${prefix}.acceptanceTestIds references missing acceptance or replay test ${testId}.`);
    if (quest.objectives.length > QUEST_MAXIMUMS.objectivesPerQuest) errors.push(`${prefix}.objectives must contain at most ${QUEST_MAXIMUMS.objectivesPerQuest} objectives.`);
    if (!quest.objectives.length) warnings.push(`${prefix} has no authored objectives.`);
    for (const [objectiveIndex, objective] of quest.objectives.entries()) {
      const objectivePrefix = `${prefix}.objectives[${objectiveIndex}]`;
      if (!stableId(objective.id)) errors.push(`${objectivePrefix}.id must be a stable non-empty id.`);
      else if (objectiveIds.has(objective.id)) errors.push(`${objectivePrefix}.id duplicates ${objective.id}; objective IDs must be globally unique.`);
      objectiveIds.add(objective.id);
      if (!objective.label) errors.push(`${objectivePrefix}.label must be player-facing text.`);
      validateTextTokens(objective.label, `${objectivePrefix}.label`);
      validateConditions(objective.visibleWhen, `${objectivePrefix}.visibleWhen`);
      validateConditions(objective.completeWhen, `${objectivePrefix}.completeWhen`);
      if (!objective.completeWhen.length) warnings.push(`${objectivePrefix} has no completion conditions.`);
      if (objective.progressVariableId) {
        const progressVariable = variableDefinitions.get(objective.progressVariableId);
        if (!progressVariable) errors.push(`${objectivePrefix}.progressVariableId references missing variable ${objective.progressVariableId}.`);
        else if (progressVariable.type !== "number") errors.push(`${objectivePrefix}.progressVariableId must reference a number variable.`);
        if (!Number.isSafeInteger(objective.target) || objective.target <= 0) errors.push(`${objectivePrefix}.target must be a positive safe integer when progressVariableId is present.`);
      } else if (objective.target !== undefined) warnings.push(`${objectivePrefix}.target is ignored without progressVariableId.`);
      for (const testId of objective.acceptanceTestIds) if (!acceptanceIds.has(testId)) errors.push(`${objectivePrefix}.acceptanceTestIds references missing acceptance or replay test ${testId}.`);
    }
  }

  for (const [ruleIndex, rule] of program.rules.entries()) {
    for (const [effectIndex, effect] of rule.effects.entries()) {
      const quest = program.quests.find((candidate) => candidate.statusVariableId === effect.variableId);
      if (quest && effect.type === "set-variable" && !QUEST_STATUS_VALUES.has(effect.value)) errors.push(`gameplayProgram.rules[${ruleIndex}].effects[${effectIndex}] sets quest ${quest.id} to invalid status ${String(effect.value)}; use 0, 1, 2, or 3.`);
    }
  }
  ...
  return {
    ...
    metrics: {
      ...
      dialoguePageCount: program.choicePages.filter((page) => page.lineId || page.speakerId || page.portraitAssetId).length,
      questCount: program.quests.length,
      questObjectiveCount: program.quests.reduce((total, quest) => total + quest.objectives.length, 0),
...
export const LOOPLAB_GAMEPLAY_RULE_POLICY = Object.freeze({
  ...
  systems: ["choice-pages", "dialogue", "speaker-and-portrait-metadata", "quest-bindings", "clocks", "turn-and-day-advancement", "integer-formulas", "variable-bound-hud"],
  dialogue: Object.freeze({ revealModes: [...DIALOGUE_REVEAL_MODES], portraitSides: [...DIALOGUE_PORTRAIT_SIDES], typewriterCharactersPerSecond: Object.freeze({ minimum: 5, maximum: 120 }), deterministic: false, reducedMotion: "instant", autoAdvance: false }),
  quests: Object.freeze({ status: QUEST_STATUS, maximums: QUEST_MAXIMUMS, runtimeOwner: "gameplayProgram.variables-and-rules", separateMutableStore: false }),
