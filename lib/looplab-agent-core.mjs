...
import { LOOPLAB_GAME_STUDIO_PLAN_SCHEMA, LOOPLAB_NARRATIVE_ROUTING_POLICY, LOOPLAB_RUNTIME_KNOWLEDGE, LOOPLAB_RUNTIME_SELECTION_POLICY, LOOPLAB_SPECIALIST_AGENTS } from "./looplab-capability-router.mjs";
import { getCapabilityPackRegistry } from "./looplab-capability-packs.mjs";
...
import {
  ...
  LOOPLAB_REPLAY_GHOST_SCHEMA,
  LOOPLAB_RUN_CODE_SCHEMA,
  LOOPLAB_RUN_VARIATION_LIMITS,
  LOOPLAB_RUN_VARIATION_POLICY,
  LOOPLAB_RUN_VARIATION_PROGRAM_SCHEMA,
  LOOPLAB_RUN_VARIATION_REPORT_SCHEMA,
  normalizeRunVariationProgram,
  suggestRunVariationProgram,
  validUtcRunDay,
} from "./looplab-run-variation.mjs";
import { resolveRunVariation, runVariationProgramDigest, runVariationSelectionProgramDigest } from "./looplab-run-variation-runtime.mjs";
import {
  LOOPLAB_INTERACTABLE_INSTANCE_SCHEMA,
  LOOPLAB_INTERACTABLE_KINDS,
  LOOPLAB_INTERACTABLE_LIMITS,
  LOOPLAB_INTERACTABLE_POLICY,
  LOOPLAB_INTERACTABLE_PREVIEW_SCHEMA,
  LOOPLAB_INTERACTABLE_REPORT_SCHEMA,
  ...
  removeInteractableInstance,
} from "./looplab-interactables.mjs";
...
export function buildStandaloneRuntimePrelude() {
  return [
    ...
    `const LOOPLAB_RUN_VARIATION_PROGRAM_SCHEMA=${JSON.stringify(LOOPLAB_RUN_VARIATION_PROGRAM_SCHEMA)};`,
    `const LOOPLAB_RUN_VARIATION_STATE_SCHEMA=${JSON.stringify("looplab-run-variation-state/v1")};`,
    `const LOOPLAB_RUN_VARIATION_SELECTION_ALGORITHM=${JSON.stringify(LOOPLAB_RUN_VARIATION_POLICY.selectionAlgorithm)};`,
    `const normalizeRunVariationProgram=${normalizeRunVariationProgram.toString()};`,
    `const runVariationProgramDigest=${runVariationProgramDigest.toString()};`,
    `const runVariationSelectionProgramDigest=${runVariationSelectionProgramDigest.toString()};`,
    `const validUtcRunDay=${validUtcRunDay.toString()};`,
    `const resolveRunVariation=${resolveRunVariation.toString()};`,
    ...
    "const createRuntimeModel=(project)=>createRuntimeModelFactory(project,{compileTileRuntimeProgram,compileWorldStreamRuntime,normalizeRunVariationProgram,resolveRunVariation,runVariationProgramDigest});",
    ...
    `const createRunCodeRuntime=${createRunCodeRuntime.toString()};`,
...
const OBJECT_PRESETS = {
  ...
  spring: { name: "Spring", width: 64, height: 18, color: "#3c3f46", solid: false, anchorMode: "ground", collisionOwner: "authored-map", collider: { enabled: true, offsetX: 0, offsetY: 0, width: 64, height: 18, trigger: true, oneWay: false, zMin: 0, zMax: 1 } },
  ladder: { name: "Ladder", width: 32, height: 192, color: "#5a5d64", solid: false, anchorMode: "ground", collisionOwner: "authored-map", collider: { enabled: true, offsetX: 0, offsetY: 0, width: 32, height: 192, trigger: true, oneWay: false, zMin: 0, zMax: 1 } },
  conveyor: { name: "Conveyor", width: 160, height: 24, color: "#34373d", solid: true, anchorMode: "ground", collisionOwner: "authored-map", collider: { enabled: true, offsetX: 0, offsetY: 0, width: 160, height: 24, trigger: false, oneWay: true, zMin: 0, zMax: 1 } },
  "crumble-platform": { name: "Crumble platform", width: 128, height: 24, color: "#55545a", solid: true, anchorMode: "ground", collisionOwner: "authored-map", collider: { enabled: true, offsetX: 0, offsetY: 0, width: 128, height: 24, trigger: false, oneWay: true, zMin: 0, zMax: 1 } },
  key: { name: "Key", width: 28, height: 28, color: "#d4ad45", solid: false, anchorMode: "ground", collisionOwner: "authored-map", collider: { enabled: true, offsetX: 0, offsetY: 0, width: 28, height: 28, trigger: true, oneWay: false, zMin: 0, zMax: 1 } },
  door: { name: "Door", width: 48, height: 112, color: "#36383d", solid: true, anchorMode: "ground", collisionOwner: "authored-map", collider: { enabled: true, offsetX: 0, offsetY: 0, width: 48, height: 112, trigger: false, oneWay: false, zMin: 0, zMax: 1 } },
  "pressure-plate": { name: "Pressure plate", width: 56, height: 12, color: "#696b70", solid: false, anchorMode: "ground", collisionOwner: "authored-map", collider: { enabled: true, offsetX: 0, offsetY: 0, width: 56, height: 12, trigger: true, oneWay: false, zMin: 0, zMax: 1 } },
  "one-way-platform": { name: "One-way platform", width: 160, height: 20, color: "#3f4248", solid: true, anchorMode: "ground", collisionOwner: "authored-map", collider: { enabled: true, offsetX: 0, offsetY: 0, width: 160, height: 20, trigger: false, oneWay: true, zMin: 0, zMax: 1 } },
};
...
function iterationReceipt(project, metadata = {}, snapshotId = null) {
  ...
  return {
    ...
    gameplayDialoguePageCount: (project.gameplayProgram?.choicePages ?? []).filter((page) => page?.lineId || page?.speakerId || page?.portraitAssetId).length,
    gameplayQuestCount: project.gameplayProgram?.quests?.length ?? 0,
    gameplayQuestObjectiveCount: (project.gameplayProgram?.quests ?? []).reduce((total, quest) => total + (quest?.objectives?.length ?? 0), 0),
...
export function validateProject(project) {
  ...
  if (project.runVariationProgram !== undefined) {
    const runVariationInspection = inspectRunVariationProgram(project, project.runVariationProgram, { strict: project.doctorProfile === "production", sourceDigest: doctorSourceDigest(project) });
    errors.push(...runVariationInspection.errors.map((message) => `runVariationProgram: ${message}`));
    warnings.push(...runVariationInspection.warnings.map((message) => `runVariationProgram: ${message}`));
  }
  ...
  const interactableInspection = inspectInteractables(project);
  errors.push(...interactableInspection.errors.map((message) => `interactables: ${message}`));
  warnings.push(...interactableInspection.warnings.map((message) => `interactables: ${message}`));
...
export function summarizeProject(project) {
  ...
  return {
    ...
    gameplayProgram: project.gameplayProgram ? {
      ...
      dialoguePageCount: (project.gameplayProgram.choicePages ?? []).filter((page) => page?.lineId || page?.speakerId || page?.portraitAssetId).length,
      questCount: project.gameplayProgram.quests?.length ?? 0,
      questObjectiveCount: (project.gameplayProgram.quests ?? []).reduce(
    ...
    runVariationProgram: project.runVariationProgram ? {
      enabled: project.runVariationProgram.enabled === true,
      poolCount: project.runVariationProgram.pools?.length ?? 0,
      variantCount: (project.runVariationProgram.pools ?? []).reduce((total, pool) => total + (pool?.variants?.length ?? 0), 0),
      ghostCount: project.runVariationProgram.ghosts?.length ?? 0,
      dailyChallengeEnabled: project.runVariationProgram.dailyChallenge?.enabled === true,
      acceptanceTestCount: project.runVariationProgram.acceptanceTestIds?.length ?? 0,
    } : null,
...
export function applyAgentCommand(inputProject, command, options = {}) {
  ...
  switch (command.op) {
    ...
    case "list_interactable_templates":
      result = listInteractableTemplates({ query: command.query, controlMode: command.controlMode, limit: command.limit, offset: command.offset });
      break;
    case "get_interactable_template":
      result = getInteractableTemplate(command.templateId);
      break;
    case "get_interactable_report":
      result = inspectInteractables(syncActiveMap(project), { requireEvidence: command.requireEvidence === true });
      break;
    case "preview_interactable_template": {
      const normalized = syncActiveMap(project);
      const mapId = command.mapId ?? normalized.activeMapId;
      const map = normalized.maps.find((candidate) => candidate.id === mapId);
      if (!map) throw new Error("Interactable-template map was not found.");
      ...
      const duplicateObjectIds = preview.objects.map((object) => object.id).filter((id) => objectIds.has(id));
      const duplicateInstance = (map.objects ?? []).some((object) => object?.interactable?.instanceId === preview.instanceId);
      result = {
        ...preview,
        applicable: duplicateObjectIds.length === 0 && !duplicateInstance,
        conflicts: { duplicateObjectIds, duplicateInstance },
      };
      break;
    }
    case "apply_interactable_template": {
      if (typeof command.expectedSourceDigest !== "string" || !command.expectedSourceDigest) throw new Error("apply_interactable_template requires expectedSourceDigest from the exact preview.");
      if (typeof command.templateDigest !== "string" || !command.templateDigest) throw new Error("apply_interactable_template requires templateDigest from the exact preview.");
      if (typeof command.previewDigest !== "string" || !command.previewDigest) throw new Error("apply_interactable_template requires previewDigest from the exact preview.");
      const normalized = syncActiveMap(project);
      const mapId = command.mapId ?? normalized.activeMapId;
      const mapIndex = normalized.maps.findIndex((candidate) => candidate.id === mapId);
      if (mapIndex < 0) throw new Error("Interactable-template map was not found.");
      const sourceDigest = doctorSourceDigest(normalized);
      const preview = materializeInteractableTemplate({
        templateId: command.templateId,
        instanceId: command.instanceId,
      ...
      assertInteractablePreview(preview, { sourceDigest: command.expectedSourceDigest, templateDigest: command.templateDigest, previewDigest: command.previewDigest });
      const currentObjects = normalized.maps[mapIndex].objects ?? [];
      if (currentObjects.some((object) => object?.interactable?.instanceId === preview.instanceId)) throw new Error(`Interactable instance already exists: ${preview.instanceId}.`);
      const existingIds = new Set(currentObjects.map((object) => object.id));
      const duplicateIds = preview.objects.map((object) => object.id).filter((id) => existingIds.has(id));
      if (duplicateIds.length) throw new Error(`Interactable object ID already exists: ${duplicateIds.join(", ")}.`);
      const maps = clone(normalized.maps);
      maps[mapIndex] = { ...maps[mapIndex], objects: [...currentObjects, ...clone(preview.objects)] };
      ...
      result = {
        ...
        objectIds: preview.objects.map((object) => object.id),
        featureContractTemplate: preview.featureContractTemplate,
        fixtureTemplates: preview.fixtureTemplates,
        nextActions: ["Bind deliberate art without changing authored collision.", "Install and calibrate the supplied feature-contract and acceptance/replay fixture templates.", "Run Project Doctor, deterministic replay, browser playtest, and one-file export verification."],
      };
      break;
    }
    case "remove_interactable_instance": {
      const normalized = syncActiveMap(project);
      const removal = removeInteractableInstance(normalized, { mapId: command.mapId ?? normalized.activeMapId, instanceId: command.instanceId });
      project = syncActiveMap(removal.project);
      changed = true;
      result = { mapId: command.mapId ?? normalized.activeMapId, instanceId: command.instanceId, removed: removal.removed };
      break;
    }
    ...
    case "get_run_variation_program":
    case "get_run_variation_report": {
      result = inspectRunVariationProgram(project, project.runVariationProgram, { strict: command.profile === "production", sourceDigest: doctorSourceDigest(project) });
      break;
    }
    case "suggest_run_variation_program": {
      result = suggestRunVariationProgram(project, command);
      break;
    }
    case "preview_run_variation": {
      const report = inspectRunVariationProgram(project, project.runVariationProgram, { strict: command.profile === "production", sourceDigest: doctorSourceDigest(project) });
      if (!report.present || !report.valid || !report.program?.enabled) throw new Error("preview_run_variation requires one valid enabled runVariationProgram.");
      const state = command.mode === "daily"
        ? resolveRunVariation(report.program, { mode: "daily", utcDay: command.utcDay })
        : resolveRunVariation(report.program, { mode: "standard", seed: command.seed ?? report.program.defaultSeed });
      result = { state, sourceDigest: report.sourceDigest, programDigest: report.programDigest, proofBoundary: LOOPLAB_RUN_VARIATION_POLICY.proofBoundary };
      break;
    }
    case "set_run_variation_program": {
      if (!command.program || typeof command.program !== "object" || Array.isArray(command.program)) throw new Error("set_run_variation_program requires a program object.");
      const program = normalizeRunVariationProgram(command.program);
      const candidate = { ...project, runVariationProgram: program };
      const inspection = inspectRunVariationProgram(candidate, program, { strict: command.profile === "production", sourceDigest: doctorSourceDigest(candidate) });
      if (inspection.errors.length) throw new Error(`Run-variation program is invalid: ${inspection.errors.join(" ")}`);
      project.runVariationProgram = program;
      changed = true;
      result = { program: clone(program), report: clone(inspection) };
      break;
    }
    case "remove_run_variation_program": {
      const removed = project.runVariationProgram !== undefined;
      delete project.runVariationProgram;
      changed = removed;
      result = { removed };
      break;
    }
    case "preview_replay_ghost": {
      const currentSourceDigest = doctorSourceDigest(project);
      if (command.expectedSourceDigest && command.expectedSourceDigest !== currentSourceDigest) throw new Error(`[stale-source] Replay-ghost preview expected ${command.expectedSourceDigest}, but the selected project is now ${currentSourceDigest}.`);
      const preview = buildReplayGhost(project, command.replayCaseId, command);
      const previewDigest = canonicalSha256({ schemaVersion: "looplab-replay-ghost-preview-receipt/v1", sourceDigest: currentSourceDigest, ghost: preview.ghost });
      result = { ...preview, sourceDigest: currentSourceDigest, previewDigest, applyCommand: { op: "apply_replay_ghost", replayCaseId: command.replayCaseId, id: preview.ghost.id, label: preview.ghost.label, sampleEveryTicks: preview.ghost.sampleEveryTicks, color: preview.ghost.color, opacity: preview.ghost.opacity, expectedSourceDigest: currentSourceDigest, expectedPreviewDigest: previewDigest } };
      break;
    }
    case "apply_replay_ghost": {
      const currentSourceDigest = doctorSourceDigest(project);
      if (!command.expectedSourceDigest || command.expectedSourceDigest !== currentSourceDigest) throw new Error(`[stale-source] Replay-ghost apply requires the exact current source digest ${currentSourceDigest}.`);
      const preview = buildReplayGhost(project, command.replayCaseId, command);
      const previewDigest = canonicalSha256({ schemaVersion: "looplab-replay-ghost-preview-receipt/v1", sourceDigest: currentSourceDigest, ghost: preview.ghost });
      if (!command.expectedPreviewDigest || command.expectedPreviewDigest !== previewDigest) throw new Error("[stale-preview] Replay-ghost apply no longer matches the reviewed trajectory.");
      const base = normalizeRunVariationProgram(project.runVariationProgram ?? suggestRunVariationProgram(project).program);
      const ghosts = base.ghosts.filter((ghost) => ghost.id !== preview.ghost.id);
      const program = normalizeRunVariationProgram({ ...base, ghosts: [...ghosts, preview.ghost] });
      const candidate = { ...project, runVariationProgram: program };
      const inspection = inspectRunVariationProgram(candidate, program, { strict: command.profile === "production", sourceDigest: doctorSourceDigest(candidate) });
      if (inspection.errors.length) throw new Error(`Replay ghost cannot be applied: ${inspection.errors.join(" ")}`);
      project.runVariationProgram = program;
      changed = true;
      result = { ghost: clone(preview.ghost), programDigest: runVariationProgramDigest(program), report: clone(inspection), sourceDigest: currentSourceDigest, previewDigest };
      break;
    }
...
export function getAgentManifest() {
  const capabilityPackRegistry = getCapabilityPackRegistry();
  const capabilityPackCapabilities = capabilityPackRegistry.packs.flatMap((pack) => pack.capabilities.map(({ id, label, owns }) => ({ id, label, owns })));
  return {
    ...
    transport: {
      ...
      agentGuideNavigationSelector: "#looplab-agent-guide-navigation",
    ...
    headlessResponses: {
      ...
      agentGuideNavigationWorkflow: "Use get_agent_guide_index on the browser-session bridge or read looplab://agent-guide-index through MCP to retrieve only the relevant invariant, lifecycle step, failure recovery, or canonical section. Follow the returned anchor into the complete guide before acting. The generated index is source-bound orientation only and never grants authority or becomes evidence.",
    ...
    narrativeRules: {
      ...
      runtimeMethods: ["getNarrativeContract", "getNarrativeReport", "getQuestState"],
      runtimeCommands: ["get_narrative_contract", "get_narrative_report", "get_quest_state"],
    ...
    gameDirector: {
      ...
      gameplayProgram: {
        ...
        dialogueQuestWorkflow: "Bind choice-page line/speaker IDs to the Narrative Contract and portrait IDs to embedded project assets. Author quests as a derived journal over the same typed gameplay variables and deterministic rules; never create a second mutable quest store. Typewriter reveal is presentation-only, reduced motion is instant, full readable text is exposed atomically, and v1 never auto-advances dialogue.",
      },
      runVariationProgram: {
        programSchemaVersion: LOOPLAB_RUN_VARIATION_PROGRAM_SCHEMA,
        reportSchemaVersion: LOOPLAB_RUN_VARIATION_REPORT_SCHEMA,
        ghostSchemaVersion: LOOPLAB_REPLAY_GHOST_SCHEMA,
        runCodeSchemaVersion: LOOPLAB_RUN_CODE_SCHEMA,
        commands: ["get_run_variation_program", "get_run_variation_report", "suggest_run_variation_program", "preview_run_variation", "set_run_variation_program", "remove_run_variation_program", "preview_replay_ghost", "apply_replay_ghost"],
        previewCommands: ["preview_start_run", "preview_start_daily_challenge", "get_preview_ghost_states"],
        exportedRuntimeCommands: ["preview_run_variation", "start_run", "start_daily_challenge", "get_ghost_states", "get_run_code_status", "export_run_code", "inspect_run_code", "import_run_code"],
        limits: LOOPLAB_RUN_VARIATION_LIMITS,
        policy: LOOPLAB_RUN_VARIATION_POLICY,
        parity: "Mouse, DOM bridge, CLI, MCP, Codex, and Claude edit the same canonical runVariationProgram. Preview and every one-file Canvas/Phaser/Pixi/melon export resolve the same source-bound seed identity before tick zero.",
      },
      interactableTemplates: {
        registrySchemaVersion: LOOPLAB_INTERACTABLE_TEMPLATE_REGISTRY.schemaVersion,
        instanceSchemaVersion: LOOPLAB_INTERACTABLE_INSTANCE_SCHEMA,
        previewSchemaVersion: LOOPLAB_INTERACTABLE_PREVIEW_SCHEMA,
        reportSchemaVersion: LOOPLAB_INTERACTABLE_REPORT_SCHEMA,
        registryRevision: LOOPLAB_INTERACTABLE_TEMPLATE_REGISTRY.revision,
        registryDigest: LOOPLAB_INTERACTABLE_TEMPLATE_REGISTRY.digest,
        templateCount: LOOPLAB_INTERACTABLE_TEMPLATE_REGISTRY.templates.length,
        templates: LOOPLAB_INTERACTABLE_TEMPLATE_REGISTRY.templates.map((entry) => ({ id: entry.id, revision: entry.revision, digest: entry.digest, label: entry.label, controlModes: entry.controlModes, roles: entry.objectPlan.map((plan) => plan.role), events: entry.events })),
        objectKinds: LOOPLAB_INTERACTABLE_KINDS,
        commands: ["list_interactable_templates", "get_interactable_template", "get_interactable_report", "preview_interactable_template", "apply_interactable_template", "remove_interactable_instance"],
        limits: LOOPLAB_INTERACTABLE_LIMITS,
        policy: LOOPLAB_INTERACTABLE_POLICY,
        workflow: "List or inspect a native template, create a source-bound preview, show the exact objects/colliders/overrides, then apply only the matching source/template/preview digests. Calibrate the supplied acceptance and replay fixture templates before claiming the instance works.",
      },
      narrativeContract: {
        policy: LOOPLAB_NARRATIVE_CONTRACT_POLICY,
    ...
    capabilityPacks: {
      ...
      revision: capabilityPackRegistry.revision,
      ...
      resource: "looplab://capability-packs",
      publicArtifact: "/capability-packs.json",
      commands: ["list_capability_packs", "get_capability_pack", "query_capability_knowledge", "inspect_capability_pack_refresh"],
      packs: capabilityPackRegistry.packs.map((pack) => ({ id: pack.id, revision: pack.revision, label: pack.label, categories: pack.categories, capabilityIds: pack.capabilities.map((entry) => entry.id), contentDigest: pack.contentDigest, digest: pack.digest, licenseExpressions: [...new Set(pack.sources.map((source) => source.licenseExpression))], calibrationCaseIds: pack.calibrationCaseIds })),
      calibration: { schemaVersion: capabilityPackRegistry.calibration.schemaVersion, valid: capabilityPackRegistry.calibration.valid, caseCount: capabilityPackRegistry.calibration.caseCount, passedCount: capabilityPackRegistry.calibration.passedCount, digest: capabilityPackRegistry.calibration.digest },
      policy: capabilityPackRegistry.policy,
      refreshPolicy: "Refresh inspection is read-only: lower revisions are rollback, same-revision different bytes are equivocation, and a higher revision remains uninstalled until schema, digest, provenance, license metadata, policy, and calibration review pass.",
      exportBoundary: "Capability packs are builder orientation data. They do not enter project JSON, provider-authored game state, runtime payloads, or one-file game exports.",
    },
    installedSkills: {
      excluded: ["hbg-loop"],
      ...
      capabilities: capabilityPackCapabilities,
    ...
    deterministicReplay: {
      ...
      digestAlgorithms: { legacyVersions: [1, 2, 3, 4, 5], legacy: "FNV-1a-32", sha256Versions: [LOOPLAB_REPLAY_SHA256_HASH_VERSION, LOOPLAB_REPLAY_COMBAT_HASH_VERSION, LOOPLAB_REPLAY_ACTOR_HASH_VERSION, LOOPLAB_REPLAY_COLLISION_HASH_VERSION, LOOPLAB_REPLAY_MOTION_CARRY_HASH_VERSION, LOOPLAB_REPLAY_ELEVATION_HASH_VERSION, LOOPLAB_REPLAY_WORLD_STREAM_HASH_VERSION, LOOPLAB_REPLAY_INTERACTABLE_HASH_VERSION, LOOPLAB_REPLAY_RUN_VARIATION_HASH_VERSION], currentVersion: LOOPLAB_REPLAY_RUN_VARIATION_HASH_VERSION, current: "SHA-256" },
      digestFormats: { legacy: "replay-<8 lowercase hex>", current: "replay-sha256-<64 lowercase hex>" },
      hashVersionPolicy: "Fixtures without hashVersion retain the original v1 FNV-1a projection; versions 1 through 13 remain byte-compatible. Every new or explicitly rerecorded fixture writes v14 SHA-256 with exact run-variation identity in addition to v13 support-object identity, ladder controller state, interactable target, and conveyor support, v12 world-stream identity, v11 elevation, v10 motion-body, and v9 authored segment-ground state. Visual loading, ghost presentation, first-draw timing, camera, and asset readiness remain excluded. Runtime-state additions must add a new projection instead of silently invalidating accepted evidence.",
    ...
    exportedRuntime: {
      ...
      methods: ["getSourceDigest", "getRuntimeAdapterInfo", "getInputActionLiveness", "getCompletionReport", "getNarrativeContract", "getNarrativeReport", "getSaveReport", "getSaveStatus", "exportSaveCode", "inspectSaveCode", "importSaveCode", "persistHostedSave", "clearHostedSave", "getRunVariationProgram", "getRunVariationReport", "getRunVariationState", "previewRunVariation", "startRun", "startDailyChallenge", "getGhostStates", "getRunCodeStatus", "exportRunCode", "inspectRunCode", "importRunCode", "getState", "getObjects", "getTraversalPaths", "getCollisionGeometry", "getElevationTransitions", "getTileProgram", "getTileRuntime", "getWorldStreamState", "markWorldStreamDraw", "getMotionBodyStates", "getActorStates", "getCombatState", "getNavigation", "getGameplayState", "getChoiceState", "getQuestState", "getHudState", "chooseChoice", "getRuntimeJoinPlan", "beginRuntimeJoinProbe", "commitRuntimeJoinProbe", "finishRuntimeJoinProbe", "getCollisionBox", "getPerformance", "getAcceptanceTests", "runAcceptanceTest", "runAcceptanceSuite", "getReplayCases", "runReplayCase", "runReplaySuite", "setInput", "step", "reset", "loadMap", "pause", "resume"],
      commands: ["get_source_digest", "get_runtime_adapter", "get_input_action_liveness", "get_completion_report", "get_narrative_contract", "get_narrative_report", "get_save_report", "get_save_status", "export_save_code", "inspect_save_code", "import_save_code", "persist_hosted_save", "clear_hosted_save", "get_run_variation_program", "get_run_variation_report", "get_run_variation_state", "preview_run_variation", "start_run", "start_daily_challenge", "get_ghost_states", "get_run_code_status", "export_run_code", "inspect_run_code", "import_run_code", "get_state", "get_objects", "get_traversal_paths", "get_collision_geometry", "get_elevation_transitions", "get_tile_program", "get_tile_runtime", "get_world_stream_state", "mark_world_stream_draw", "get_motion_body_states", "get_actor_states", "get_combat_state", "get_navigation", "get_gameplay_state", "get_choice_state", "get_quest_state", "get_hud_state", "choose_choice", "get_runtime_join_plan", "begin_runtime_join_probe", "commit_runtime_join_probe", "finish_runtime_join_probe", "get_collision_box", "get_acceptance_tests", "run_acceptance_test", "run_acceptance_suite", "get_replay_cases", "run_replay_case", "run_replay_suite", "set_input", "step", "reset", "load_map", "pause", "resume"],
    },
    ...
    agentGuideNavigation: {
      schemaVersion: LOOPLAB_AGENT_GUIDE_INDEX.schemaVersion,
      sourceDigest: LOOPLAB_AGENT_GUIDE_INDEX.source.digest,
      indexDigest: LOOPLAB_AGENT_GUIDE_INDEX.indexDigest,
      headingCount: LOOPLAB_AGENT_GUIDE_INDEX.source.headingCount,
      counts: {
        sections: LOOPLAB_AGENT_GUIDE_INDEX.sections.length,
        invariants: LOOPLAB_AGENT_GUIDE_INDEX.invariants.length,
        lifecycle: LOOPLAB_AGENT_GUIDE_INDEX.lifecycle.length,
        recoveries: LOOPLAB_AGENT_GUIDE_INDEX.recoveries.length,
      },
      publicUrl: LOOPLAB_AGENT_GUIDE_INDEX.indexResource.publicUrl,
      guideUrl: LOOPLAB_AGENT_GUIDE_INDEX.source.publicUrl,
      resources: [LOOPLAB_AGENT_GUIDE_INDEX.indexResource.resourceUri, LOOPLAB_AGENT_GUIDE_INDEX.source.resourceUri],
      browserCommand: { op: "get_agent_guide_index", query: "stale source", category: "recovery", limit: 8 },
      cli: "npm run agent -- guide [query] [--category=all|section|invariant|lifecycle|recovery] [--limit=24]",
      policy: clone(LOOPLAB_AGENT_GUIDE_INDEX.policy),
    },
    mcpServer: {
      ...
      resources: ["looplab://manifest", "looplab://agent-playbook", "looplab://capability-packs", "looplab://agent-guide", "looplab://agent-guide-index", "looplab://mcp-setup"],
...
export function getCompactAgentManifest() {
  ...
  return {
    ...
    agentGuideNavigation: manifest.agentGuideNavigation,
...
function buildStandaloneHtmlInternal(project, { verificationArtifact = false } = {}) {
  ...
  const runVariationReportConfig = serializeProjectMetadata(doctor.runVariationReport);
...
<head>
  ...
  <style>
    *{box-sizing:border-box}html,body{margin:0;min-height:100%;background:#171714;color:#fff;font-family:ui-monospace,SFMono-Regular,Consolas,monospace}body{display:grid;place-items:center;padding:20px;overscroll-behavior:none}.game-shell{width:min(100%,1100px)}.game-bar{display:flex;flex-wrap:wrap;justify-content:space-between;align-items:center;gap:10px;margin-bottom:10px;text-transform:uppercase;letter-spacing:.08em;font-size:12px}.game-bar strong{font-size:15px}.game-stats{display:flex;flex-wrap:wrap;align-items:center;justify-content:flex-end;gap:10px}.game-bar button,.touch-controls button{min-width:44px;min-height:44px;border:1px solid #fff;background:transparent;color:#fff;padding:8px 12px;cursor:pointer;font:inherit}.game-bar button:focus-visible,.touch-controls button:focus-visible,canvas:focus-visible{outline:3px solid #c8ff4d;outline-offset:2px}.map-name{color:#c8ff4d}.runtime-status{min-width:9ch;color:#b9b9b0}.frame{position:relative;overflow:hidden;border:2px solid #fff;background:#000;box-shadow:8px 8px 0 #5b5cf0}canvas{display:block;width:100%;height:auto;image-rendering:pixelated;touch-action:none}.transition{pointer-events:none;position:absolute;inset:0;background:#080808;opacity:0}.hint{opacity:.72;margin:14px 0 0;text-align:center;font-size:12px;line-height:1.6}.touch-controls{display:none;grid-template-columns:repeat(3,52px) 1fr repeat(2,68px);gap:7px;align-items:end;justify-content:center;margin-top:16px;touch-action:none}.touch-controls[data-active=true]{display:grid}.touch-controls[hidden]{display:none!important}.touch-controls button{padding:0;background:#272722;font-weight:800}.touch-controls [data-input=up]{grid-column:2}.touch-controls [data-input=left]{grid-column:1;grid-row:2}.touch-controls [data-input=down]{grid-column:2;grid-row:2}.touch-controls [data-input=right]{grid-column:3;grid-row:2}.touch-controls [data-input=jump]{grid-column:5;grid-row:1/3}.touch-controls [data-input=interact]{grid-column:6;grid-row:1/3}.runtime-bridge{margin-top:14px;border-top:1px solid #45453f;color:#b9b9b0;font-size:11px}.runtime-bridge summary{width:max-content;cursor:pointer;padding:10px 0;text-transform:uppercase;letter-spacing:.08em}.runtime-bridge form{display:grid;grid-template-columns:1fr auto;gap:7px;padding:0 0 14px}.runtime-bridge label{display:grid;gap:4px}.runtime-bridge label:last-of-type{grid-column:1/-1}.runtime-bridge textarea{width:100%;min-height:64px;resize:vertical;border:1px solid #65655d;background:#0e0e0c;color:#fff;padding:8px;font:11px/1.4 inherit}.runtime-bridge button{align-self:end;min-height:44px;border:1px solid #c8ff4d;background:#c8ff4d;color:#151515;padding:8px 14px;font:700 11px/1 inherit;text-transform:uppercase}.runtime-bridge span{font-size:9px;text-transform:uppercase}@media(max-width:700px){body{padding:10px 10px 18px}.game-bar{align-items:flex-end}.game-stats{justify-content:flex-start}.hint{font-size:11px}}@media(prefers-reduced-motion:reduce){.transition{display:none}}
    ...
    .choice-dialogue-grid{display:grid;grid-template-columns:minmax(90px,26%) minmax(0,1fr);gap:18px;align-items:end}.choice-dialogue-grid[data-portrait=none]{grid-template-columns:1fr}.choice-dialogue-grid[data-side=right] .choice-portrait{order:2}.choice-portrait{display:block;width:100%;max-height:260px;object-fit:contain;object-position:center bottom;image-rendering:pixelated}.choice-portrait[hidden]{display:none}.choice-speaker{margin:0 0 7px;color:#bbbcb8;font:800 11px/1.2 inherit;letter-spacing:.12em;text-transform:uppercase}.choice-speaker[hidden]{display:none}.choice-body-visual[hidden]{display:none}.choice-body-screen-reader{position:absolute!important;width:1px!important;height:1px!important;padding:0!important;margin:-1px!important;overflow:hidden!important;clip:rect(0,0,0,0)!important;white-space:nowrap!important;border:0!important}.quest-journal{pointer-events:none;position:absolute;z-index:4;inset:70px 12px auto auto;width:min(330px,42%);max-height:45%;overflow:hidden;border:1px solid rgba(255,255,255,.58);background:rgba(38,40,42,.94);padding:10px;color:#fff;box-shadow:5px 5px 0 rgba(0,0,0,.38)}.quest-journal[hidden]{display:none}.quest-journal h2{margin:0 0 7px;font:800 11px/1.2 inherit;letter-spacing:.12em;text-transform:uppercase}.quest-entry{border-top:1px solid rgba(255,255,255,.24);padding-top:7px}.quest-entry:first-of-type{border-top:0}.quest-entry strong{display:flex;justify-content:space-between;gap:8px;font-size:12px}.quest-entry strong small{color:#bbbcb8;text-transform:uppercase}.quest-entry p{margin:4px 0;color:#deded8;font:10px/1.35 inherit}.quest-entry ul{margin:5px 0 0;padding-left:17px;font:10px/1.35 inherit}.quest-entry li[data-complete=true]{color:#b9b9b0;text-decoration:line-through}@media(max-width:700px){.choice-dialogue-grid{grid-template-columns:minmax(64px,22%) minmax(0,1fr);gap:10px}.quest-journal{inset:58px 7px auto auto;width:min(300px,70%);max-height:30%}}
    .save-dialog{width:min(calc(100% - 24px),680px);max-height:calc(100% - 24px);overflow:auto;border:2px solid #303033;background:#d6d6d2;color:#202024;padding:0;box-shadow:10px 10px 0 rgba(0,0,0,.55);font:13px/1.45 inherit}.save-dialog::backdrop{background:rgba(8,8,10,.76)}.save-dialog form{display:grid;gap:12px;padding:20px}.save-dialog h2{margin:0;font:800 22px/1.1 inherit}.save-dialog p{margin:0;color:#4a4a50}.save-dialog textarea{width:100%;min-height:170px;resize:vertical;border:2px solid #3d3d42;background:#f2f2ef;color:#19191d;padding:10px;font:11px/1.4 inherit}.save-dialog input{width:100%;min-height:44px;border:2px solid #3d3d42;background:#f2f2ef;color:#19191d;padding:8px 10px;font:12px/1.2 inherit}.save-actions{display:flex;flex-wrap:wrap;gap:8px}.save-actions button{min-height:44px;border:2px solid #3d3d42;background:#3d3d42;color:#fff;padding:8px 12px;cursor:pointer;font:700 11px/1 inherit;text-transform:uppercase}.save-actions button.secondary{background:transparent;color:#242428}.save-actions button:focus-visible,.save-dialog textarea:focus-visible,.save-dialog input:focus-visible{outline:3px solid #5b5cf0;outline-offset:2px}.save-status{min-height:1.5em;font-weight:700}.save-boundary{padding:10px;border-left:4px solid #5b5cf0;background:#e7e7e3}@media(max-width:700px){.save-dialog form{padding:14px}.save-dialog textarea{min-height:130px}}
...
<body>
  ...
  <main class="game-shell">
    ...
    <div class="frame">
      ...
      <aside class="quest-journal" id="quest-journal" aria-label="Quest journal" aria-live="polite" aria-atomic="true" hidden><h2>Quest journal</h2><div id="quest-journal-entries"></div></aside>
      <section class="choice-layer" id="choice-layer" role="dialog" aria-modal="true" aria-labelledby="choice-title" aria-describedby="choice-body" hidden>
        <div class="choice-card"><div class="choice-dialogue-grid" id="choice-dialogue-grid" data-portrait="none" data-side="left"><img class="choice-portrait" id="choice-portrait" alt="" aria-hidden="true" hidden><div class="choice-content"><p class="choice-speaker" id="choice-speaker" hidden></p><h2 id="choice-title" tabindex="-1"></h2><p id="choice-body" aria-live="polite" aria-atomic="true"></p><p class="choice-body-visual" id="choice-body-visual" aria-hidden="true" hidden></p></div></div><div class="choice-options" id="choice-options"></div></div>
      </section>
    ...
    <dialog class="save-dialog" id="run-dialog" aria-labelledby="run-title" aria-describedby="run-boundary">
      <form method="dialog"><h2 id="run-title">Seeded run</h2><p class="save-boundary" id="run-boundary">A seed or UTC day selects the same authored variants on every compatible copy. LR1 codes start at tick zero; they do not authenticate scores.</p><label for="run-seed">Run seed</label><input id="run-seed" type="text" maxlength="128" autocomplete="off" spellcheck="false"><label for="run-day">Daily challenge day (UTC)</label><input id="run-day" type="date"><label for="run-code">Run code</label><textarea id="run-code" spellcheck="false" autocomplete="off" aria-describedby="run-status"></textarea><p class="save-status" id="run-status" role="status" aria-live="polite"></p><div class="save-actions"><button id="run-start" type="button">Start seed</button><button id="run-daily" type="button">Start daily</button><button id="run-export" type="button">Generate LR1</button><button id="run-import" type="button">Restore LR1</button><button class="secondary" id="run-close" type="submit">Close</button></div></form>
    </dialog>
...
${embeddedStorageScript}${embeddedEngineScript}  <script>
    ...
    const runVariationReport=${runVariationReportConfig};
    ${runtimePrelude}
    ...
    const runCodes=createRunCodeRuntime(engine,{sourceDigest:projectSourceDigest,programDigest:runVariationReport.program?.enabled===true?runVariationReport.programDigest||'':''});
    ...
    const runDay=document.getElementById('run-day');
    ...
    const questJournalEntries=document.getElementById('quest-journal-entries');
    const choiceLayer=document.getElementById('choice-layer');
    const choiceDialogueGrid=document.getElementById('choice-dialogue-grid');
    const choicePortrait=document.getElementById('choice-portrait');
    ...
    const choiceBodyVisual=document.getElementById('choice-body-visual');
    ...
    let choiceRevealTimer=0;
    let choicePreviouslyFocused=null;
    ...
    function syncQuestUi(){
      const state=engine.getQuestState();
      questJournalEntries.textContent='';
      (state.quests||[]).forEach(function(quest){const entry=document.createElement('section');entry.className='quest-entry';entry.dataset.questId=quest.id;const title=document.createElement('strong');const label=document.createElement('span');label.textContent=quest.title;const status=document.createElement('small');status.textContent=quest.status;title.append(label,status);entry.append(title);if(quest.description){const description=document.createElement('p');description.textContent=quest.description;entry.append(description)}if(quest.objectives.length){const list=document.createElement('ul');quest.objectives.forEach(function(objective){const item=document.createElement('li');item.dataset.objectiveId=objective.id;item.dataset.complete=String(objective.completed);item.textContent=objective.label+(objective.progress===null?'':' '+String(objective.progress)+' / '+String(objective.target));list.append(item)});entry.append(list)}questJournalEntries.append(entry)});
      questJournal.hidden=(state.quests||[]).length===0;
    }
    function dialogueReducedMotion(){const preference=gameShell.getState().preferences.reducedMotion;return preference==='reduce'||(preference!=='full'&&typeof matchMedia==='function'&&matchMedia('(prefers-reduced-motion: reduce)').matches)}
    function cancelChoiceReveal(){if(choiceRevealTimer&&typeof clearInterval==='function')clearInterval(choiceRevealTimer);choiceRevealTimer=0}
    function revealChoiceBody(choice){cancelChoiceReveal();choiceBody.textContent=choice.body;choiceBodyVisual.textContent='';const typewriter=choice.revealMode==='typewriter'&&!dialogueReducedMotion()&&typeof setInterval==='function'&&typeof clearInterval==='function';if(typewriter)choiceBody.setAttribute('class','choice-body-screen-reader');else choiceBody.removeAttribute('class');choiceBodyVisual.hidden=!typewriter;if(!typewriter)return;let visible=0;const speed=Math.max(5,Math.min(120,Number(choice.charactersPerSecond||30)));choiceRevealTimer=setInterval(function(){visible=Math.min(choice.body.length,visible+Math.max(1,Math.ceil(speed/20)));choiceBodyVisual.textContent=choice.body.slice(0,visible);if(visible>=choice.body.length)cancelChoiceReveal()},50)}
    function syncSystemsUi(){
      ...
      const wasOpen=!choiceLayer.hidden;
      choiceUiSignature=signature;
      if(!choice){cancelChoiceReveal();choiceLayer.hidden=true;choiceLayer.inert=true;choiceLayer.removeAttribute('data-line-id');choiceLayer.removeAttribute('data-speaker-id');choiceSpeaker.hidden=true;choiceSpeaker.textContent='';choicePortrait.hidden=true;choicePortrait.removeAttribute('src');choiceTitle.textContent='';choiceBody.textContent='';choiceBody.removeAttribute('class');choiceBodyVisual.hidden=true;choiceBodyVisual.textContent='';choiceOptions.textContent='';if(wasOpen){const target=choicePreviouslyFocused&&typeof document.contains==='function'&&document.contains(choicePreviouslyFocused)?choicePreviouslyFocused:displayCanvas;choicePreviouslyFocused=null;queueMicrotask(function(){target?.focus?.()})}return}
      if(!wasOpen)choicePreviouslyFocused=document.activeElement;
      choiceLayer.hidden=false;choiceLayer.inert=false;choiceLayer.setAttribute('aria-modal',String(choice.modal!==false));choiceLayer.setAttribute('aria-label',choice.ariaLabel||choice.title||'Dialogue');if(choice.lineId)choiceLayer.dataset.lineId=choice.lineId;else choiceLayer.removeAttribute('data-line-id');if(choice.speakerId)choiceLayer.dataset.speakerId=choice.speakerId;else choiceLayer.removeAttribute('data-speaker-id');choiceTitle.textContent=choice.title;choiceSpeaker.textContent=choice.speakerName||'';choiceSpeaker.hidden=!choice.speakerName;choiceOptions.textContent='';choiceDialogueGrid.dataset.side=choice.portraitSide||'left';
      const portrait=choice.portraitAssetId?assets.get(choice.portraitAssetId):null;if(portrait?.dataUrl){choicePortrait.src=portrait.dataUrl;choicePortrait.hidden=false;choiceDialogueGrid.dataset.portrait='present'}else{choicePortrait.hidden=true;choicePortrait.removeAttribute('src');choiceDialogueGrid.dataset.portrait='none'}
      revealChoiceBody(choice);
      choice.choices.forEach(function(choiceOption){const button=document.createElement('button');button.type='button';button.textContent=choiceOption.label;button.disabled=!choiceOption.enabled;button.dataset.choiceId=choiceOption.id;button.dataset.actionId=choiceOption.actionId;button.addEventListener('click',function(){if(engine.chooseChoice(choiceOption.id)){announce('Choice queued: '+choiceOption.label);syncUi()}});choiceOptions.append(button)});
      queueMicrotask(function(){choiceTitle.focus()});
    }
    ...
    function syncRunUi(message){const status=runCodes.getStatus();runToggle.hidden=!status.enabled;runToggle.setAttribute('aria-expanded',String(runDialog.open));if(!runSeed.value)runSeed.value=String(engine.getRunVariationState()?.seed||project.runVariationProgram?.defaultSeed||'standard');if(!runDay.value)runDay.value=new Date().toISOString().slice(0,10);if(message!==undefined)runStatus.textContent=message;else runStatus.textContent=status.enabled?'Seeds and UTC days resolve authored variation without browser storage.':'This game has no enabled run-variation program.'}
    ...
    function finishRunStart(state,message){releaseInputs();presentation.reset();handleEvents(engine.drainEvents(),{autoSave:false});gameShell.start();last=performance.now();accumulator=0;syncUi();syncRunUi(message);draw();announce(message);return{ok:true,state:JSON.parse(JSON.stringify(state)),run:JSON.parse(JSON.stringify(engine.getRunVariationState()))}}
    function startSeededRun(seed){try{return finishRunStart(engine.startRun({seed:String(seed||'').trim()}),'Started seed '+String(seed||'').trim())}catch(error){const message=error instanceof Error?error.message:String(error);syncRunUi(message);return{ok:false,error:message}}}
    function startDailyRun(utcDay){try{return finishRunStart(engine.startDailyChallenge({utcDay:String(utcDay||'').trim()}),'Started daily '+String(utcDay||'').trim())}catch(error){const message=error instanceof Error?error.message:String(error);syncRunUi(message);return{ok:false,error:message}}}
    function generateRunCode(){try{const code=runCodes.exportCode();runCode.value=code;runCode.focus();runCode.select();syncRunUi('LR1 run code generated. It starts this exact seed at tick zero.');return{ok:true,code:code,status:runCodes.getStatus()}}catch(error){const message=error instanceof Error?error.message:String(error);syncRunUi(message);return{ok:false,error:message,status:runCodes.getStatus()}}}
    function applyRunCode(code){const result=runCodes.importCode(String(code||''));if(result.ok)finishRunStart(result.state,'LR1 run restored at tick zero.');else syncRunUi(result.error);return result}
    function openRunDialog(){if(!runCodes.getStatus().enabled)return;if(gameShell.getState().state==='playing')gameShell.pause('user');syncRunUi();runDialog.showModal();runToggle.setAttribute('aria-expanded','true');queueMicrotask(function(){runSeed.focus();runSeed.select()})}
    function handleEvents(events,eventOptions={}){presentation.handleEvents(events);const persisted=saves.handleEvents(events,eventOptions);if(persisted&&!persisted.ok)syncSaveUi();events.forEach(function(event){if(event.type==='coin.collected')announce('Collected '+event.count);if(event.type==='player.respawned')announce('Respawned');if(event.type==='goal.reached')announce('Goal reached');if(event.type==='gameplay.rule-fired')announce('State changed: '+event.ruleId);if(event.type==='traversal.started')announce('Locked onto '+event.pathId);if(event.type==='traversal.completed')announce('Traversal complete');if(event.type==='traversal.bailed')announce('Traversal released');if(event.type==='choice.opened')announce('Decision ready');if(event.type==='choice.selected')announce('Selected '+event.choiceId);if(event.type==='quest.started')announce('Quest started: '+event.questId);if(event.type==='quest.completed')announce('Quest completed: '+event.questId);if(event.type==='quest.failed')announce('Quest failed: '+event.questId);if(event.type==='quest.progressed'&&event.completionChanged)announce('Quest objective updated: '+event.objectiveId);if(event.type==='clock.advanced')announce((event.clockId||'Clock')+' '+String(event.value));if(event.type==='gameplay.expression-fault')announce('Formula fault: '+event.fault);if(event.type==='projectile.hit')announce('Hit '+(event.actorId||event.objectId));if(event.type==='projectile.overflow')announce('Projectile pool full: '+event.emitterId);if(event.type==='health.depleted')announce('Defeated '+(event.actorId||event.objectId));if(event.type==='map.changed'){announce('Entered '+event.mapName);if(event.transition!=='instant')transitionAlpha=1}});gameShell.sync();syncUi()}
    ...
    function runtimeDepthKey(object,slice){const projection=activeProjection();if(projection.type==='dimetric-2:1'){const anchorX=Number(object.x||0)+Number(object.groundAnchor?.offsetX??Number(object.width||0)/2);const anchorY=Number(object.y||0)+Number(object.groundAnchor?.offsetY??object.height??0);return Number(object.depthLayer||0)*1_000_000_000+(anchorX+anchorY)*1024+Number(object.z||0)*32+Number(object.depthBias||0)+Number(slice?.depthBias||0)}return Number(object.depthLayer||0)*1_000_000_000+Number(object.z||0)*1_000_000+(Number(object.y||0)+Number(object.height||0))*100+Number(object.depthBias||0)+Number(slice?.depthBias||0)}
    function ghostRenderEntries(){const player=engine.getObjects().find(function(object){return object.kind==='player'});if(!player)return[];return engine.getGhostStates().map(function(ghost){const width=Math.max(8,Number(player.width||24));const height=Math.max(12,Number(player.height||34));const object={...player,id:'presentation-ghost:'+ghost.id,name:ghost.label,x:Number(ghost.x||0),y:Number(ghost.y||0),z:Number(ghost.z||0),supportZ:Number(ghost.z||0),vx:0,vy:0,width:width,height:height,color:ghost.color,opacity:ghost.opacity,solid:false,collected:false,hidden:false,collider:{...player.collider,enabled:false},groundAnchor:player.groundAnchor||{offsetX:width/2,offsetY:height}};return{kind:'ghost',id:object.id,depth:runtimeDepthKey(object,null),entry:{ghost:ghost,object:object}}})}
    function drawGhost(ghost,object){const width=Number(object.width);const height=Number(object.height);const placement=objectPlacement(object);ctx.save();ctx.globalAlpha=Math.max(.1,Math.min(.9,Number(ghost.opacity||.5)));ctx.translate(placement.x-object.x,placement.y-object.y);ctx.fillStyle=ghost.color||'#c4ccd4';ctx.strokeStyle='rgba(255,255,255,.78)';ctx.lineWidth=2;rounded(object.x,object.y,width,height,Math.min(9,width/3));ctx.strokeRect(object.x+1,object.y+1,Math.max(1,width-2),Math.max(1,height-2));ctx.fillStyle='rgba(255,255,255,.7)';ctx.fillRect(object.x+width*.2,object.y+height*.22,Math.max(2,width*.16),Math.max(2,height*.12));ctx.fillRect(object.x+width*.64,object.y+height*.22,Math.max(2,width*.16),Math.max(2,height*.12));ctx.restore()}
    ...
    function replayRunVariation(state){if(!state)return null;return{schemaVersion:state.schemaVersion,enabled:Boolean(state.enabled),mode:state.mode,seed:state.seed,utcDay:state.utcDay??null,selectionProgramDigest:state.selectionProgramDigest,selectionAlgorithm:state.selectionAlgorithm,selectionDigest:state.selectionDigest,selections:JSON.parse(JSON.stringify(state.selections??[])),assignments:JSON.parse(JSON.stringify(state.assignments??{}))}}
    ...
    function replaySnapshot(replayEngine,hashVersion){
      ...
      if(Number(hashVersion)>=13&&common.player)common.player={...common.player,groundObjectId:state.player.groundObjectId??null,interactableMode:state.player.interactableMode??'default',interactableTargetId:state.player.interactableTargetId??null,conveyorObjectId:state.player.conveyorObjectId??null};
    ...
    runToggle.addEventListener('click',openRunDialog);document.getElementById('run-start').addEventListener('click',function(){startSeededRun(runSeed.value)});document.getElementById('run-daily').addEventListener('click',function(){startDailyRun(runDay.value)});document.getElementById('run-export').addEventListener('click',generateRunCode);document.getElementById('run-import').addEventListener('click',function(){applyRunCode(runCode.value)});runDialog.addEventListener('close',function(){runToggle.setAttribute('aria-expanded','false');runToggle.focus()});
...
const runtimeApi={version:'${LOOPLAB_EXPORTED_RUNTIME_VERSION}',getSourceDigest:function(){return projectSourceDigest},getRuntimeAdapterInfo:getRuntimeAdapterInfo,getInputActionLiveness:function(){return JSON.parse(JSON.stringify(inputActionLiveness))},getCompletionReport:function(){return JSON.parse(JSON.stringify(completionReport))},getNarrativeContract:function(){return JSON.parse(JSON.stringify(project.narrativeContract||null))},getNarrativeReport:function(){return JSON.parse(JSON.stringify(narrativeReport))},getPresentationProgram:function(){return JSON.parse(JSON.stringify(project.presentationProgram||null))},getPresentationReport:function(){return JSON.parse(JSON.stringify(presentationReport))},getPresentationStatus:function(){return JSON.parse(JSON.stringify(presentation.getStatus()))},setAudioMuted:function(muted){const status=presentation.setMuted(muted!==false);syncAudioToggle();return JSON.parse(JSON.stringify(status))},getSaveReport:function(){return JSON.parse(JSON.stringify(saveReport))},getSaveStatus:function(){return JSON.parse(JSON.stringify(saves.getStatus()))},exportSaveCode:generatePortableSave,inspectSaveCode:function(code){return JSON.parse(JSON.stringify(saves.inspectCode(String(code||''))))},importSaveCode:applyPortableSave,persistHostedSave:function(){const result=saves.persistHosted();syncSaveUi();return JSON.parse(JSON.stringify(result))},clearHostedSave:function(){const result=saves.clearHosted();syncSaveUi();return JSON.parse(JSON.stringify(result))},getState:engine.getState,getObjects:function(){return engine.getObjects().map(function(object){return JSON.parse(JSON.stringify(object))})},getTraversalPaths:function(){return engine.getTraversalPaths().map(function(path){return JSON.parse(JSON.stringify(path))})},getCollisionGeometry:function(){return JSON.parse(JSON.stringify(engine.getCollisionGeometry()))},getElevationTransitions:function(){return JSON.parse(JSON.stringify(engine.getElevationTransitions()))},getMotionBodyStates:function(){return JSON.parse(JSON.stringify(engine.getMotionBodyStates()))},getActorStates:function(){return JSON.parse(JSON.stringify(engine.getActorStates()))},getNavigation:function(){return JSON.parse(JSON.stringify(engine.getNavigation()))},getGameplayState:function(){return JSON.parse(JSON.stringify(engine.getGameplayState()))},getChoiceState:function(){return JSON.parse(JSON.stringify(engine.getChoiceState()))},getQuestState:function(){return JSON.parse(JSON.stringify(engine.getQuestState()))},getHudState:function(){return JSON.parse(JSON.stringify(engine.getHudState()))},chooseChoice:function(choiceId){const queued=engine.chooseChoice(String(choiceId||''));syncUi();return queued},getRuntimeJoinPlan:function(){return JSON.parse(JSON.stringify(runtimeJoinPlan))},beginRuntimeJoinProbe:beginRuntimeJoinProbe,commitRuntimeJoinProbe:commitRuntimeJoinProbe,finishRuntimeJoinProbe:finishRuntimeJoinProbe,getCollisionBox:function(id){const object=typeof id==='string'?engine.getObjects().find(function(candidate){return candidate.id===id}):id;return object?engine.colliderBox(object):null},getPerformance:getPerformance,getAcceptanceTests:function(){return JSON.parse(JSON.stringify(project.acceptanceTests||[]))},runAcceptanceTest:runEmbeddedAcceptanceTest,runAcceptanceSuite:runEmbeddedAcceptanceSuite,getReplayCases:function(){return JSON.parse(JSON.stringify(project.replay?.cases||[]))},runReplayCase:runEmbeddedReplayCase,runReplaySuite:runEmbeddedReplaySuite,setInput:setInput,step:function(milliseconds){const delta=Number(milliseconds||0);const emitted=stepExact(delta);handleEvents(emitted);presentation.update(delta);draw();return {events:emitted,state:engine.getState(),performance:getPerformance(),presentation:presentation.getStatus(),save:saves.getStatus()}},reset:reset,loadMap:function(mapId,spawnId){const loaded=engine.loadMap(mapId,spawnId||null);handleEvents(engine.drainEvents());syncUi();draw();return loaded},pause:function(){manuallyPaused=true;releaseInputs();void presentation.suspend().then(syncAudioToggle);announce('Paused')},resume:function(){manuallyPaused=false;last=performance.now();accumulator=0;void presentation.resume().then(syncAudioToggle);announce('Ready')}};
    ...
    function runRuntimeCommand(command){
      ...
      if(command.op==='get_run_variation_program')return {ok:true,program:runtimeApi.getRunVariationProgram()};
      if(command.op==='get_run_variation_report')return {ok:true,report:runtimeApi.getRunVariationReport()};
      if(command.op==='get_run_variation_state')return {ok:true,run:runtimeApi.getRunVariationState()};
      if(command.op==='preview_run_variation')return {ok:true,run:runtimeApi.previewRunVariation(command.mode==='daily'?{mode:'daily',utcDay:command.utcDay}:{mode:'standard',seed:command.seed})};
      if(command.op==='start_run')return runtimeApi.startRun({seed:command.seed});
      if(command.op==='start_daily_challenge')return runtimeApi.startDailyChallenge({utcDay:command.utcDay});
      if(command.op==='get_ghost_states')return {ok:true,ghosts:runtimeApi.getGhostStates()};
      if(command.op==='get_run_code_status')return {ok:true,status:runtimeApi.getRunCodeStatus()};
      if(command.op==='export_run_code')return runtimeApi.exportRunCode();
      if(command.op==='inspect_run_code')return {ok:true,inspection:runtimeApi.inspectRunCode(String(command.code||''))};
      if(command.op==='import_run_code')return runtimeApi.importRunCode(String(command.code||''));
      ...
      if(command.op==='get_quest_state')return {ok:true,quests:runtimeApi.getQuestState()};
    ...
    async function initializeRuntime(){
      try{
        const bootSave=saves.restoreHosted();handleEvents(engine.drainEvents(),{autoSave:false});syncUi();syncAudioToggle();syncSaveUi();syncRunUi();syncShellUi();if(bootSave.restored)announce('Stored save restored');else if(saves.getStatus().hosted.configured&&saves.getStatus().hosted.state==='unavailable')announce('Ready — portable saves available');draw();await bootPrimaryRuntime();runtimeBridge.dataset.ready='true';document.dispatchEvent(new CustomEvent('looplab-runtime-ready',{bubbles:true,detail:{version:'${LOOPLAB_EXPORTED_RUNTIME_VERSION}',sourceDigest:projectSourceDigest,mapId:engine.getState().activeMapId,adapter:getRuntimeAdapterInfo(),presentation:presentation.getStatus(),gameShell:gameShell.getState(),save:saves.getStatus(),run:engine.getRunVariationState(),runCode:runCodes.getStatus()}}));
