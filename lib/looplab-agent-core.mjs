...
export function buildStandaloneRuntimePrelude() {
  return [
    ...
    "const createRuntimeModel=(project)=>createRuntimeModelFactory(project,{compileTileRuntimeProgram});",
...
export function compareIterationHistory(inputProject, firstId, secondId) {
  const project = syncActiveMap(clone(inputProject));
...
export function invalidateVerifiedAuthoring(previousProject, nextProject, options = {}) {
  ...
  const now = options.now ?? new Date().toISOString();
  const id = options.id ?? `iteration-${now.replace(/[:.]/g, "-")}-${Math.random().toString(16).slice(2, 8)}`;
  const iteration = {
    ...nextIteration,
    id,
...
export function validateProject(project) {
  ...
  for (const [index, map] of (project.maps ?? []).entries()) {
    ...
    validateObjects(map.objects, `${prefix}.objects`, assetIds, errors, warnings, { playerRequired });
...
export function applyAgentCommand(inputProject, command, options = {}) {
  ...
  switch (command.op) {
    ...
    case "set_runtime_profile": {
      ...
      if (!new Set(["canvas", "phaser"]).has(framework)) throw new Error("set_runtime_profile framework must be canvas or phaser. Other adapters are not release-ready yet.");
      const engineDelivery = framework === "phaser" ? "inline-script-tag" : "built-in-inline";
      ...
      result = {
        ...
        primaryFrameOwner: framework === "phaser" ? "phaser" : "looplab-canvas",
        renderAdapter: framework === "phaser" ? "phaser-canvas-with-looplab-draw-hook" : "looplab-canvas-2d",
        singleFileDelivery: engineDelivery,
        ...(framework === "phaser" ? { version: LOOPLAB_PHASER_BROWSER_VERSION, browserBundleBytes: LOOPLAB_PHASER_BROWSER_BYTES, browserBundleSha256: LOOPLAB_PHASER_BROWSER_SHA256 } : {}),
      };
      break;
    }
    ...
    case "import_path_editor_navigation": {
      ...
      changed = true;
      result = { navigation: clone(project.navigation), projection: clone(project.projection), authoredRoute: summarizeAuthoredRouteDocument(project.navigation.authoredRoute), sourceFormat: "path-editor-v2", mapId: project.activeMapId };
      break;
    }
    ...
    case "update_map": {
      ...
      const changes = clone(command.changes ?? {});
      const allowed = new Set(["name", "width", "height", "background", "gravity", "grid", "controlMode", "projection", "navigation", "objects", "traversalPaths", "collisionGeometry", "tileProgram", "clearanceZones", "hudSafeAreas", "maxInteractionGap", "interactionPolicy", "chunk"]);
    ...
    case "compare_iterations": {
      ...
      result = compareIterationHistory(project, ids[0], ids[1]);
      break;
    }
...
export function getAgentManifest() {
  return {
    ...
    gameStudioProductionPlan: {
      ...
      adapterStatus: { canvas: "release-ready", phaser: "release-ready-pinned-3.90.0", pixi: "knowledge-integrated-adapter-pending", melon: "knowledge-integrated-adapter-pending" },
    ...
    installedSkills: {
      ...
      oneFileRuntimePolicy: {
        ...
        pixi: { status: "knowledge-integrated-adapter-pending", targetDelivery: "inline-browser-umd-or-iife" },
        melon: { status: "knowledge-integrated-adapter-pending", targetDelivery: "tree-shaken-inline-iife" },
    ...
    verification: {
      ...
      visualPerception: {
        ...
        sources: ["known-runtime-object-geometry", "measured-hud-geometry", "object-bound-project-doctor-findings", "same-session-pixel-diff"],
    ...
    deterministicReplay: {
      ...
      digestAlgorithms: { legacyVersions: [1, 2, 3, 4, 5], legacy: "FNV-1a-32", sha256Versions: [LOOPLAB_REPLAY_SHA256_HASH_VERSION, LOOPLAB_REPLAY_COMBAT_HASH_VERSION, LOOPLAB_REPLAY_ACTOR_HASH_VERSION, LOOPLAB_REPLAY_COLLISION_HASH_VERSION, LOOPLAB_REPLAY_MOTION_CARRY_HASH_VERSION], currentVersion: LOOPLAB_REPLAY_MOTION_CARRY_HASH_VERSION, current: "SHA-256" },
      digestFormats: { legacy: "replay-<8 lowercase hex>", current: "replay-sha256-<64 lowercase hex>" },
      hashVersionPolicy: "Fixtures without hashVersion retain the original v1 FNV-1a projection; versions 1 through 9 remain byte-compatible. Every new or explicitly rerecorded fixture writes v10 SHA-256 with explicit motion-body rider, accepted-delta, and crush state in addition to v9 authored segment-ground contact. Runtime-state additions must add a new projection instead of silently invalidating accepted evidence.",
    ...
    authoredCollisionGeometry: {
      ...
      evidence: "Project Doctor validates geometry, winding, elevation, bounds, and references. Replay v9 introduced segment-ground identity, normal, and slide state; current v10 retains it while adding motion-body rider/crush state. Browser evidence verifies the rendered result.",
    ...
    iterationLedger: {
      ...
      selectionPolicy: "compare_iterations returns the same source-bound decision packet to the UI, CLI, MCP, Codex, and Claude. It reports hard-gate feasibility and Pareto relations without naming an automatic creative winner; changed candidates require explicit preview, play, and human-directed continuation.",
    ...
    exportReceipt: {
      schemaVersion: "looplab-export-receipt/v4",
    ...
    visualAuthoring: {
      ...
      dimetricMapEditor: {
        ...
        editorTools: ["select", "traversal", "navigation", "timed-route", "walkable-area", "blocked-area", "test-route"],
      ...
      navigationGraph: {
        ...
        supports: ["layer visibility and locks", "elevation ranges", "one-way links", "cost multipliers", "stable destinations", "walkable polygons", "blocked polygons", "admissible A-star test routes", "exact blocked-polygon crossing checks", "Path Editor v2 import and export", "lossless rich-route source documents", "animation timing", "per-stop waits", "facing and depth", "meeting windows", "route events", "deterministic hash receipts"],
        pathEditorRoundTrip: { baseFormat: "path-editor-v2", extension: "looplab-rich-route-v2", policy: "Portable percentage x/y remains available, while the Looplab extension losslessly preserves projection, z, the complete authored route source document, timings, waits, facing, meetings, events, depth, and hashes. Schedule edits mark preserved hashes stale until measured replay or render evidence verifies the exact current digest." },
    ...
    exportedRuntime: {
      ...
      engineDelivery: { canvas: "built-in-inline", phaser: "inline-script-tag" },
      pendingEngineTargets: { pixi: "inline-browser-umd-or-iife", melon: "tree-shaken-inline-iife" },
...
${embeddedStorageScript}${embeddedEngineScript}  <script>
    ...
    const pinnedRuntimeVendor=${runtimeFramework === "phaser" ? serializeProjectMetadata({ id: "phaser", version: LOOPLAB_PHASER_BROWSER_VERSION, sha256: LOOPLAB_PHASER_BROWSER_SHA256, bytes: LOOPLAB_PHASER_BROWSER_BYTES }) : "null"};
    ...
    let tileRuntimeCache={mapId:null,value:null};
    ...
    const presentation=createPresentationRuntime(project.presentationProgram,{host:globalThis,document:document,width:canvas.width,height:canvas.height,getPoint:presentationPoint});
    const gameShell=createGameShellRuntime(project.gameShell,{host:globalThis,getGameState:function(){return engine.getState()},getGameplayState:function(){return engine.getGameplayState()},getCombatState:function(){return engine.getCombatState()},releaseInputs:releaseInputs,suspendPresentation:function(){void presentation.suspend().then(syncAudioToggle)},resumePresentation:function(){void presentation.resume().then(syncAudioToggle)},resetSimulation:resetSimulationOnly,resetPresentation:function(){presentation.reset()},setMuted:function(value){presentation.setMuted(value)},setVolume:function(value){presentation.setMasterVolume(value)},setReducedMotion:function(value){presentation.setReducedMotion(value)},setTouchControlSize:function(value){document.getElementById('touch-controls')?.style?.setProperty?.('--looplab-touch-size',String(value)+'px')},openSettingsDialog:function(){if(!shellSettingsDialog.open)shellSettingsDialog.showModal()},closeSettingsDialog:function(){if(shellSettingsDialog.open)shellSettingsDialog.close()},focusTarget:function(state){queueMicrotask(function(){if(state==='playing')canvas.focus();else shellPrimary.focus()})},onChange:syncShellUi});
    const frameSamples=[];
    ...
    function syncUi(){const state=engine.getState();if(canvas.width!==state.width||canvas.height!==state.height){if(primaryRuntime?.scale?.resize)primaryRuntime.scale.resize(state.width,state.height);else{canvas.width=state.width;canvas.height=state.height}}score.textContent=String(state.collectedCount);mapName.textContent=state.mapName||state.activeMapId;const explicitHud=(project.gameplayProgram?.hudBindings||[]).length>0;const visible=explicitHud?[]:(project.gameplayProgram?.variables||[]).filter(function(variable){return variable.visible===true}).map(function(variable){return (variable.label||variable.id)+': '+String(state.variables?.[variable.id]??'')});gameplayState.textContent=visible.join(' · ');gameplayState.hidden=visible.length===0;canvas.setAttribute('aria-label','${title} — '+(state.mapName||state.activeMapId));syncSystemsUi();syncShellUi()}
    ...
    function drawObject(object,slice){ctx.save();ctx.globalAlpha=Math.max(0,Math.min(1,Number(object.opacity??1)));const asset=object.assetId&&assets.get(object.assetId);const image=asset&&assetImages.get(asset.id);if(!asset&&activeProjection().type==='dimetric-2:1'&&object.kind==='platform'){drawDimetricPlatform(object);ctx.restore();return}const placement=objectPlacement(object);ctx.translate(placement.x-Number(object.x||0),placement.y-Number(object.y||0));const feedbackTransform=presentation.getObjectTransform(object.id);if(feedbackTransform.scaleX!==1||feedbackTransform.scaleY!==1){const anchorX=Number(object.x||0)+Number(object.width||0)/2;const anchorY=Number(object.y||0)+Number(object.height||0);ctx.translate(anchorX,anchorY);ctx.scale(feedbackTransform.scaleX,feedbackTransform.scaleY);ctx.translate(-anchorX,-anchorY)}const bounds=sliceBounds(object,slice,asset);if(bounds){ctx.beginPath();ctx.rect(object.x,bounds.destinationY,object.width,bounds.destinationHeight);ctx.clip()}if(asset&&image&&image.complete&&image.naturalWidth){let requested=object.assetFrame||0;if(asset.frames>1&&object.kind==='coin')requested=Math.floor(visualTick/8)%asset.frames;if(asset.frames>1&&object.kind==='player'){const state=engine.getState();requested=state.activeTraversalPathId?Math.min(3,asset.frames-1):Math.abs(Number(object.vy||0))>48?Math.min(2,asset.frames-1):Math.abs(Number(object.vx||0))>18?Math.min(Math.floor(visualTick/8)%2,asset.frames-1):0}const frame=Math.max(0,Math.min(asset.frames-1,requested));const columns=Math.max(1,asset.columns||1);const sx=(frame%columns)*asset.frameWidth;const sy=Math.floor(frame/columns)*asset.frameHeight;if(bounds)ctx.drawImage(image,sx,sy+bounds.sourceY,asset.frameWidth,bounds.sourceHeight,object.x,bounds.destinationY,object.width,bounds.destinationHeight);else ctx.drawImage(image,sx,sy,asset.frameWidth,asset.frameHeight,object.x,object.y,object.width,object.height)}else drawPrimitive(object);ctx.restore()}
    function activeTileRuntime(){const mapId=engine.getState().activeMapId;if(tileRuntimeCache.mapId!==mapId){tileRuntimeCache={mapId:mapId,value:engine.getTileRuntime()}}return tileRuntimeCache.value}
    function drawTile(entry){const asset=assets.get(entry.assetId);const image=asset&&assetImages.get(asset.id);if(!asset||!image||!image.complete||!image.naturalWidth)return;const projection=activeProjection();const anchorPoint=projection.type==='dimetric-2:1'?worldToScreen({x:entry.worldX,y:entry.worldY,z:entry.z},projection):{x:entry.worldX,y:entry.worldY};const width=Number(entry.destinationWidth||asset.frameWidth||1);const height=Number(entry.destinationHeight||asset.frameHeight||1);const anchor=entry.anchor||'top-left';const anchorX=anchor==='bottom-center'||anchor==='center'?width/2:0;const anchorY=anchor==='bottom-left'||anchor==='bottom-center'?height:anchor==='center'?height/2:0;const x=Math.round(anchorPoint.x-anchorX+Number(entry.drawOffsetX||0));const y=Math.round(anchorPoint.y-anchorY+Number(entry.drawOffsetY||0));if(x+width<0||y+height<0||x>canvas.width||y>canvas.height)return;const frame=Math.max(0,Math.min(Number(asset.frames||1)-1,Number(entry.frame||0)));const columns=Math.max(1,Number(asset.columns||1));const sx=(frame%columns)*Number(asset.frameWidth||width);const sy=Math.floor(frame/columns)*Number(asset.frameHeight||height);ctx.save();ctx.globalAlpha=Math.max(0,Math.min(1,Number(entry.opacity??1)));ctx.globalCompositeOperation=entry.blendMode==='normal'?'source-over':entry.blendMode||'source-over';ctx.translate(x,y);if(entry.flipD)ctx.transform(0,height/width,width/height,0,0,0);if(entry.flipH){ctx.translate(width,0);ctx.scale(-1,1)}if(entry.flipV){ctx.translate(0,height);ctx.scale(1,-1)}ctx.drawImage(image,sx,sy,Number(asset.frameWidth||width),Number(asset.frameHeight||height),0,0,width,height);ctx.restore()}
    function draw(){visualTick+=1;const state=engine.getState();ctx.fillStyle=state.background||'#111';ctx.fillRect(0,0,canvas.width,canvas.height);ctx.imageSmoothingEnabled=false;const cameraOffset=presentation.getCameraOffset();ctx.save();ctx.translate(cameraOffset.x,cameraOffset.y);const tileEntries=activeTileRuntime().visualEntries||[];tileEntries.filter(function(entry){return entry.role==='ground-static'}).forEach(drawTile);drawTraversalPaths();const interleaved=[...engine.renderEntries().map(function(entry){return{kind:'object',id:entry.object.id,depth:entry.depth,entry:entry}}),...tileEntries.filter(function(entry){return entry.role==='interleaved'}).map(function(entry){return{kind:'tile',id:entry.id,depth:entry.depth,entry:entry}})].sort(function(first,second){return first.depth-second.depth||String(first.id).localeCompare(String(second.id))});interleaved.forEach(function(item){if(item.kind==='tile')drawTile(item.entry);else drawObject(item.entry.object,item.entry.slice)});tileEntries.filter(function(entry){return entry.role==='foreground'}).forEach(drawTile);presentation.drawWorld(ctx);ctx.restore();presentation.drawOverlay(ctx,canvas.width,canvas.height);transitionAlpha=Math.max(0,transitionAlpha-.08);transition.style.opacity=String(transitionAlpha)}
    ...
    function frame(now,scheduleNext=true,providedElapsedMs=null){const elapsedMs=Math.max(0,providedElapsedMs??(now-last));last=now;recordFrame(elapsedMs);syncGamepadInputs();if(!gameShell.getState().simulationBlocked&&!document.hidden){presentation.update(elapsedMs);handleEvents(advanceFixed(elapsedMs/1000))}if(scheduleNext){draw();requestAnimationFrame(frame)}}
    function bootPrimaryRuntime(){
      if(runtimeFramework!=='phaser'){requestAnimationFrame(frame);return}
    ...
    function getRuntimeAdapterInfo(){return {framework:runtimeFramework,primaryFrameOwner:runtimeFramework==='phaser'?'phaser':'looplab-canvas',renderAdapter:runtimeFramework==='phaser'?'phaser-canvas-with-looplab-draw-hook':'looplab-canvas-2d',singleFile:true,networkFree:true,exportProfile:saveReport.profile,storageFree:saveReport.profile==='strict',portableSaves:saves.getStatus().enabled,vendor:pinnedRuntimeVendor?{...pinnedRuntimeVendor,loadedVersion:globalThis.Phaser?.VERSION??null}:null}}
    ...
    const bootSave=saves.restoreHosted();handleEvents(engine.drainEvents(),{autoSave:false});syncUi();syncAudioToggle();syncSaveUi();syncShellUi();if(bootSave.restored)announce('Stored save restored');else if(saves.getStatus().hosted.configured&&saves.getStatus().hosted.state==='unavailable')announce('Ready — portable saves available');draw();bootPrimaryRuntime();runtimeBridge.dataset.ready='true';document.dispatchEvent(new CustomEvent('looplab-runtime-ready',{bubbles:true,detail:{version:'${LOOPLAB_EXPORTED_RUNTIME_VERSION}',sourceDigest:projectSourceDigest,mapId:engine.getState().activeMapId,adapter:getRuntimeAdapterInfo(),presentation:presentation.getStatus(),gameShell:gameShell.getState(),save:saves.getStatus()}}));
  </script>
</body>
</html>`;
  const audit = assertStandaloneHtml(html);
