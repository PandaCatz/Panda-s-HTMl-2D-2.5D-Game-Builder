...
export function applyAgentCommand(inputProject, command, options = {}) {
  ...
  switch (command.op) {
    ...
    case "update_map": {
      ...
      const changes = clone(command.changes ?? {});
      const allowed = new Set(["name", "width", "height", "background", "gravity", "grid", "controlMode", "projection", "navigation", "objects", "traversalPaths", "collisionGeometry", "elevationTransitions", "tileProgram", "clearanceZones", "hudSafeAreas", "maxInteractionGap", "interactionPolicy", "chunk"]);
...
export function getAgentManifest() {
  return {
    ...
    deterministicReplay: {
      ...
      digestAlgorithms: { legacyVersions: [1, 2, 3, 4, 5], legacy: "FNV-1a-32", sha256Versions: [LOOPLAB_REPLAY_SHA256_HASH_VERSION, LOOPLAB_REPLAY_COMBAT_HASH_VERSION, LOOPLAB_REPLAY_ACTOR_HASH_VERSION, LOOPLAB_REPLAY_COLLISION_HASH_VERSION, LOOPLAB_REPLAY_MOTION_CARRY_HASH_VERSION, LOOPLAB_REPLAY_ELEVATION_HASH_VERSION], currentVersion: LOOPLAB_REPLAY_ELEVATION_HASH_VERSION, current: "SHA-256" },
      digestFormats: { legacy: "replay-<8 lowercase hex>", current: "replay-sha256-<64 lowercase hex>" },
      hashVersionPolicy: "Fixtures without hashVersion retain the original v1 FNV-1a projection; versions 1 through 10 remain byte-compatible. Every new or explicitly rerecorded fixture writes v11 SHA-256 with active elevation-transition identity, progress, and support Z in addition to the v10 motion-body and v9 authored segment-ground state. Runtime-state additions must add a new projection instead of silently invalidating accepted evidence.",
