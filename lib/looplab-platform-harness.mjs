...
export async function runPlatformHarness(options = {}) {
  ...
  try {
    ...
    checks.push(pageErrors.length === 0 && relevantConsoleErrors.length === 0
      ...
      : failedCheck("no-unhandled-errors", `${pageErrors.length} page error(s) and ${relevantConsoleErrors.length} console error(s) were observed.`, { pageErrors, pageErrorDetails, consoleErrors: relevantConsoleErrors }));
    try {
    ...
    if (abortSignal?.aborted || error?.name === "AbortError") {
      ...
      checks.push(failedCheck("runtime-ready", error instanceof Error ? error.message : String(error), { pageErrors, pageErrorDetails, consoleErrors, observedRequests }));
    } else {
