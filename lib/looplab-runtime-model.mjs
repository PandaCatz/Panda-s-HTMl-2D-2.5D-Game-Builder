export function createRuntimeModel(project) {
  ...
  function updatePlatformer(player, dt, direction, jumping, jumpPressed) {
    ...
    if (resolvePlatformerFloorSegments(player, previousBox, wasGrounded, previousHorizontalBox)) {
      ...
      setObjectZ(player, groundSegment?.zMin ?? player.z ?? 0);
    }
  ...
  function validateSaveState(input) {
    ...
    const stateFields = saveVersion === 4 ? [...baseStateFields, "motionBodyStates", "combatState", "actorStates"] : saveVersion === 3 ? [...baseStateFields, "motionBodyStates", "combatState"] : saveVersion === 2 ? [...baseStateFields, "motionBodyStates"] : baseStateFields;
    if (!exactFields(input, stateFields, "save state")) return { valid: false, errors, state: null };
    if (![1, 2, 3, 4].includes(saveVersion)) addError("Save state version must be 1, 2, 3, or 4.");
    const expectedSaveSchema = saveVersion === 4 ? "looplab-runtime-save-state/v4" : saveVersion === 3 ? "looplab-runtime-save-state/v3" : saveVersion === 2 ? "looplab-runtime-save-state/v2" : "looplab-runtime-save-state/v1";
    ...
    const state = errors.length ? null : {
      schemaVersion: saveVersion === 4 ? "looplab-runtime-save-state/v4" : saveVersion === 3 ? "looplab-runtime-save-state/v3" : saveVersion === 2 ? "looplab-runtime-save-state/v2" : "looplab-runtime-save-state/v1",
      version: saveVersion,
  ...
  function exportSaveState() {
    ...
    const saveVersion = hasActorState ? 4 : hasCombatState ? 3 : savedMotionBodyStates.length ? 2 : 1;
    return {
      schemaVersion: saveVersion === 4 ? "looplab-runtime-save-state/v4" : saveVersion === 3 ? "looplab-runtime-save-state/v3" : saveVersion === 2 ? "looplab-runtime-save-state/v2" : "looplab-runtime-save-state/v1",
      version: saveVersion,
