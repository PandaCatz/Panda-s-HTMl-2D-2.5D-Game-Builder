export function createRuntimeModel(project) {
  ...
  const normalizeRunVariationProgram = typeof runtimeDependencies.normalizeRunVariationProgram === "function"
    ? runtimeDependencies.normalizeRunVariationProgram
    : (input = {}) => ({ schemaVersion: "looplab-run-variation-program/v1", version: 1, enabled: input?.enabled === true, seedNamespace: String(input?.seedNamespace ?? "looplab-run"), defaultSeed: String(input?.defaultSeed ?? "standard"), dailyChallenge: { enabled: input?.dailyChallenge?.enabled === true, namespace: String(input?.dailyChallenge?.namespace ?? "daily") }, pools: Array.isArray(input?.pools) ? clone(input.pools) : [], ghosts: Array.isArray(input?.ghosts) ? clone(input.ghosts) : [], acceptanceTestIds: Array.isArray(input?.acceptanceTestIds) ? clone(input.acceptanceTestIds) : [] });
  const runVariationProgramDigest = typeof runtimeDependencies.runVariationProgramDigest === "function"
    ? runtimeDependencies.runVariationProgramDigest
    : () => "sha256:" + "0".repeat(64);
  const resolveRunVariation = typeof runtimeDependencies.resolveRunVariation === "function"
    ? runtimeDependencies.resolveRunVariation
    : (input = {}, options = {}) => ({ schemaVersion: "looplab-run-variation-state/v1", enabled: false, mode: options.mode === "daily" ? "daily" : "standard", seed: String(options.seed ?? input?.defaultSeed ?? "standard"), utcDay: options.mode === "daily" ? String(options.utcDay ?? "") : null, programDigest: runVariationProgramDigest(input), selectionProgramDigest: "sha256:" + "0".repeat(64), selectionAlgorithm: "disabled", selectionDigest: "sha256:" + "0".repeat(64), selections: [], assignments: {} });
  const runtimeObjectChangeKeys = new Set([
    "hidden", "solid", "color", "opacity", "runtimeState", "active", "conducted", "cooldownTicks", "dropThroughTicks", "durationTicks", "elapsedTicks", "enabled", "energy", "health", "hp", "interactableStateTicks", "locked", "maxHp", "mode", "motionX", "motionY", "muted", "open", "ownerId", "pathId", "pathProgressVariableId", "phase", "pinTicks", "progress", "resonantTicks", "rootStage", "staggerTicks", "state", "supportId", "targetId", "threaded", "value",
  ]);
  const fallbackMap = {
    id: "main",
    name: "Main map",
  ...
  const runVariationProgram = normalizeRunVariationProgram(project.runVariationProgram ?? { enabled: false });
  ...
  const quests = Array.isArray(gameplayProgram.quests) ? gameplayProgram.quests : [];
  const narrativeCharacters = new Map((project.narrativeContract?.characters || []).map((character) => [character.id, character]));
  ...
  function resetGameplayVariables(scope = "session") {
    if (scope === "session") for (const key of Object.keys(gameplayVariables)) delete gameplayVariables[key];
    for (const variable of gameplayProgram.variables || []) {
      if (!variable || typeof variable.id !== "string" || !variable.id) continue;
      if (scope === "run" && variable.resetPolicy === "session" && Object.prototype.hasOwnProperty.call(gameplayVariables, variable.id)) continue;
      gameplayVariables[variable.id] = clone(variable.initial);
    }
  }

  function applyRunVariationAssignments() {
    for (const [variableId, value] of Object.entries(runVariationState?.assignments ?? {})) {
      if (!variableDefinition(variableId)) continue;
      gameplayVariables[variableId] = clone(value);
    }
  }
  ...
  function questStatusName(value) {
    return ({ 0: "inactive", 1: "active", 2: "completed", 3: "failed" })[Number(value)] || "invalid";
  }

  function setGameplayVariable(id, value, sourceId = null) {
    if (!variableDefinition(id)) return false;
    const previous = gameplayVariables[id];
    const affectedQuests = quests.filter((quest) => quest.statusVariableId === id);
    const affectedObjectives = quests.flatMap((quest) => (quest.objectives || [])
      .filter((objective) => objective.progressVariableId === id)
      .map((objective) => ({ quest, objective, completed: conditionListPasses(objective.completeWhen), progress: Number(gameplayVariables[id] || 0) })));
    const next = boundedVariableValue(id, value);
    if (previous === next) return false;
    gameplayVariables[id] = next;
    gameplayRevision += 1;
    for (const quest of affectedQuests) {
      const previousStatus = questStatusName(previous);
      const status = questStatusName(next);
      const type = status === "active" ? "quest.started" : status === "completed" ? "quest.completed" : status === "failed" ? "quest.failed" : "quest.status-changed";
      emit(type, { mapId: activeMap?.id, questId: quest.id, variableId: id, previousStatus, status, sourceId });
    }
    for (const entry of affectedObjectives) {
      const completed = conditionListPasses(entry.objective.completeWhen);
      emit("quest.progressed", {
        mapId: activeMap?.id,
        questId: entry.quest.id,
        objectiveId: entry.objective.id,
        variableId: id,
        previousValue: entry.progress,
        value: Number(next || 0),
        target: entry.objective.target ?? null,
        completed,
        completionChanged: entry.completed !== completed,
        sourceId,
      });
    }
    return true;
  ...
  function setRuntimeObjectChanges(object, changes) {
    if (!object || !changes || typeof changes !== "object") return false;
    const accepted = {};
    let changed = false;
    for (const [key, value] of Object.entries(changes)) {
      if (key !== "colliderEnabled" && !runtimeObjectChangeKeys.has(key)) continue;
      const current = key === "colliderEnabled" ? object.collider?.enabled !== false : object[key];
      if (current !== value) changed = true;
      accepted[key] = value;
    }
    if (!Object.keys(accepted).length || !changed) return false;
    const key = runtimeKey(activeMap.id, object.id);
    const merged = { ...(objectOverrides.get(key) || {}), ...accepted };
    objectOverrides.set(key, merged);
    applyObjectOverride(object, merged);
    gameplayRevision += 1;
    return true;
  }
  ...
  function colliderBoxIgnoringEnabled(object) {
  ...
  function sweptBoxesOverlap(start, end, target) {
    if (!start || !end || !target) return false;
    if (boxesOverlap(start, target) || boxesOverlap(end, target)) return true;
    const axes = [
      { startMin: start.x, startMax: start.x + start.width, endMin: end.x, targetMin: target.x, targetMax: target.x + target.width },
      { startMin: start.y, startMax: start.y + start.height, endMin: end.y, targetMin: target.y, targetMax: target.y + target.height },
      { startMin: start.zMin, startMax: start.zMax, endMin: end.zMin, targetMin: target.zMin, targetMax: target.zMax },
    ];
    let entry = -Infinity;
    let exit = Infinity;
    for (const axis of axes) {
      const delta = axis.endMin - axis.startMin;
      if (Math.abs(delta) <= 0.0000001) {
        if (axis.startMax <= axis.targetMin || axis.startMin >= axis.targetMax) return false;
        continue;
      }
      const first = (axis.targetMin - axis.startMax) / delta;
      const second = (axis.targetMax - axis.startMin) / delta;
  ...
  function sweptOverlapsObject(startPlayerBox, player, target) {
    if (!target || target.hidden || target.collider?.enabled === false) return false;
    return sweptBoxesOverlap(startPlayerBox, colliderBox(player), colliderBox(target));
  }
  ...
  function getChoiceState() {
    ...
    if (!page) return null;
    const speaker = page.speakerId ? narrativeCharacters.get(page.speakerId) || null : null;
    return {
      id: page.id,
      title: interpolateGameplayText(page.title),
      body: interpolateGameplayText(page.body),
      modal: page.modal !== false,
      lineId: page.lineId || null,
      speakerId: page.speakerId || null,
      speakerName: speaker?.name || null,
      portraitAssetId: page.portraitAssetId || null,
      portraitSide: page.portraitSide || "left",
      revealMode: page.revealMode || "instant",
      charactersPerSecond: Number(page.charactersPerSecond || 0),
      ariaLabel: interpolateGameplayText(page.ariaLabel || page.title || speaker?.name || "Dialogue"),
  ...
  function getQuestState() {
    return {
      ...
      quests: quests
        .map((quest) => {
          const statusValue = Number(gameplayVariables[quest.statusVariableId]);
          const visible = conditionListPasses(quest.visibleWhen) && statusValue !== 0;
          const giver = quest.giverId ? narrativeCharacters.get(quest.giverId) || null : null;
          return {
            id: quest.id,
            title: interpolateGameplayText(quest.title),
            description: interpolateGameplayText(quest.description),
            giverId: quest.giverId || null,
            giverName: giver?.name || null,
            statusVariableId: quest.statusVariableId,
            statusValue,
            status: questStatusName(statusValue),
            visible,
            objectives: (quest.objectives || [])
              .map((objective) => {
                const progress = objective.progressVariableId ? Number(gameplayVariables[objective.progressVariableId] || 0) : null;
                const target = objective.target ?? null;
                return {
                  id: objective.id,
                  label: interpolateGameplayText(objective.label),
                  visible: conditionListPasses(objective.visibleWhen),
                  completed: conditionListPasses(objective.completeWhen),
                  progressVariableId: objective.progressVariableId || null,
                  progress,
                  target,
                };
              })
  ...
  function applyGameplayEffect(effect, rule, player) {
    if (effect.type === "set-variable") setGameplayVariable(effect.variableId, effect.value, rule.id);
    else if (effect.type === "add-variable") setGameplayVariable(effect.variableId, Number(gameplayVariables[effect.variableId] || 0) + Number(effect.value || 0), rule.id);
    else if (effect.type === "set-variable-expression") {
      const result = evaluateIntegerExpression(effect.expression);
      setGameplayVariable(effect.variableId, result.value, rule.id);
    ...
    else if (effect.type === "toggle-variable") setGameplayVariable(effect.variableId, !gameplayVariables[effect.variableId], rule.id);
      ...
      if (clock) {
        ...
        setGameplayVariable(clock.variableId, safeInteger(next), rule.id);
  ...
  function prepareRuntimeObject(source, mapId) {
    return {
      ...
      interactableMode: "default",
      interactableTargetId: null,
      conveyorObjectId: null,
      authoredHidden: Boolean(source.hidden),
      authoredSolid: Boolean(source.solid),
      authoredColliderEnabled: source.collider?.enabled !== false,
      collected: collectedIds.has(mapId + ":" + source.id) || (source.kind === "key" && collectedIds.has(keyToken(source.interactable?.parameters?.keyId))),
    };
  }
  ...
  function resetRuntime(options = {}) {
    const preserveSessionVariables = options.preserveSessionVariables === true;
    const runOptions = options.runOptions && typeof options.runOptions === "object" ? options.runOptions : {};
    clearInputState();
    collectedIds.clear();
    ...
    resetActorStates();
    overlapContacts.clear();
    resetGameplayVariables(preserveSessionVariables ? "run" : "session");
    runVariationState = resolveRunVariation(runVariationProgram, runOptions);
    applyRunVariationAssignments();
    simulationTick = 0;
    interactionHeld = false;
    portalCooldown = 0;
    activeTraversal = null;
    ...
    if (runVariationProgram.enabled) emit("run.started", { mode: runVariationState.mode, seed: runVariationState.seed, utcDay: runVariationState.utcDay, selectionDigest: runVariationState.selectionDigest });
    return getRunVariationState();
  }

  function reset() {
    return resetRuntime({ preserveSessionVariables: false, runOptions: { mode: "standard", seed: runVariationProgram.defaultSeed } });
  }

  function previewRunVariation(options = {}) {
    return clone(resolveRunVariation(runVariationProgram, options));
  }

  function startRun(options = {}) {
    return resetRuntime({ preserveSessionVariables: true, runOptions: { mode: "standard", seed: options.seed ?? runVariationProgram.defaultSeed } });
  }

  function startDailyChallenge(options = {}) {
    return resetRuntime({ preserveSessionVariables: true, runOptions: { mode: "daily", utcDay: options.utcDay } });
  }

  function getRunVariationState() {
    return { ...clone(runVariationState), simulationTick };
  }

  function getGhostStates() {
    const states = [];
    for (const ghost of runVariationProgram.ghosts || []) {
      const frames = Array.isArray(ghost.frames) ? ghost.frames : [];
      let before = null;
      let after = null;
      for (const frame of frames) {
        if (frame.mapId !== activeMap.id) continue;
        if (frame.tick <= simulationTick) before = frame;
        if (frame.tick >= simulationTick) { after = frame; break; }
      }
      const first = before ?? after;
      if (!first) continue;
      const second = after ?? before;
      const span = Math.max(0, Number(second.tick) - Number(first.tick));
      const alpha = span > 0 ? Math.max(0, Math.min(1, (simulationTick - Number(first.tick)) / span)) : 0;
      states.push({
        schemaVersion: "looplab-replay-ghost-state/v1",
        id: ghost.id,
        label: ghost.label,
        replayCaseId: ghost.replayCaseId,
        tick: simulationTick,
        mapId: activeMap.id,
        x: Number(first.x) + (Number(second.x) - Number(first.x)) * alpha,
        y: Number(first.y) + (Number(second.y) - Number(first.y)) * alpha,
        z: Number(first.z || 0) + (Number(second.z || 0) - Number(first.z || 0)) * alpha,
        facingX: alpha < 0.5 ? Number(first.facingX || 1) : Number(second.facingX || 1),
        color: ghost.color,
        opacity: Number(ghost.opacity),
        presentationOnly: true,
      });
    }
    return states.sort((first, second) => compareStableIds(first.id, second.id));
  }
  ...
  function solidObjects(player) {
    return [
      ...objects.filter((object) => object !== player && !object.hidden && object.solid && object.collider?.enabled !== false && !object.collider?.trigger && !(object.kind === "one-way-platform" && Number(object.dropThroughTicks || 0) > 0)),
  ...
  function updatePlatformer(player, dt, direction, jumping, jumpPressed) {
    ...
    for (const solid of solids) {
      ...
      if (player.vy >= 0 && crossedTop) {
        ...
        player.vy = 0;
        player.grounded = true;
        player.groundObjectId = solid.id ?? null;
  ...
  function interactableParameters(object) {
    const parameters = object?.interactable?.parameters;
    return parameters && typeof parameters === "object" && !Array.isArray(parameters) ? parameters : {};
  }

  function tickInteractableStates(player, tickDelta) {
    if (tickDelta <= 0) return;
    for (const object of objects) {
      if (!object?.interactable) continue;
      if (Number(object.cooldownTicks || 0) > 0) setRuntimeObjectChanges(object, { cooldownTicks: Math.max(0, Number(object.cooldownTicks || 0) - tickDelta) });
      if (Number(object.dropThroughTicks || 0) > 0) setRuntimeObjectChanges(object, { dropThroughTicks: Math.max(0, Number(object.dropThroughTicks || 0) - tickDelta) });
      if (object.kind !== "crumble-platform") continue;
      const parameters = interactableParameters(object);
      const state = object.runtimeState || "idle";
      if (state === "armed") {
        const remaining = Math.max(0, Number(object.interactableStateTicks || 0) - tickDelta);
        if (remaining > 0) {
          setRuntimeObjectChanges(object, { interactableStateTicks: remaining });
        } else {
          setRuntimeObjectChanges(object, {
            runtimeState: "disabled",
            interactableStateTicks: Math.max(1, Number(parameters.disabledTicks || 1)),
            hidden: parameters.hideWhenDisabled === true,
            solid: false,
            colliderEnabled: false,
          });
          emit("crumble.fell", { mapId: activeMap.id, objectId: object.id, instanceId: object.interactable.instanceId });
        }
      } else if (state === "disabled" && parameters.reset === true) {
        const remaining = Math.max(0, Number(object.interactableStateTicks || 0) - tickDelta);
        if (remaining > 0) {
          setRuntimeObjectChanges(object, { interactableStateTicks: remaining });
        } else if (player && boxesOverlap(colliderBoxIgnoringEnabled(player), colliderBoxIgnoringEnabled(object))) {
          setRuntimeObjectChanges(object, { interactableStateTicks: 1 });
        } else {
          setRuntimeObjectChanges(object, {
            runtimeState: "idle",
            interactableStateTicks: 0,
            hidden: Boolean(object.authoredHidden),
            solid: Boolean(object.authoredSolid),
            colliderEnabled: object.authoredColliderEnabled !== false,
          });
          emit("crumble.reset", { mapId: activeMap.id, objectId: object.id, instanceId: object.interactable.instanceId });
        }
      }
    }
  }

  function tryDropThroughPlatform(player, down, jumpPressed) {
    if (!down || !jumpPressed || !player.groundObjectId) return false;
    const support = objects.find((object) => object.id === player.groundObjectId);
    if (!support || support.kind !== "one-way-platform" || support.collider?.oneWay !== true) return false;
    const parameters = interactableParameters(support);
    setRuntimeObjectChanges(support, { dropThroughTicks: Math.max(2, Number(parameters.dropThroughTicks || 2)) });
    player.y += Math.max(0, Number(parameters.dropNudge || 0));
    player.grounded = false;
    player.coyoteTicksRemaining = 0;
    player.jumpBufferTicksRemaining = 0;
    clearGroundContact(player);
    emit("one-way.dropped", { mapId: activeMap.id, objectId: support.id, instanceId: support.interactable.instanceId });
    return true;
  }

  function exitLadder(player, reason, applyImpulse = false) {
    if (player.interactableMode !== "ladder") return false;
    const ladder = objects.find((object) => object.id === player.interactableTargetId);
    const parameters = interactableParameters(ladder);
    const ladderId = player.interactableTargetId;
    player.interactableMode = "default";
    player.interactableTargetId = null;
    if (applyImpulse) player.vy = Number(parameters.exitImpulseY ?? -260);
    emit("ladder.exited", { mapId: activeMap.id, objectId: ladderId, instanceId: ladder?.interactable?.instanceId ?? null, reason });
    return true;
  }

  function tryEnterLadder(player, freshInteraction, direction) {
    if (player.interactableMode === "ladder") return false;
    const playerBox = colliderBox(player);
    if (!playerBox) return false;
    const ladder = objects.find((object) => {
      if (object.hidden || object.kind !== "ladder" || object.collider?.enabled === false) return false;
      const parameters = interactableParameters(object);
      if (parameters.requireFreshInteract !== false && !freshInteraction) return false;
      if (parameters.requireFreshInteract === false && !freshInteraction && Number(direction.y || 0) === 0) return false;
      return boxesOverlap(playerBox, colliderBox(object));
    });
    if (!ladder) return false;
    player.interactableMode = "ladder";
    player.interactableTargetId = ladder.id;
    player.vx = 0;
    player.vy = 0;
    player.grounded = false;
    clearGroundContact(player);
    emit("ladder.entered", { mapId: activeMap.id, objectId: ladder.id, instanceId: ladder.interactable.instanceId });
    return true;
  }

  function updateLadder(player, dt, direction, freshInteraction, jumpExitPressed) {
    if (player.interactableMode !== "ladder") return false;
    const ladder = objects.find((object) => object.id === player.interactableTargetId && object.kind === "ladder" && !object.hidden && object.collider?.enabled !== false);
    if (!ladder) {
      exitLadder(player, "missing", false);
      return false;
    }
    if (freshInteraction || jumpExitPressed) {
      exitLadder(player, freshInteraction ? "interact" : "jump", jumpExitPressed);
      return false;
    }
    const parameters = interactableParameters(ladder);
    const ladderBox = colliderBox(ladder);
    const playerBox = colliderBox(player);
    if (!ladderBox || !playerBox) {
      exitLadder(player, "invalid-collider", false);
      return false;
    }
    const targetX = ladderBox.x + ladderBox.width / 2 - Number(player.collider?.offsetX || 0) - playerBox.width / 2;
    const startX = Number(player.x || 0);
    player.x = moveToward(startX, targetX, Math.max(0, Number(parameters.snapSpeed || 0)) * dt);
    player.y += Number(direction.y || 0) * Math.max(0, Number(parameters.climbSpeed || 0)) * dt;
    player.vx = dt > 0 ? (player.x - startX) / dt : 0;
    player.vy = Number(direction.y || 0) * Math.max(0, Number(parameters.climbSpeed || 0));
    player.grounded = false;
    clearGroundContact(player);
    if (!boxesOverlap(colliderBox(player), ladderBox)) exitLadder(player, "boundary", false);
    return true;
  }

  function keyToken(keyId) {
    return `key:${String(keyId || "")}`;
  }

  function collectedPickupCount() {
    let count = 0;
    for (const id of collectedIds) if (!String(id).startsWith("key:")) count += 1;
    return count;
  }
  ...
  function boxDistance(first, second) {
    ...
    const dz = Math.max(first.zMin - second.zMax, second.zMin - first.zMax, 0);
    return Math.hypot(dx, dy, dz);
  }

  function openInteractableDoor(door, cause) {
    if (!door || door.runtimeState === "open") return false;
    setRuntimeObjectChanges(door, { runtimeState: "open", open: true, solid: false, colliderEnabled: false });
    emit("door.opened", { mapId: activeMap.id, objectId: door.id, instanceId: door.interactable?.instanceId ?? null, cause });
    return true;
  }

  function closeInteractableDoor(door, cause) {
    if (!door || door.runtimeState !== "open") return false;
    setRuntimeObjectChanges(door, { runtimeState: "closed", open: false, solid: Boolean(door.authoredSolid), colliderEnabled: door.authoredColliderEnabled !== false });
    emit("door.closed", { mapId: activeMap.id, objectId: door.id, instanceId: door.interactable?.instanceId ?? null, cause });
    return true;
  }

  function tryInteractWithKeyDoor(player, freshInteraction) {
    if (!freshInteraction) return false;
    const playerBox = colliderBox(player);
    const doors = objects.filter((object) => object.kind === "door" && object?.interactable?.templateId === "key-door" && object?.interactable?.role === "door" && object.runtimeState !== "open");
    doors.sort((first, second) => compareStableIds(first.id, second.id));
    const door = doors.find((candidate) => boxDistance(playerBox, colliderBoxIgnoringEnabled(candidate)) <= Math.max(0, Number(interactableParameters(candidate).interactionRadius || 0)));
    if (!door) return false;
    const keyId = interactableParameters(door).keyId;
    if (!collectedIds.has(keyToken(keyId))) {
      emit("door.locked", { mapId: activeMap.id, objectId: door.id, instanceId: door.interactable.instanceId, keyId });
      return true;
    }
    openInteractableDoor(door, "interact");
    return true;
  }

  function activateSpring(player, spring, dt) {
    const parameters = interactableParameters(spring);
    const impulseX = Number(parameters.impulseX || 0);
    const impulseY = Number(parameters.impulseY || 0);
    if (parameters.velocityMode === "add") {
      player.vx = Number(player.vx || 0) + impulseX;
      player.vy = Number(player.vy || 0) + impulseY;
    } else {
      player.vx = impulseX;
      player.vy = impulseY;
    }
    player.grounded = false;
    clearGroundContact(player);
    setRuntimeObjectChanges(spring, { cooldownTicks: Math.max(0, Number(parameters.cooldownTicks || 0)) });
    if ((activeMap.controlMode ?? project.controlMode) === "topdown") {
      const duration = Math.max(1, Number(parameters.topdownImpulseTicks || 1)) / 60;
      const solids = solidObjects(player);
      const previousHorizontalBox = colliderBox(player);
      player.x += player.vx * duration;
      resolveHorizontal(player, solids, previousHorizontalBox);
      const previousVerticalBox = colliderBox(player);
      player.y += player.vy * duration;
      resolveVerticalTopdown(player, solids, previousVerticalBox);
    }
    emit("spring.launched", { mapId: activeMap.id, objectId: spring.id, instanceId: spring.interactable.instanceId, impulseX, impulseY, velocityMode: parameters.velocityMode, dt });
  }

  function updateConveyorSupport(player, dt) {
    const previousId = player.conveyorObjectId;
    let conveyor = player.groundObjectId ? objects.find((object) => object.id === player.groundObjectId && object.kind === "conveyor") : null;
    if (previousId && previousId !== conveyor?.id) emit("conveyor.disengaged", { mapId: activeMap.id, objectId: previousId });
    if (conveyor && previousId !== conveyor.id) emit("conveyor.engaged", { mapId: activeMap.id, objectId: conveyor.id, instanceId: conveyor.interactable.instanceId });
    player.conveyorObjectId = conveyor?.id ?? null;
    if (!conveyor) return;
    const previousBox = colliderBox(player);
    player.x += Number(interactableParameters(conveyor).speed || 0) * dt;
    resolveHorizontal(player, solidObjects(player).filter((solid) => solid !== conveyor), previousBox);
    const playerBox = colliderBox(player);
    const conveyorBox = colliderBox(conveyor);
    const remainsSupported = Boolean(playerBox && conveyorBox && rangesOverlap(playerBox.x, playerBox.x + playerBox.width, conveyorBox.x, conveyorBox.x + conveyorBox.width));
    if (!remainsSupported) {
      player.grounded = false;
      player.groundObjectId = null;
      player.conveyorObjectId = null;
      emit("conveyor.disengaged", { mapId: activeMap.id, objectId: conveyor.id });
      conveyor = null;
    }
  }

  function updateCrumbleSupport(player) {
    if (!player.groundObjectId) return;
    const platform = objects.find((object) => object.id === player.groundObjectId && object.kind === "crumble-platform");
    if (!platform || (platform.runtimeState || "idle") !== "idle") return;
    const parameters = interactableParameters(platform);
    setRuntimeObjectChanges(platform, { runtimeState: "armed", interactableStateTicks: Math.max(1, Number(parameters.warningTicks || 1)) });
    emit("crumble.armed", { mapId: activeMap.id, objectId: platform.id, instanceId: platform.interactable.instanceId });
  }

  function updatePressurePlates(player, startPlayerBox) {
    const endPlayerBox = colliderBox(player);
    for (const plate of objects.filter((object) => object.kind === "pressure-plate" && object?.interactable?.role === "plate")) {
      const parameters = interactableParameters(plate);
      const plateBox = colliderBox(plate);
      const endTouching = boxesOverlap(endPlayerBox, plateBox);
      const sweptTouching = sweptBoxesOverlap(startPlayerBox, endPlayerBox, plateBox);
      const state = plate.runtimeState || "idle";
      const targetId = Array.isArray(parameters.targetObjectIds) ? parameters.targetObjectIds[0] : null;
      const gate = targetId ? objects.find((object) => object.id === targetId && object?.interactable?.instanceId === plate.interactable.instanceId) : null;
      if (state === "idle" && sweptTouching) {
        setRuntimeObjectChanges(plate, { runtimeState: parameters.latch === true ? "latched" : "pressed", active: true });
        emit("plate.pressed", { mapId: activeMap.id, objectId: plate.id, instanceId: plate.interactable.instanceId, targetObjectId: targetId });
        openInteractableDoor(gate, "pressure-plate");
      } else if (state === "pressed" && !endTouching) {
        setRuntimeObjectChanges(plate, { runtimeState: "released", active: false });
        emit("plate.released", { mapId: activeMap.id, objectId: plate.id, instanceId: plate.interactable.instanceId, targetObjectId: targetId });
      }
      if ((plate.runtimeState || "idle") === "released" && gate) {
        if (!boxesOverlap(colliderBoxIgnoringEnabled(player), colliderBoxIgnoringEnabled(gate))) {
          closeInteractableDoor(gate, "pressure-plate");
          setRuntimeObjectChanges(plate, { runtimeState: "idle", active: false });
        }
      }
    }
  }

  function updateSweptInteractableSensors(player, startPlayerBox, dt) {
    for (const key of objects.filter((object) => object.kind === "key" && !object.collected)) {
      if (!sweptOverlapsObject(startPlayerBox, player, key)) continue;
      const keyId = interactableParameters(key).keyId;
      key.collected = true;
      collectedIds.add(keyToken(keyId));
      gameplayRevision += 1;
      emit("key.collected", { mapId: activeMap.id, objectId: key.id, instanceId: key.interactable.instanceId, keyId });
    }
    for (const spring of objects.filter((object) => object.kind === "spring" && Number(object.cooldownTicks || 0) <= 0)) {
      if (sweptOverlapsObject(startPlayerBox, player, spring)) activateSpring(player, spring, dt);
    }
    updatePressurePlates(player, startPlayerBox);
    for (const door of objects.filter((object) => object.kind === "door" && object?.interactable?.templateId === "key-door" && object.runtimeState !== "open" && interactableParameters(object).autoOpen === true)) {
      const parameters = interactableParameters(door);
      if (collectedIds.has(keyToken(parameters.keyId)) && boxDistance(colliderBox(player), colliderBoxIgnoringEnabled(door)) <= Math.max(0, Number(parameters.interactionRadius || 0))) openInteractableDoor(door, "automatic");
    }
  }
  ...
  function update(rawDt) {
    ...
    const ladderJumpPressed = ["Space", "jump"].some((code) => pressedInputs.has(code));
    ...
    const tickDelta = dt > 0 ? Math.max(1, Math.round(dt * 60)) : 0;
    tickInteractableStates(player, tickDelta);
    const startPlayerBox = colliderBox(player);
    let interactionConsumed = false;
    const dropThrough = tryDropThroughPlatform(player, down, jumpPressed);
    const ladderWasActive = player.interactableMode === "ladder";
    const ladderEntered = !activeTraversal && !ladderWasActive && tryEnterLadder(player, freshInteraction, direction);
    if (ladderEntered || (ladderWasActive && freshInteraction)) interactionConsumed = true;

    if (activeTraversal && freshInteraction && !interactionConsumed) interactionConsumed = bailTraversal(player);
    if (!activeTraversal && player.interactableMode !== "ladder" && freshInteraction && !interactionConsumed) interactionConsumed = tryBeginTraversal(player, direction);

    if (activeTraversal) updateTraversal(player, dt);
    else if (ladderEntered) {
      // Entering is itself this tick's controller transition; climbing begins on the next deterministic sample.
    }
    else if (player.interactableMode === "ladder") updateLadder(player, dt, direction, freshInteraction, ladderJumpPressed);
    else if ((activeMap.controlMode ?? project.controlMode) === "topdown") updateTopdown(player, dt, direction);
    else updatePlatformer(player, dt, direction, jumping && !dropThrough, jumpPressed && !dropThrough);

    if ((activeMap.controlMode ?? project.controlMode) === "platformer") {
      updateConveyorSupport(player, dt);
      updateCrumbleSupport(player);
    }

    refreshWorldStreamWindow(player);
    updateSweptInteractableSensors(player, startPlayerBox, dt);
    if (freshInteraction && !interactionConsumed) interactionConsumed = tryInteractWithKeyDoor(player, true);
    ...
    for (const object of objects) {
      if (!object.hidden && object.kind === "coin" && !object.collected && overlaps(player, object)) {
        ...
        gameplayRevision += 1;
        emit("coin.collected", { mapId: activeMap.id, objectId: object.id, count: collectedPickupCount() });
      }
  ...
  function validateSaveState(input) {
    ...
    const stateFields = saveVersion === 7 ? [...baseStateFields, "motionBodyStates", "combatState", "actorStates", "elevationState", "worldStreamState", "runVariationState"] : saveVersion === 6 ? [...baseStateFields, "motionBodyStates", "combatState", "actorStates", "elevationState", "worldStreamState"] : saveVersion === 5 ? [...baseStateFields, "motionBodyStates", "combatState", "actorStates", "elevationState"] : saveVersion === 4 ? [...baseStateFields, "motionBodyStates", "combatState", "actorStates"] : saveVersion === 3 ? [...baseStateFields, "motionBodyStates", "combatState"] : saveVersion === 2 ? [...baseStateFields, "motionBodyStates"] : baseStateFields;
    if (!exactFields(input, stateFields, "save state")) return { valid: false, errors, state: null };
    if (![1, 2, 3, 4, 5, 6, 7].includes(saveVersion)) addError("Save state version must be 1, 2, 3, 4, 5, 6, or 7.");
    const expectedSaveSchema = saveVersion === 7 ? "looplab-runtime-save-state/v7" : saveVersion === 6 ? "looplab-runtime-save-state/v6" : saveVersion === 5 ? "looplab-runtime-save-state/v5" : saveVersion === 4 ? "looplab-runtime-save-state/v4" : saveVersion === 3 ? "looplab-runtime-save-state/v3" : saveVersion === 2 ? "looplab-runtime-save-state/v2" : "looplab-runtime-save-state/v1";
    ...
    for (const map of maps) {
      for (const object of map.objects || []) {
        validCollectedIds.add(runtimeKey(map.id, object.id));
        if (object?.kind === "key" && object?.interactable?.templateId === "key-door" && object?.interactable?.role === "key") {
          const logicalKeyId = object.interactable?.parameters?.keyId;
          if (typeof logicalKeyId === "string" && logicalKeyId) validCollectedIds.add(keyToken(logicalKeyId));
        }
      }
    }
    ...
    if (saveVersion >= 6 && input.worldStreamState !== null) {
    ...
    let normalizedRunVariationState = null;
    if (saveVersion === 7) {
      const fields = ["schemaVersion", "enabled", "mode", "seed", "utcDay", "programDigest", "selectionProgramDigest", "selectionAlgorithm", "selectionDigest", "selections", "assignments", "simulationTick"];
      if (exactFields(input.runVariationState, fields, "runVariationState")) {
        let expectedRun = null;
        try { expectedRun = resolveRunVariation(runVariationProgram, input.runVariationState.mode === "daily" ? { mode: "daily", utcDay: input.runVariationState.utcDay } : { mode: "standard", seed: input.runVariationState.seed }); }
        catch (error) { addError(error instanceof Error ? error.message : String(error)); }
        if (expectedRun) {
          for (const field of ["schemaVersion", "enabled", "mode", "seed", "utcDay", "programDigest", "selectionProgramDigest", "selectionAlgorithm", "selectionDigest"]) if (input.runVariationState[field] !== expectedRun[field]) addError(`runVariationState.${field} does not match current deterministic source.`);
          if (JSON.stringify(input.runVariationState.selections) !== JSON.stringify(expectedRun.selections)) addError("runVariationState.selections do not match current deterministic source.");
          if (JSON.stringify(input.runVariationState.assignments) !== JSON.stringify(expectedRun.assignments)) addError("runVariationState.assignments do not match current deterministic source.");
        }
        if (!Number.isSafeInteger(input.runVariationState.simulationTick) || input.runVariationState.simulationTick < 0 || input.runVariationState.simulationTick > 100_000_000) addError("runVariationState.simulationTick must be a bounded non-negative safe integer.");
        normalizedRunVariationState = expectedRun ? { ...clone(expectedRun), simulationTick: Number(input.runVariationState.simulationTick) } : null;
      }
    }

    const state = errors.length ? null : {
      schemaVersion: saveVersion === 7 ? "looplab-runtime-save-state/v7" : saveVersion === 6 ? "looplab-runtime-save-state/v6" : saveVersion === 5 ? "looplab-runtime-save-state/v5" : saveVersion === 4 ? "looplab-runtime-save-state/v4" : saveVersion === 3 ? "looplab-runtime-save-state/v3" : saveVersion === 2 ? "looplab-runtime-save-state/v2" : "looplab-runtime-save-state/v1",
      version: saveVersion,
      ...
      ...(saveVersion === 7 ? { runVariationState: normalizedRunVariationState } : {}),
  ...
  function exportSaveState() {
    ...
    const hasRunVariation = project.runVariationProgram != null && runVariationProgram.enabled === true;
    const saveVersion = hasRunVariation ? 7 : hasWorldStream ? 6 : hasActiveElevation ? 5 : hasActorState ? 4 : hasCombatState ? 3 : savedMotionBodyStates.length ? 2 : 1;
    return {
      schemaVersion: saveVersion === 7 ? "looplab-runtime-save-state/v7" : saveVersion === 6 ? "looplab-runtime-save-state/v6" : saveVersion === 5 ? "looplab-runtime-save-state/v5" : saveVersion === 4 ? "looplab-runtime-save-state/v4" : saveVersion === 3 ? "looplab-runtime-save-state/v3" : saveVersion === 2 ? "looplab-runtime-save-state/v2" : "looplab-runtime-save-state/v1",
      version: saveVersion,
      ...
      ...(saveVersion >= 6 ? { worldStreamState: hasWorldStream ? { schemaVersion: "looplab-world-stream-save-state/v1", hostMapId: activeMap.id, routeDigest: activeWorldStreamState.routeDigest, currentOrdinal: Number(activeWorldStreamState.currentOrdinal || 0), currentInstanceId: activeWorldStreamState.currentInstanceId } : null } : {}),
      ...(saveVersion === 7 ? { runVariationState: getRunVariationState() } : {}),
      activeChoicePageId,
      pendingChoiceId,
      won,
    };
  ...
  function applyValidatedSaveState(state, announce = true) {
    clearInputState();
    if (state.runVariationState) {
      runVariationState = clone(state.runVariationState);
      delete runVariationState.simulationTick;
      simulationTick = Number(state.runVariationState.simulationTick || 0);
    } else {
      runVariationState = resolveRunVariation(runVariationProgram, { mode: "standard", seed: runVariationProgram.defaultSeed });
      simulationTick = 0;
    }
    collectedIds.clear();
  ...
  function getState() {
    ...
    return {
      ...
      collectedCount: collectedPickupCount(),
      activeTraversalPathId: activeTraversal?.pathId ?? null,
      activeChoicePageId,
      gameplayRevision,
      simulationTick,
      runVariation: getRunVariationState(),
      ...
      player: player ? {
        ...
        groundObjectId: player.groundObjectId ?? null,
        ...
        interactableMode: player.interactableMode ?? "default",
        interactableTargetId: player.interactableTargetId ?? null,
        conveyorObjectId: player.conveyorObjectId ?? null,
      } : null,
      won,
    };
  }
