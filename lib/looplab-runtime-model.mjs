export function createRuntimeModel(project) {
  ...
  function loadMap(id, spawnId) {
    ...
    objects = (target.objects || []).map((object) => ({
      ...
      elevationProgress: 0,
      elevationSupportZ: Number(object.supportZ ?? object.z ?? 0),
      jumpHeld: false,
      coyoteTicksRemaining: 0,
      jumpBufferTicksRemaining: 0,
      collected: collectedIds.has(target.id + ":" + object.id),
    }));
    activePaths = clone(target.traversalPaths || []);
    activeCollisionGeometry = target.collisionGeometry && typeof target.collisionGeometry === "object" ? clone(target.collisionGeometry) : null;
    ...
    activeTileRuntime = compileTileRuntimeProgram(target.tileProgram, target);
  ...
  function update(rawDt) {
    ...
    const width = Number(activeMap.width ?? project.width);
    const height = Number(activeMap.height ?? project.height);
    player.x = Math.max(0, Math.min(width - player.width, player.x));
    if (player.y > height + 80) respawn(player, null, { cause: "fall" });
    player.y = Math.max(-80, Math.min(height - player.height, player.y));
  ...
  function validateSaveState(input) {
    ...
    const stateFields = saveVersion === 5 ? [...baseStateFields, "motionBodyStates", "combatState", "actorStates", "elevationState"] : saveVersion === 4 ? [...baseStateFields, "motionBodyStates", "combatState", "actorStates"] : saveVersion === 3 ? [...baseStateFields, "motionBodyStates", "combatState"] : saveVersion === 2 ? [...baseStateFields, "motionBodyStates"] : baseStateFields;
    if (!exactFields(input, stateFields, "save state")) return { valid: false, errors, state: null };
    if (![1, 2, 3, 4, 5].includes(saveVersion)) addError("Save state version must be 1, 2, 3, 4, or 5.");
    const expectedSaveSchema = saveVersion === 5 ? "looplab-runtime-save-state/v5" : saveVersion === 4 ? "looplab-runtime-save-state/v4" : saveVersion === 3 ? "looplab-runtime-save-state/v3" : saveVersion === 2 ? "looplab-runtime-save-state/v2" : "looplab-runtime-save-state/v1";
    ...
    for (const id of normalizedCollectedIds) if (!validCollectedIds.has(id)) addError(`collectedIds references missing object ${id}.`);
    ...
    if (targetPlayer) {
      if (!exactFields(input.player, ["id", "x", "y", "z"], "player")) normalizedPlayer = null;
        ...
        const width = Number(targetMap.width ?? project.width ?? 0);
        const height = Number(targetMap.height ?? project.height ?? 0);
        if (input.player.x < -80 || input.player.x > width + 80 || input.player.y < -80 || input.player.y > height + 80 || Math.abs(input.player.z) > 1_000_000) addError("Saved player position is outside the bounded active map envelope.");
    ...
    const state = errors.length ? null : {
      schemaVersion: saveVersion === 5 ? "looplab-runtime-save-state/v5" : saveVersion === 4 ? "looplab-runtime-save-state/v4" : saveVersion === 3 ? "looplab-runtime-save-state/v3" : saveVersion === 2 ? "looplab-runtime-save-state/v2" : "looplab-runtime-save-state/v1",
      version: saveVersion,
  ...
  function exportSaveState() {
    ...
    const saveVersion = hasActiveElevation ? 5 : hasActorState ? 4 : hasCombatState ? 3 : savedMotionBodyStates.length ? 2 : 1;
    return {
      schemaVersion: saveVersion === 5 ? "looplab-runtime-save-state/v5" : saveVersion === 4 ? "looplab-runtime-save-state/v4" : saveVersion === 3 ? "looplab-runtime-save-state/v3" : saveVersion === 2 ? "looplab-runtime-save-state/v2" : "looplab-runtime-save-state/v1",
      version: saveVersion,
      ...
      ...(saveVersion === 5 ? { elevationState: { transitionId: player.elevationTransitionId, segmentId: player.elevationSegmentId ?? null, progress: Number(player.elevationProgress || 0), supportZ: Number(player.elevationSupportZ ?? player.z ?? 0) } } : {}),
