...
export function createAiArtProviderRequest(request, apiKey) {
  ...
  assertProviderPayloadPrivacy({
    payload: request.providerPayload,
    references: (request.referenceImages ?? []).map((reference) => ({ filename: reference.filename, mimeType: reference.mimeType })),
  }, { label: "AI-art provider payload" });
