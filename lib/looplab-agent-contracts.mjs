...
export const LOOPLAB_MCP_SERVER_VERSION = "1.5.0";
...
const worldStreamSocketInput = {
  type: "object",
  properties: {
    id: combatStableId,
    tag: string("Exact authored route compatibility tag.", { minLength: 1, maxLength: 128 }),
    edge: enumeration(["left", "right", "top", "bottom"], "Exact map boundary owned by this socket."),
    x: number("Authored map-space socket x.", { minimum: -1048576, maximum: 1048576 }),
    y: number("Authored map-space socket y.", { minimum: -1048576, maximum: 1048576 }),
    z: number("Independent authored support elevation.", { minimum: -1048576, maximum: 1048576 }),
    span: number("Readable authored crossing span.", { exclusiveMinimum: 0, maximum: 65536 }),
  },
  required: ["id", "tag", "edge", "x", "y", "z", "span"],
  additionalProperties: false,
};
const worldStreamInput = {
  type: "object",
  description: "Strict map-owned deterministic continuous-world route. It composes embedded 2D chunks by exact authored sockets and never infers collision or adjacency from pixels.",
  properties: {
    schemaVersion: { const: "looplab-world-stream/v1" },
    owner: { const: "authored-map" },
    enabled: boolean("Whether runtime composition is active."),
    mode: enumeration(["finite", "seeded"], "Exact finite sequence or deterministic seeded weighted route."),
    axis: enumeration(["horizontal", "vertical"], "Version-1 route composition axis."),
    seed: string("Stable deterministic route seed.", { minLength: 1, maxLength: 128 }),
    startTemplateId: combatStableId,
    horizon: integer("Maximum generated chunk count.", { minimum: 2, maximum: 4096 }),
    sequence: array(combatStableId, "Finite ordered template IDs; empty for seeded mode.", { maxItems: 4096 }),
    budgets: {
      type: "object",
      properties: {
        retainBehind: integer("Resident chunks retained behind the player.", { minimum: 0, maximum: 15 }),
        prefetchAhead: integer("Resident chunks prepared ahead of the player.", { minimum: 0, maximum: 15 }),
        maxResidentChunks: integer("Hard resident chunk limit.", { minimum: 1, maximum: 16 }),
        maxResidentTileCells: integer("Hard resident rendered-cell limit.", { minimum: 1, maximum: 262144 }),
        maxResidentCollisionCells: integer("Hard resident collision-cell limit.", { minimum: 1, maximum: 131072 }),
        maxDecodedRgbaBytes: integer("Hard unique decoded RGBA memory limit.", { minimum: 1, maximum: 1073741824 }),
        cullPadding: number("Camera culling padding in world units.", { minimum: 0, maximum: 4096 }),
      },
      required: ["retainBehind", "prefetchAhead", "maxResidentChunks", "maxResidentTileCells", "maxResidentCollisionCells", "maxDecodedRgbaBytes", "cullPadding"],
      additionalProperties: false,
    },
    tolerances: {
      type: "object",
      properties: {
        crossAxis: number("Maximum socket cross-axis difference.", { minimum: 0, maximum: 64 }),
        z: number("Maximum socket support-elevation difference.", { minimum: 0, maximum: 64 }),
        span: number("Maximum socket-span difference.", { minimum: 0, maximum: 64 }),
      },
      required: ["crossAxis", "z", "span"],
      additionalProperties: false,
    },
    templates: array({
      type: "object",
      properties: {
        id: combatStableId,
        name: string("Human-readable authored chunk name.", { minLength: 1, maxLength: 240 }),
        mapId: combatStableId,
        weight: number("Deterministic seeded-selection weight.", { exclusiveMinimum: 0, maximum: 1000000 }),
        entry: { anyOf: [worldStreamSocketInput, { type: "null" }] },
        exit: { anyOf: [worldStreamSocketInput, { type: "null" }] },
      },
      required: ["id", "name", "mapId", "weight", "entry", "exit"],
      additionalProperties: false,
    }, "Stable authored chunk templates.", { minItems: 2, maxItems: 128 }),
  },
  required: ["schemaVersion", "owner", "enabled", "mode", "axis", "seed", "startTemplateId", "horizon", "sequence", "budgets", "tolerances", "templates"],
  additionalProperties: false,
};
...
define(["list_interactable_templates"], schema({
  query: string("Bounded deterministic filter over template IDs, labels, summaries, roles, kinds, and emitted events.", { maxLength: 240 }),
  controlMode: enumeration(["platformer", "topdown"], "Optional exact compatible map controller."),
  limit: integer("Maximum compact template summaries.", { minimum: 1, maximum: 50 }),
  offset: integer("Zero-based deterministic result offset.", { minimum: 0, maximum: 1024 }),
}));
define(["get_interactable_template"], schema({
  templateId: string("Exact native interactable-template ID.", { pattern: "^[a-z0-9][a-z0-9-]*$", maxLength: 120 }),
}, ["templateId"]));
define(["get_interactable_report"], schema({
  requireEvidence: boolean("Also report instances missing their calibrated acceptance/replay evidence pair."),
}));
const interactablePlacementInput = {
  templateId: string("Exact native interactable-template ID.", { pattern: "^[a-z0-9][a-z0-9-]*$", maxLength: 120 }),
  instanceId: string("Stable authored instance ID used by every materialized object and fixture template.", { pattern: "^[A-Za-z0-9][A-Za-z0-9._:-]*$", maxLength: 120 }),
  mapId: string("Exact target map ID; defaults to the active map.", { minLength: 1, maxLength: 160 }),
  x: number("Authored ground-contact origin x.", { minimum: -1000000, maximum: 1000000 }),
  y: number("Authored ground-contact origin y.", { minimum: -1000000, maximum: 1000000 }),
  z: number("Authored support height.", { minimum: -1000000, maximum: 1000000 }),
  parameters: object("Strict bounded explicit overrides. Omitted fields inherit the versioned native template; unknown fields are rejected."),
};
define(["preview_interactable_template"], schema(interactablePlacementInput, ["templateId", "instanceId", "x", "y"]));
define(["apply_interactable_template"], schema({
  ...interactablePlacementInput,
  templateDigest: string("Exact installed template digest returned by preview_interactable_template.", { pattern: "^sha256:[a-f0-9]{64}$" }),
  previewDigest: string("Exact source-bound materialization digest returned by preview_interactable_template.", { pattern: "^sha256:[a-f0-9]{64}$" }),
}, ["templateId", "instanceId", "x", "y", "expectedSourceDigest", "templateDigest", "previewDigest"]));
define(["remove_interactable_instance"], schema({
  mapId: string("Exact target map ID; defaults to the active map.", { minLength: 1, maxLength: 160 }),
  instanceId: string("Exact interactable instance ID; every object in that instance is removed together.", { pattern: "^[A-Za-z0-9][A-Za-z0-9._:-]*$", maxLength: 120 }),
}, ["instanceId"]));
define(["get_agent_guide_index"], schema({
  query: string("Deterministic bounded text matched against section titles, invariant summaries, lifecycle steps, failure signals, recoveries, anchors, and referenced commands.", { maxLength: 240 }),
  category: enumeration(["all", "section", "invariant", "lifecycle", "recovery"], "Optional exact entry-kind filter; defaults to all."),
  limit: integer("Maximum compact navigation entries.", { minimum: 1, maximum: 50 }),
}));
...
define(["get_run_variation_report"], schema({ profile: enumeration(["prototype", "production"], "Project Doctor severity profile.") }));
define(["suggest_run_variation_program"], schema({
  seedNamespace: string("Stable bounded game-specific seed namespace.", { minLength: 1, maxLength: 96 }),
  defaultSeed: string("Default explicit run seed.", { minLength: 1, maxLength: 128 }),
  dailyEnabled: boolean("Prepare the editable daily-challenge starter."),
}));
define(["preview_run_variation"], schema({
  mode: enumeration(["standard", "daily"], "Resolve an explicit seed or UTC daily identity without mutating source."),
  seed: string("Explicit standard-run seed.", { minLength: 1, maxLength: 128 }),
  utcDay: string("Explicit real UTC day in YYYY-MM-DD form.", { pattern: "^\\d{4}-\\d{2}-\\d{2}$" }),
  profile: enumeration(["prototype", "production"], "Project Doctor severity profile."),
}));
define(["set_run_variation_program"], schema({
  program: object("Strict renderer-neutral run-variation program."),
  profile: enumeration(["prototype", "production"], "Project Doctor severity profile."),
}, ["program"]));
define(["preview_replay_ghost"], schema({
  replayCaseId: string("Exact passing deterministic replay case ID.", { minLength: 1, maxLength: 160 }),
  id: string("Stable ghost ID.", { pattern: "^[A-Za-z0-9][A-Za-z0-9._:-]*$", maxLength: 160 }),
  label: string("Player-facing ghost label.", { minLength: 1, maxLength: 160 }),
  sampleEveryTicks: integer("Trajectory sample interval.", { minimum: 1, maximum: 60 }),
  color: string("Opaque six-digit ghost color.", { pattern: "^#[0-9a-fA-F]{6}$" }),
  opacity: number("Presentation-only ghost opacity.", { minimum: 0.1, maximum: 0.9 }),
  expectedSourceDigest: string("Optional current Project Doctor source digest; mismatch fails closed.", { pattern: "^source-[a-f0-9]{64}$" }),
}, ["replayCaseId"]));
define(["apply_replay_ghost"], schema({
  replayCaseId: string("Exact passing deterministic replay case ID.", { minLength: 1, maxLength: 160 }),
  id: string("Stable ghost ID.", { pattern: "^[A-Za-z0-9][A-Za-z0-9._:-]*$", maxLength: 160 }),
  label: string("Player-facing ghost label.", { minLength: 1, maxLength: 160 }),
  sampleEveryTicks: integer("Trajectory sample interval.", { minimum: 1, maximum: 60 }),
  ...
  opacity: number("Presentation-only ghost opacity.", { minimum: 0.1, maximum: 0.9 }),
  profile: enumeration(["prototype", "production"], "Project Doctor severity profile."),
  expectedSourceDigest: string("Exact Project Doctor source digest used for preview.", { pattern: "^source-[a-f0-9]{64}$" }),
  expectedPreviewDigest: string("Exact replay-ghost preview digest.", { pattern: "^sha256:[a-f0-9]{64}$" }),
}, ["replayCaseId", "expectedSourceDigest", "expectedPreviewDigest"]));
...
const communityExchangeDependency = schema({
  sourceName: string("Exact relative dependency name referenced by the parent document.", { minLength: 1, maxLength: 512 }),
  sourceText: string("Complete explicitly supplied TSX or Tiled JSON dependency text; no path or URL is resolved implicitly.", { minLength: 1, maxLength: 4_000_000 }),
  format: enumeration(["tsx", "json"], "Dependency format."),
}, ["sourceName", "sourceText"]);
const communityExchangeAssetBinding = schema({
  sourceName: string("Exact Tiled tileset source, name, or image-source key.", { minLength: 1, maxLength: 512 }),
  assetId: combatStableId,
}, ["sourceName", "assetId"]);
const asepriteAnimationTarget = schema({
  type: enumeration(["object-id", "object-kind"], "Existing renderer-neutral presentation target type."),
  id: combatStableId,
}, ["type", "id"]);
define(["list_community_exchanges", "get_community_exchange_report"], schema({ kind: enumeration(["tiled", "aseprite"], "Optional exchange-kind filter.") }));
const tiledCommunityExchangeInput = {
  sourceName: string("Display filename for the exact retained Tiled document.", { minLength: 1, maxLength: 512 }),
  sourceText: string("Complete Tiled JSON or TMX source text.", { minLength: 1, maxLength: 4_000_000 }),
  format: enumeration(["auto", "tiled-json", "tmx"], "Tiled document format; auto uses the filename and first non-space character."),
  mapId: combatStableId,
  exchangeId: combatStableId,
  dependencies: array(communityExchangeDependency, "Explicit external TSX or Tiled JSON dependencies; LoopLab never reads a path or URL.", { maxItems: 32 }),
  assetBindings: array(communityExchangeAssetBinding, "Exact tileset-to-embedded-asset bindings.", { maxItems: 64 }),
  replaceExisting: boolean("Permit replacement of existing visual tile layers after preview."),
  resizeMap: boolean("Permit the exact imported logical map dimensions after preview."),
  allowProjectionChange: boolean("Permit an orthographic/dimetric projection change after preview."),
  expectedPreviewDigest: string("Exact preview digest returned by preview_tiled_import.", { pattern: "^sha256:[a-f0-9]{64}$" }),
};
define(["preview_tiled_import"], schema(tiledCommunityExchangeInput, ["sourceText", "mapId", "assetBindings"]));
define(["apply_tiled_import"], schema(tiledCommunityExchangeInput, ["sourceText", "mapId", "assetBindings", "expectedPreviewDigest"]));
const asepriteCommunityExchangeInput = {
  sourceName: string("Display filename for the exact retained Aseprite JSON document.", { minLength: 1, maxLength: 512 }),
  sourceText: string("Complete Aseprite JSON array/hash atlas metadata.", { minLength: 1, maxLength: 4_000_000 }),
  assetId: combatStableId,
  exchangeId: combatStableId,
  machineId: combatStableId,
  target: asepriteAnimationTarget,
  replaceExisting: boolean("Permit replacement of an existing presentation animation machine after preview."),
  allowTimingApproximation: boolean("Permit explicit mean-FPS projection when Aseprite frames use different durations; exact durations remain in retained source."),
  expectedPreviewDigest: string("Exact preview digest returned by preview_aseprite_import.", { pattern: "^sha256:[a-f0-9]{64}$" }),
};
define(["preview_aseprite_import"], schema(asepriteCommunityExchangeInput, ["sourceText", "assetId", "target"]));
define(["apply_aseprite_import"], schema(asepriteCommunityExchangeInput, ["sourceText", "assetId", "target", "expectedPreviewDigest"]));
define(["export_community_exchange"], schema({
  exchangeId: combatStableId,
  allowStaleOriginal: boolean("Return the exact originally imported bytes even though canonical LoopLab source changed, clearly labelled stale."),
}, ["exchangeId"]));
define(["get_world_stream", "get_world_stream_report"], schema({ mapId: combatStableId, strict: boolean("Include release-facing seam-evidence warnings.") }));
define(["get_world_stream_plan"], schema({ mapId: combatStableId, count: integer("Bounded seeded planning horizon override.", { minimum: 1, maximum: 4096 }) }));
define(["suggest_world_stream"], schema({
  mapId: combatStableId,
  mode: enumeration(["finite", "seeded"], "Suggested route mode."),
  axis: enumeration(["horizontal", "vertical"], "Suggested composition axis."),
  seed: string("Stable deterministic seed.", { minLength: 1, maxLength: 128 }),
  horizon: integer("Maximum planned chunk count.", { minimum: 2, maximum: 4096 }),
  tag: string("Exact compatibility tag assigned to suggested sockets.", { minLength: 1, maxLength: 128 }),
  z: number("Exact support elevation assigned to suggested sockets.", { minimum: -1048576, maximum: 1048576 }),
  span: number("Suggested readable socket span.", { exclusiveMinimum: 0, maximum: 65536 }),
  budgets: object("Optional bounded resident budgets; normalized and inspected before storage."),
  tolerances: object("Optional socket tolerances; normalized and inspected before storage."),
}));
define(["set_world_stream"], schema({ mapId: combatStableId, program: worldStreamInput, strict: boolean("Return release-facing seam-evidence warnings after storage.") }, ["program"]));
define(["remove_world_stream"], schema({ mapId: combatStableId }));
...
define(["list_capability_packs"], schema({
  query: string("Bounded text search across pack labels, categories, decisions, and capability ownership.", { maxLength: 240 }),
  category: string("Exact lowercase pack category.", { pattern: "^[a-z0-9][a-z0-9-]*$", maxLength: 80 }),
  limit: integer("Maximum pack summaries.", { minimum: 1, maximum: 40 }),
  offset: integer("Deterministic result offset.", { minimum: 0, maximum: 10000 }),
}));
define(["get_capability_pack"], schema({ packId: string("Exact built-in capability-pack ID.", { pattern: "^[a-z0-9][a-z0-9-]*$", maxLength: 120 }) }, ["packId"]));
define(["query_capability_knowledge"], schema({
  query: string("Bounded intent or implementation question.", { maxLength: 600 }),
  packIds: array(string("Exact capability-pack ID.", { pattern: "^[a-z0-9][a-z0-9-]*$", maxLength: 120 }), "Optional exact pack scope.", { maxItems: 16, uniqueItems: true }),
  capabilityIds: array(string("Exact native capability ID.", { pattern: "^[a-z0-9][a-z0-9-]*$", maxLength: 120 }), "Optional exact capability scope.", { maxItems: 32, uniqueItems: true }),
  limit: integer("Maximum bounded knowledge records.", { minimum: 1, maximum: 40 }),
}));
define(["inspect_capability_pack_refresh"], schema({
  candidate: object("Complete declarative looplab-capability-pack/v1 candidate. The operation validates and compares it but never installs it."),
}, ["candidate"]));
...
define(["preview_start_run"], schema({ seed: string("Explicit run seed applied before tick zero.", { minLength: 1, maxLength: 128 }) }, ["seed"]));
define(["preview_start_daily_challenge"], schema({ utcDay: string("Explicit real UTC day applied before tick zero.", { pattern: "^\\d{4}-\\d{2}-\\d{2}$" }) }, ["utcDay"]));
define(["get_preview_ghost_states"], noArgs());
...
const CORE_MUTATING = new Set([
  ...
  "set_gameplay_program", "remove_gameplay_program", "set_run_variation_program", "remove_run_variation_program", "apply_replay_ghost", "set_combat_program", "remove_combat_program", "set_actor_program", "remove_actor_program", "set_collision_geometry", "remove_collision_geometry", "set_tile_program", "remove_tile_program", "apply_tile_patch", "apply_tiled_import", "apply_aseprite_import", "set_world_stream", "remove_world_stream", "set_narrative_contract", "remove_narrative_contract", "set_visual_identity", "remove_visual_identity", "set_presentation_program", "remove_presentation_program", "set_game_shell", "remove_game_shell", "set_tuning_contract", "remove_tuning_contract", "set_structural_scaffold_contract", "remove_structural_scaffold_contract", "set_spatial_layout_contract", "remove_spatial_layout_contract", "set_verb_architecture", "remove_verb_architecture", "queue_agent_request",
  "apply_interactable_template", "remove_interactable_instance",
...
const BROWSER_MUTATING = new Set([
  ...
  "preview_resume", "preview_step", "preview_reset", "preview_start_run", "preview_start_daily_challenge", "preview_load_map", "select_object",
]);
...
const SPECIAL_DESCRIPTIONS = Object.freeze({
  ...
  get_agent_guide_index: "Search the generated source-bound AI Agent Guide navigation layer. It returns exact canonical anchors, line ranges, invariants, lifecycle steps, and failure recoveries for orientation only; it cannot execute, mutate, verify, or grant authority.",
  list_capability_packs: "List compact built-in capability packs with content digests, provenance, license declarations, calibration coverage, and exact native capability IDs. Packs are orientation-only and never load or execute code.",
  get_capability_pack: "Read one complete deterministic capability pack, including decision knowledge, authority boundaries, source metadata, and router calibration evidence.",
  query_capability_knowledge: "Search bounded native capability decision knowledge without asking a provider or loading an external skill at runtime. Results remain advisory and cannot mutate or verify a game.",
  inspect_capability_pack_refresh: "Compare one complete candidate pack against the installed revision. Reject rollback and same-revision equivocation; validate provenance, license metadata, authority, and calibration without downloading or installing anything.",
  ...
  list_interactable_templates: "Search LoopLab's immutable native interactable vocabulary. Results expose controller compatibility, object roles, emitted events, revisions, and content digests without creating gameplay geometry.",
  get_interactable_template: "Read one complete versioned interactable template, including bounded parameters, exact object roles, authority policy, feature-contract starter, and acceptance/replay fixture starters.",
  get_interactable_report: "Inspect every authored interactable instance for exact template revision, content digest, parameter inheritance, role completeness, authored collision ownership, references, and optional evidence coverage.",
  preview_interactable_template: "Materialize one bounded native interactable bundle against an exact project source without mutation, returning complete objects, colliders, explicit overrides, fixture starters, and a canonical preview digest.",
  apply_interactable_template: "Apply only the exact source-, template-, and preview-digested interactable bundle previously reviewed; duplicate instance and object IDs are rejected atomically.",
  remove_interactable_instance: "Remove every authored object belonging to one exact interactable instance so multi-object mechanics cannot be left partially broken.",
  ...
  list_community_exchanges: "List compact retained Tiled and Aseprite source envelopes, target freshness, exact-source hashes, and byte-identical export availability without returning source bodies.",
  get_community_exchange_report: "Inspect authoring-only Tiled/Aseprite exchange integrity, canonical target freshness, collision-ownership boundaries, and exact-source export status.",
  preview_tiled_import: "Parse bounded Tiled JSON or TMX plus explicitly supplied TSX dependencies, project visual tile layers on a clone, preserve existing authored collision, and return exact source/preview/Doctor receipts without mutation.",
  apply_tiled_import: "Apply only the exact source- and preview-digest Tiled projection after schema, tile-program, and both-profile Project Doctor non-regression gates; object-layer collision is never imported.",
  preview_aseprite_import: "Parse bounded Aseprite JSON array/hash metadata and propose a uniform-grid asset plus renderer-neutral animation machine; trimmed, rotated, ambiguous, or unreviewed timing data is never guessed.",
  apply_aseprite_import: "Apply only the exact source- and preview-digest Aseprite asset/animation projection after schema and both-profile Project Doctor non-regression gates.",
  export_community_exchange: "Return byte-identical originally imported source only when its retained SHA-256 is valid; stale originals require explicit opt-in and are never labelled as current semantics.",
  get_world_stream: "Inspect one map-owned deterministic continuous-world source, exact digest, bounded plan, resident budgets, and explicit judgment boundaries.",
  get_world_stream_report: "Validate exact chunk sockets, deterministic route composition, v1 system restrictions, unique decoded-memory cost, and release-facing seam-evidence requirements.",
  get_world_stream_plan: "Materialize a bounded deterministic finite or seeded chunk plan without mutating source, fetching assets, or claiming that geometry is visually seamless.",
  suggest_world_stream: "Prepare an editable provider-free route from compatible authored maps. It assigns explicit edge sockets but never infers joins, collision, pacing, or visual continuity from pixels.",
  set_world_stream: "Store one strict map-owned deterministic chunk route after exact schema, map, socket, system, and budget validation.",
  remove_world_stream: "Remove continuous chunk composition from one host map without deleting any source maps, tiles, objects, or assets.",
  ...
  get_run_variation_program: "Read the exact canonical authored pools, daily-challenge policy, replay ghosts, and two-seed evidence links without resolving a hidden random value.",
  get_run_variation_report: "Validate typed variation assignments, stable SHA-256 seed resolution, reset policy, daily UTC semantics, ghost freshness, budgets, and two-seed evidence without claiming balance or fun.",
  suggest_run_variation_program: "Prepare one provider-free editable two-variant starter from declared gameplay variables. It never selects or applies creative values automatically.",
  preview_run_variation: "Resolve one explicit seed or UTC day against the exact saved authored pools without mutating source, starting a runtime, reading wall-clock time, or claiming balance.",
  set_run_variation_program: "Store one strict renderer-neutral run-variation program shared by preview, replay, saves, Canvas/Phaser/Pixi/melon export, UI, CLI, MCP, Codex, and Claude.",
  remove_run_variation_program: "Remove authored variation, daily challenges, and presentation ghosts without changing gameplay variables, replay fixtures, maps, collision, or art.",
  preview_replay_ghost: "Run one exact passing replay and derive a bounded presentation-only trajectory with source and preview digests; no project mutation occurs.",
  apply_replay_ghost: "Apply only the exact current-source replay trajectory reviewed by preview. Ghosts remain outside objects, collision, gameplay, saves, and replay hashes.",
  preview_start_run: "Reset the active builder preview into one explicit saved run seed before tick zero; source is not mutated and the preview must already be active.",
  preview_start_daily_challenge: "Reset the active builder preview into one explicit saved UTC daily identity before tick zero; the simulation never reads wall-clock time.",
  get_preview_ghost_states: "Read presentation-only replay ghost poses from the active builder preview without adding them to objects, collision, saves, completion, or replay truth.",
  ...
  return LOOPLAB_AGENT_COMMANDS.map((op) => {
    ...
    return {
      ...
      annotations: {
        ...
        destructiveHint: mutatesState && !LEDGER_MUTATING.has(op) && (DESTRUCTIVE.test(op) || op.startsWith("update_") || op.startsWith("set_") || op === "promote_iteration" || op === "apply_previewed_batch" || op === "apply_tiled_import" || op === "apply_aseprite_import" || op === "auto_repair" || op === "converge"),
