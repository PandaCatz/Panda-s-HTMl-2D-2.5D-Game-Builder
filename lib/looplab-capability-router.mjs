...
export const LOOPLAB_RUNTIME_KNOWLEDGE = Object.freeze({
  ...
  pixi: {
    role: "Renderer-first scene-graph adapter",
    adapterStatus: "release-ready-pinned-8.19.0",
  ...
  melon: {
    role: "Integrated Tiled-oriented 2D engine adapter",
    adapterStatus: "release-ready-pinned-17.4.0",
...
export const LOOPLAB_RUNTIME_SELECTION_POLICY = Object.freeze({
  schemaVersion: "looplab-runtime-selection-policy/v2",
  mode: "deterministic-quality-fit",
  defaultFramework: "canvas",
  automaticCandidates: ["canvas", "phaser", "pixi", "melon"],
  explicitCandidates: ["canvas", "phaser", "pixi", "melon"],
  releaseReadyAdapters: ["canvas", "phaser", "pixi", "melon"],
  phaserThreshold: 3,
  adapterThresholds: Object.freeze({ canvas: 0, phaser: 3, pixi: 3, melon: 3 }),
  ...
  singleFilePolicy: "Single-file delivery never disqualifies a release-ready adapter. The exact selected browser bundle and every selected asset must be inlined with no module, CDN, or runtime network dependency.",
...
export const GAME_STUDIO_CAPABILITIES = [
  ...
  {
    id: "pixijs-rendering",
    label: "PixiJS Rendering · strict-CSP inline UMD",
    owns: ["scene-graph-patterns", "texture-lifecycle", "render-groups", "ticker-ownership", "pointer-event-patterns", "inline-umd-delivery", "strict-csp-static-sync-polyfills"],
  ...
  {
    id: "melonjs-engine",
    label: "melonJS Engine · inline IIFE",
    owns: ["application-and-stage-patterns", "tiled-import-validation", "pool-lifecycle", "sparse-tile-layers", "collision-debug-patterns", "tree-shaken-inline-delivery"],
  ...
  },
  {
    ...
    applies: (context) => context.dimension === "2d" && (context.hasWorldStream || context.promptMatches(/continuous|chunk|district|panorama|endless|streaming|seam|scrolling world|closed loop/i)),
  },
  {
...
export function selectGameRuntime(project, request = {}) {
  ...
  const readyRanking = knowledgeRanking.filter(([framework]) => RELEASE_READY_2D_FRAMEWORKS.has(framework));
  const leadingReadyFramework = readyRanking[0]?.[0] ?? "canvas";
  const leadingReadyScore = readyRanking[0]?.[1] ?? 0;
  const leadingReadyThreshold = LOOPLAB_RUNTIME_SELECTION_POLICY.adapterThresholds[leadingReadyFramework] ?? Number.POSITIVE_INFINITY;
  const recommendedFramework = leadingReadyFramework !== "canvas" && leadingReadyScore >= leadingReadyThreshold && leadingReadyScore > scores.canvas ? leadingReadyFramework : "canvas";
  ...
  const scoreGap = Math.max(0, Number(readyRanking[0]?.[1] ?? 0) - Number(readyRanking[1]?.[1] ?? 0));
  ...
  if (explicitOverride && selectedFramework !== recommendedFramework) reasons.unshift(`${selectedFramework} was selected explicitly; LoopLab preserved that runtime choice instead of replacing it with the automatic recommendation.`);
  ...
  if (singleFile && selectedFramework !== "canvas") reasons.push(`Single-file delivery remains compatible: LoopLab must inline the exact pinned ${selectedFramework} browser bundle and all selected assets.`);
  ...
  return {
    ...
    confidence,
    scores,
    threshold: LOOPLAB_RUNTIME_SELECTION_POLICY.adapterThresholds[recommendedFramework] ?? 0,
    thresholds: LOOPLAB_RUNTIME_SELECTION_POLICY.adapterThresholds,
    signals,
    reasons,
    knowledgeReasons,
    singleFile: {
...
function normalizeContext(project, request = {}) {
  ...
  return {
    ...
    hasElevationTransitions: (project?.maps ?? [project]).some((map) => (map?.elevationTransitions?.transitions?.length ?? 0) > 0),
    hasWorldStream: (project?.maps ?? [project]).some((map) => map?.worldStream?.enabled !== false && map?.worldStream?.templates?.length > 0),
    requestedDimension,
...
export function routeGameStudioWork(project, request = {}) {
  ...
  const rendererBoundary = context.framework === "phaser"
      ...
      : context.framework === "pixi"
        ? "PixiJS owns the visible ticker and scene-graph presentation through one trusted script containing the pinned browser UMD followed by its same-version official strict-CSP static synchronizers; LoopLab's canonical 2D surface, simulation, authored collision, replay, and DOM UI remain authoritative."
        : context.framework === "melon"
          ? "melonJS owns the visible Canvas application and stage lifecycle through one pinned tree-shaken inline IIFE, a standalone Application, and an explicit camera with engine physics disabled; LoopLab simulation and authored collision remain authoritative."
  ...
  const productionPlan = {
    ...
    architecture: {
      rule: "The deterministic simulation, authored project data, and saveable state stay independent from the selected release-ready renderer. Canvas, Phaser, PixiJS, and melonJS are disposable presentation/frame adapters; exactly one may own a shipped build.",
