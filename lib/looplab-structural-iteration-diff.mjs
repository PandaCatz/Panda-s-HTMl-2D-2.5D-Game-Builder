...
import { normalizeWorldStream, worldStreamDigest } from "./looplab-world-stream.mjs";
...
function elevationTransitionSnapshot(transition = {}) {
  return {
    id: text(transition.id),
    label: text(transition.name, text(transition.id, "Elevation transition")),
    enabled: transition.enabled !== false,
    kind: text(transition.kind, "ramp"),
    width: finite(transition.width),
    entryRadius: finite(transition.entryRadius),
    entryZTolerance: finite(transition.entryZTolerance),
    oneWay: transition.oneWay === true,
    fromLayerId: text(transition.fromLayerId) || null,
    toLayerId: text(transition.toLayerId) || null,
    navigationLinkId: text(transition.navigationLinkId) || null,
    collisionChainId: text(transition.collisionChainId) || null,
    points: (Array.isArray(transition.points) ? transition.points : []).map((point, index) => ({
      id: text(point?.id, `point-${index + 1}`),
      x: finite(point?.x),
      y: finite(point?.y),
      z: finite(point?.z),
    })),
...
function worldStreamSnapshot(program) {
  if (!program || typeof program !== "object" || Array.isArray(program)) return null;
  const normalized = normalizeWorldStream(program);
  return {
    schemaVersion: normalized.schemaVersion,
    digest: worldStreamDigest(normalized),
    enabled: normalized.enabled,
    mode: normalized.mode,
    axis: normalized.axis,
    seed: normalized.seed,
    startTemplateId: normalized.startTemplateId,
    horizon: normalized.horizon,
    sequence: clone(normalized.sequence),
    budgets: clone(normalized.budgets),
    tolerances: clone(normalized.tolerances),
    templates: clone(normalized.templates),
  };
}

function worldStreamChangeKinds(first, second) {
  const kinds = [];
  if (first.enabled !== second.enabled) kinds.push("availability-changed");
  if (first.mode !== second.mode || first.axis !== second.axis) kinds.push("route-mode-changed");
  if (first.seed !== second.seed || first.startTemplateId !== second.startTemplateId || first.horizon !== second.horizon || !same(first.sequence, second.sequence)) kinds.push("route-selection-changed");
  if (!same(first.templates, second.templates)) kinds.push("templates-or-sockets-changed");
  if (!same(first.budgets, second.budgets)) kinds.push("residency-budget-changed");
  if (!same(first.tolerances, second.tolerances)) kinds.push("socket-tolerance-changed");
  return kinds;
}
...
function elevationTransitionChangeKinds(first, second) {
  const kinds = [];
  if (!same(first.points, second.points)) kinds.push("support-path-changed");
  if (first.kind !== second.kind) kinds.push("kind-changed");
  if (first.width !== second.width || first.entryRadius !== second.entryRadius || first.entryZTolerance !== second.entryZTolerance) kinds.push("corridor-changed");
  if (first.enabled !== second.enabled || first.oneWay !== second.oneWay) kinds.push("availability-changed");
  if (first.fromLayerId !== second.fromLayerId || first.toLayerId !== second.toLayerId || first.navigationLinkId !== second.navigationLinkId) kinds.push("navigation-binding-changed");
  if (first.collisionChainId !== second.collisionChainId) kinds.push("collision-binding-changed");
  return kinds;
}
...
function mapStructuralDiff(firstMap, secondMap, warnings) {
  ...
  const firstElevationTransitions = indexed(firstMap?.elevationTransitions?.transitions ?? [], `map ${mapId} elevation transition`, warnings);
  const secondElevationTransitions = indexed(secondMap?.elevationTransitions?.transitions ?? [], `map ${mapId} elevation transition`, warnings);
  const elevationTransitionChanges = diffIndexed(firstElevationTransitions, secondElevationTransitions, elevationTransitionSnapshot, elevationTransitionChangeKinds);
  ...
  const firstWorldStream = worldStreamSnapshot(firstMap?.worldStream);
  const secondWorldStream = worldStreamSnapshot(secondMap?.worldStream);
  const worldStreamChanges = !firstWorldStream && !secondWorldStream
    ? []
    : !firstWorldStream
      ? [changeRecord(`${mapId}:world-stream`, "added", null, secondWorldStream, ["added"])]
      : !secondWorldStream
        ? [changeRecord(`${mapId}:world-stream`, "removed", firstWorldStream, null, ["removed"])]
        : worldStreamChangeKinds(firstWorldStream, secondWorldStream).length
          ? [changeRecord(`${mapId}:world-stream`, "modified", firstWorldStream, secondWorldStream, worldStreamChangeKinds(firstWorldStream, secondWorldStream))]
          : [];

  const detailCount = objectChanges.length + colliderChanges.length + chainChanges.length + elevationTransitionChanges.length + tileColliderChanges.length + worldStreamChanges.length;
  return {
    mapId,
...
function summaryFor(maps, firstMapCount, secondMapCount) {
  ...
  const elevationTransitions = maps.flatMap((map) => map.elevationTransitionChanges);
  const tileColliders = maps.flatMap((map) => map.tileColliderChanges);
  const worldStreams = maps.flatMap((map) => map.worldStreamChanges);
  ...
  return {
    ...
    elevationTransitions: countsFor(elevationTransitions),
    tileColliders: countsFor(tileColliders),
    worldStreams: countsFor(worldStreams),
  };
}
...
function boundedMaps(maps, maximumDetailChanges) {
  ...
  return maps.map((map) => {
    const next = { ...map };
    let retained = 0;
    for (const field of ["objectChanges", "colliderChanges", "chainChanges", "elevationTransitionChanges", "tileColliderChanges", "worldStreamChanges"]) {
...
export function buildStructuralIterationDiff({ firstProject, secondProject, first = {}, second = {}, maximumDetailChanges = 512 } = {}) {
  ...
  const receipt = {
    ...
    policy: {
      identity: "Stable authored map/object/chain/elevation-transition/layer/world-stream template IDs only; array order, proximity, names, art, and pixels never establish identity.",
      ...
      elevationAuthority: "Authored elevation transitions own support-Z interpolation while their navigation and collision bindings retain independent route and contact authority.",
      worldStreamAuthority: "Authored chunk templates, sockets, selection policy, and residency budgets own continuous-world composition; screenshots and renderer state never establish adjacency.",
