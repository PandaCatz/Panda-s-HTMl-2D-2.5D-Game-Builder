...
function mapStructuralDiff(firstMap, secondMap, warnings) {
  ...
  const detailCount = objectChanges.length + colliderChanges.length + chainChanges.length + elevationTransitionChanges.length + tileColliderChanges.length;
  return {
    mapId,
...
function boundedMaps(maps, maximumDetailChanges) {
  ...
  return maps.map((map) => {
    const next = { ...map };
    let retained = 0;
    for (const field of ["objectChanges", "colliderChanges", "chainChanges", "elevationTransitionChanges", "tileColliderChanges"]) {
...
export function buildStructuralIterationDiff({ firstProject, secondProject, first = {}, second = {}, maximumDetailChanges = 512 } = {}) {
  ...
  const receipt = {
    ...
    policy: {
      identity: "Stable authored map/object/chain/elevation-transition/layer IDs only; array order, proximity, names, art, and pixels never establish identity.",
